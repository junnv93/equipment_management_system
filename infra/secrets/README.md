...
| 파일                   | 용도                          | 대상 compose                        |
| ---------------------- | ----------------------------- | ----------------------------------- |
| `onprem.env.sops.yaml` | 온프레미스 서버 배포용 secret | `infra/compose/onprem.override.yml` |
| `lan.env.sops.yaml`    | legacy LAN 배포용 secret      | `infra/compose/lan.override.yml`    |
...
3. 기존 메인테이너가 `sops updatekeys infra/secrets/onprem.env.sops.yaml` 로 기존 파일도 새 키로 재암호화
...
```bash
pnpm secrets:decrypt onprem    # → /run/secrets/onprem.env (tmpfs, 재부팅 시 소실)
pnpm secrets:decrypt lan       # → /run/secrets/lan.env (legacy)
pnpm secrets:decrypt prod
```

그 다음 compose 는 `/run/secrets/<env>.env` 를 참조합니다. 자동화된 경로는 `pnpm compose:onprem` / `pnpm compose:prod`.

### Secret 회전

- 값만 회전: `pnpm secrets:edit ENV=onprem` 으로 해당 값만 수정 후 커밋
...
| 4   | `infra/secrets/{onprem,prod}.env.sops.yaml` | 온프레미스/프로덕션 배포 | `pnpm secrets:edit ENV=onprem` / `ENV=prod`          |
...
```bash
TMP=$(mktemp) && sops decrypt --output-type dotenv infra/secrets/onprem.env.sops.yaml > "$TMP"
...
# 4) 온프레미스 / 프로덕션 sops 에도 반영
pnpm secrets:edit ENV=onprem   # $EDITOR 에서 FOO_SECRET 추가 → 저장 시 자동 재암호화
pnpm secrets:edit ENV=prod
