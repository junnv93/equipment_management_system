...
describe('Site Permissions (e2e)', () => {
    ...
    if (ctx?.app && adminToken) {
      try {
        await request(ctx.app.getHttpServer())
          .delete(API_ENDPOINTS.EQUIPMENT.DELETE(suwonEquipmentUuid))
      ...
      try {
        await request(ctx.app.getHttpServer())
          .delete(API_ENDPOINTS.EQUIPMENT.DELETE(uiwangEquipmentUuid))
          .set('Authorization', `Bearer ${adminToken}`);
      } catch {
        // 무시
      }
  ...
  describe('사이트별 조회 권한', () => {
    ...
    it('시험실무자는 다른 사이트 장비 상세 조회 시 접근 제한될 수 있음', async () => {
      ...
      const response = await request(ctx.app.getHttpServer())
        .get(API_ENDPOINTS.EQUIPMENT.GET(uiwangEquipmentUuid))
        .set('Authorization', `Bearer ${testOperatorToken}`);
  ...
  describe('장비 등록 시 사이트 필수', () => {
    ...
    it('장비 등록 시 site 필드가 있으면 성공해야 함', async () => {
      ...
      if (response.body.id || response.body.data?.id) {
        await request(ctx.app.getHttpServer())
          .delete(API_ENDPOINTS.EQUIPMENT.DELETE(response.body.id || response.body.data?.id))
          .set('Authorization', `Bearer ${adminToken}`);
      }
    });
  });
  ...
  describe('팀별 권한 제한', () => {
    ...
    afterAll(async () => {
      if (rfEquipmentUuid && adminToken) {
        try {
          await request(ctx.app.getHttpServer())
            .delete(API_ENDPOINTS.EQUIPMENT.DELETE(rfEquipmentUuid))
            .set('Authorization', `Bearer ${adminToken}`);
        } catch {
          // 이미 삭제된 경우 무시
        }
