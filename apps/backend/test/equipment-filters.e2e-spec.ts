...
describe('Equipment Filters (e2e)', () => {
  ...
  describe('GET /equipment?classification= - 장비 분류 필터', () => {
    it('classification=fcc_emc_rf 필터 적용', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?classification=fcc_emc_rf`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('items');
    ...
    it('classification=sar 필터 적용', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?classification=sar`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('items');
    ...
    it('잘못된 classification 값은 Zod 검증 실패 (400)', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?classification=invalid_value`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(400);
    });
  });
  ...
  describe('GET /equipment?calibrationOverdue= - 교정 기한 초과 필터', () => {
    it('calibrationOverdue=true 필터 적용', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?calibrationOverdue=true`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('items');
    ...
    it('calibrationOverdue=false는 기한 초과 제외', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?calibrationOverdue=false`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200);
    });
  });
  ...
  describe('GET /equipment?status= & calibrationOverdue= - 복합 필터', () => {
    it('status=available & calibrationOverdue=true 조합', () => {
      return request(ctx.app.getHttpServer())
        .get(
          `${API_ENDPOINTS.EQUIPMENT.LIST}?status=available&calibrationOverdue=true`,
        )
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
  ...
  describe('GET /equipment?managementMethod= - 교정 방법 필터', () => {
    it('managementMethod=external_calibration 필터 적용', () => {
      return request(ctx.app.getHttpServer())
        .get(
          `${API_ENDPOINTS.EQUIPMENT.LIST}?managementMethod=external_calibration`,
        )
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
    ...
    it('managementMethod=self_inspection 필터 적용', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?managementMethod=self_inspection`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('items');
    ...
    it('managementMethod=not_applicable 필터 적용', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?managementMethod=not_applicable`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('items');
  ...
  describe('GET /equipment - 교정 기한 필터 (calibrationDue/calibrationDueAfter)', () => {
    it('calibrationDue=30 - 30일 이내 교정 임박 장비', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?calibrationDue=30`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('items');
    ...
    it('calibrationDueAfter=30 - 30일 이후 교정 여유 장비', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?calibrationDueAfter=30`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('items');
  ...
  describe('GET /equipment - 상태 필터 (EQUIPMENT_STATUS_FILTER_OPTIONS)', () => {
    ...
    validStatuses.forEach((status) => {
      it(`status=${status} 필터 적용`, () => {
        return request(ctx.app.getHttpServer())
          .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?status=${status}`)
          .set('Authorization', `Bearer ${authToken}`)
          .expect(200)
          .expect((res) => {
            expect(res.body).toHaveProperty('items');
    ...
    it('calibration_overdue는 유효한 EquipmentStatus가 아님 (Zod 400)', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?status=calibration_overdue`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(400);
    });
    ...
    it('retired는 유효한 EquipmentStatus가 아님 (Zod 400)', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?status=retired`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(400);
    });
  });
  ...
  describe('GET /equipment - 복잡한 필터 조합', () => {
    it('classification + status + managementMethod 조합', () => {
      return request(ctx.app.getHttpServer())
        .get(
          `${API_ENDPOINTS.EQUIPMENT.LIST}?classification=fcc_emc_rf&status=available&managementMethod=external_calibration`,
        )
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
        .expect((res) => {
  ...
  describe('GET /equipment - 파라미터 검증', () => {
    it('음수 페이지는 Zod 검증 실패 (400)', () => {
      return request(ctx.app.getHttpServer())
        .get(`${API_ENDPOINTS.EQUIPMENT.LIST}?page=-1`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(400);
    });
  });
