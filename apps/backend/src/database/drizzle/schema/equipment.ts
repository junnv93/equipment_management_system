import { integer, pgEnum, pgTable, serial, text, timestamp, boolean, varchar, index } from 'drizzle-orm/pg-core';
...
export const equipment = pgTable('equipment', {
  ...
  return {
    ...
    nextCalibrationDateIdx: index('equipment_next_calibration_date_idx').on(table.nextCalibrationDate),
    ...
    calibrationDueIdx: index('equipment_calibration_due_idx').on(table.isActive, table.nextCalibrationDate),
  };
});
...
// Zod 스키마 생성 (타입 문제로 인해 as const로 타입 고정)
// @ts-ignore - drizzle-zod 타입 불일치 문제 무시
export const insertEquipmentSchema = createInsertSchema(equipment as any);

// 추가 검증이 필요한 필드는 별도로 확장
export const equipmentValidationSchema = insertEquipmentSchema.extend({
...
// @ts-ignore - drizzle-zod 타입 불일치 문제 무시
export const selectEquipmentSchema = createSelectSchema(equipment as any);
