...
async function seed() {
  ...
  try {
    ...
    // =========================================================================
    console.log('\n🔧 테스트 장비 생성 중...');
    const testEquipments = await db.insert(equipment).values([
      // === 수원 사이트 장비 (10개) ===
    ...
    // =========================================================================
    console.log('\n📍 장비 위치 변동 이력 생성 중...');
    const locationHistoryData = await db.insert(equipmentLocationHistory).values([
      // 스펙트럼 분석기 위치 변동
      {
    ...
    // =========================================================================
    console.log('\n🔧 장비 유지보수 이력 생성 중...');
    const maintenanceHistoryData = await db.insert(equipmentMaintenanceHistory).values([
      // 스펙트럼 분석기
      {
        equipmentId: EQUIP_SPECTRUM_ANALYZER_ID,
    ...
    // =========================================================================
    console.log('\n⚠️ 장비 사고/손상 이력 생성 중...');
    const incidentHistoryData = await db.insert(equipmentIncidentHistory).values([
      // 네트워크 분석기 - 부적합 원인
      {
        equipmentId: EQUIP_NETWORK_ANALYZER_ID,
    ...
    // =========================================================================
    console.log('\n📋 교정 기록 생성 중...');
    const calibrationsData = await db.insert(calibrations).values([
      // 승인됨 (approved) - 6건
      {
        id: CALIB_001_ID,
        equipmentId: EQUIP_SPECTRUM_ANALYZER_ID,
    ...
    // =========================================================================
    console.log('\n📦 반출 항목 생성 중...');
    const checkoutItemsData = await db.insert(checkoutItems).values([
