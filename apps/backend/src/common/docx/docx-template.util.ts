...
export class DocxTemplate {
  ...
  setDataRows(
    ...
    for (const data of dataRows) {
      ...
      if (data.length > cells.length) {
        throw new InternalServerErrorException(
          `[${this.formLabel}] setDataRows 실패: 데이터 열(${data.length}개)이 템플릿 셀(${cells.length}개)보다 많음. 양식의 열 구조가 변경되었을 수 있습니다.`
        );
      }
      data.forEach((val, ci) => {
        if (ci < cells.length) {
   ...
   *
   * 하나의 셀 안에 텍스트 단락과 이미지를 번갈아 배치합니다.
