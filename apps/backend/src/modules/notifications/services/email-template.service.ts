...
export class EmailTemplateService {
  ...
  buildTemporaryPasswordEmail(data: TemporaryPasswordData): EmailContent {
    return {
      subject: '[장비 관리 시스템] 임시 비밀번호가 발급되었습니다',
      html: this.wrapLayout(
        '임시 비밀번호 안내',
        `
        <p style="margin:0 0 16px;color:#333333;font-size:15px;line-height:1.6;">
          안녕하세요, <strong>${this.escapeHtml(data.userName)}</strong>님.<br>
          임시 비밀번호가 발급되었습니다. 아래 비밀번호로 로그인 후 즉시 변경해 주세요.
        </p>
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px;border:1px solid #e5e7eb;border-radius:4px;">
          <tr>
            <td style="padding:12px 16px;background-color:#f9fafb;font-weight:600;color:#374151;width:140px;">임시 비밀번호</td>
            <td style="padding:12px 16px;color:#1e40af;font-weight:700;font-size:18px;letter-spacing:2px;">${this.escapeHtml(data.tempPassword)}</td>
          </tr>
        </table>
        <p style="margin:0 0 24px;color:#6b7280;font-size:13px;line-height:1.6;">
          ⚠️ 보안을 위해 이 임시 비밀번호는 최초 로그인 후 반드시 변경해 주세요.
        </p>
        ${this.buildCtaButton('로그인하기', data.loginUrl)}
        `
      ),
    };
  }
