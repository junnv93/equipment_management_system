...
export const updateCableSchema = z.object({
  ...versionedSchema,
  managementNumber: z.string().trim().max(20, VM.string.max('관리번호', 20)).optional(),
  length: z.string().trim().max(20, VM.string.max('길이', 20)).optional(),
  ...
  serialNumber: z.string().trim().max(100, VM.string.max('시리얼번호', 100)).optional(),
  location: z.string().trim().max(50, VM.string.max('위치', 50)).optional(),
