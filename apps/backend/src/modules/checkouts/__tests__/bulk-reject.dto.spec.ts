 ...
 *   - reason: min(REJECTION_REASON_MIN_LENGTH) / max(LONG_TEXT_MAX_LENGTH) / trim
...
describe('bulkRejectSchema', () => {
  ...
  describe('reason 검증', () => {
    ...
    it('(MIN-1)자 → 실패 (단건 reject와 동일 min 기준)', () => {
      const result = bulkRejectSchema.safeParse({
        ids: [VALID_UUID],
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe(VM.string.min('반려 사유', MIN));
      }
    });
    ...
    it(`${MAX + 1}자 → 실패 (max ${MAX})`, () => {
      const reason = 'x'.repeat(MAX + 1);
      const result = bulkRejectSchema.safeParse({ ids: [VALID_UUID], reason });
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe(VM.string.max('반려 사유', MAX));
      }
    });
  });
