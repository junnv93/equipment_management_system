...
export class EquipmentController {
  ...
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: '권한 없음' })
  @RequirePermissions(Permission.UPDATE_EQUIPMENT)
  updateStatus(@Param('id') id: string, @Body('status') status: EquipmentStatusEnum) {
    return this.equipmentService.updateStatus(id, status);
  }
  
  @Get('team/:teamId')
  @ApiOperation({ summary: '팀별 장비 목록 조회', description: '특정 팀에 할당된 장비 목록을 조회합니다.' })
  ...
  @Get('calibration/due')
  @ApiOperation({ summary: '교정 예정 장비 조회', description: '특정 기간 내에 교정이 예정된 장비를 조회합니다.' })
