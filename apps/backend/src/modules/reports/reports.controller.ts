...
export class ReportsController {
  ...
  @ApiQuery({ name: 'equipmentId', required: false, description: '특정 장비 ID (미지정 시 전체 장비)' })
  @ApiQuery({ name: 'departmentId', required: false, description: '특정 부서 ID (미지정 시 전체 부서)' })
  ...
  @ApiOperation({ summary: '교정 상태 보고서', description: '장비 교정 상태에 대한 통계를 제공합니다.' })
  @ApiQuery({ name: 'status', required: false, description: '교정 상태 (due, overdue, completed)' })
  @ApiQuery({ name: 'timeframe', required: false, description: '기간 (week, month, quarter, year)' })
  ...
  @ApiQuery({ name: 'period', required: false, description: '기간 (week, month, quarter, year)', enum: ['week', 'month', 'quarter', 'year'] })
  @ApiQuery({ name: 'equipmentId', required: false, description: '특정 장비 ID (미지정 시 전체 장비)' })
  @ApiQuery({ name: 'categoryId', required: false, description: '특정 장비 카테고리 ID (미지정 시 전체 카테고리)' })
  ...
  @ApiOperation({ summary: '장비 가동 중단 보고서', description: '장비 유지보수 및 수리로 인한 가동 중단 통계를 제공합니다.' })
  @ApiQuery({ name: 'startDate', required: false, description: '시작 날짜 (ISO 형식)' })
  @ApiQuery({ name: 'endDate', required: false, description: '종료 날짜 (ISO 형식)' })
  @ApiQuery({ name: 'equipmentId', required: false, description: '특정 장비 ID (미지정 시 전체 장비)' })
  ...
  @ApiOperation({ summary: '장비 사용 보고서 내보내기', description: '장비별 사용 통계를 Excel 또는 CSV 형식으로 내보냅니다.' })
  @ApiQuery({ name: 'format', required: true, description: '내보내기 형식 (excel, csv)', enum: ['excel', 'csv'] })
  @ApiQuery({ name: 'startDate', required: false, description: '시작 날짜 (ISO 형식)' })
  @ApiQuery({ name: 'endDate', required: false, description: '종료 날짜 (ISO 형식)' })
