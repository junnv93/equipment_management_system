...
export function useInboundSectionPagination() {
  ...
  const inboundPage = Math.max(1, Number(searchParams.get('inboundPage') ?? '1'));
  ...
  const setInboundPage = (page: number) => navigate({ inboundPage: String(page) });
  ...
  return { inboundPage, rentalPage, internalPage, setInboundPage, setRentalPage, setInternalPage };
}
