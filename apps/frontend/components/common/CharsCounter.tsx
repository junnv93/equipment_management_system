...
/**
 * 문자 수 카운터 SSOT — count/max 임계값 기반 색상 자동 토글.
 *
 * 색상 정책:
...
export const CharsCounter = memo(function CharsCounter({
  count,
  max,
  warningRatio = CHAR_COUNTER_TOKENS.warningRatio,
  className,
  ariaLive = 'polite',
  children,
}: CharsCounterProps) {
  ...
  return (
    <span
      ...
      role="status"
    >
      {children ?? t('charCounter.ratio', { count, max })}
    </span>
  );
});
