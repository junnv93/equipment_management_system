...
test.describe('반출/반입 관리 페이지', () => {
  test.describe('반출 목록 페이지', () => {
    ...
    // FIXME: Superseded by Group A tests (a1-list-basic.spec.ts A-3, A-4)
    // Issue: Uses waitForResponse antipattern, search placeholder may not match
    test.fixme('검색 기능이 정상 동작함', async ({ page }) => {
      await page.goto('/checkouts');
    ...
    // FIXME: Superseded by Group A tests (a2-list-filters.spec.ts A-5)
    // Issue: Status filter selector doesn't match actual UI (combobox vs button)
    test.fixme('상태 필터가 정상 동작함', async ({ page }) => {
      await page.goto('/checkouts');
  ...
  test.describe('반출 신청 페이지', () => {
    ...
    // FIXME: Superseded by Group B tests (b1-create-validation.spec.ts B-1)
    // Issue: Submit button is correctly disabled when no equipment is selected
    test.fixme('필수 필드 미입력 시 유효성 검사 오류 표시', async ({ page }) => {
      await page.goto('/checkouts/create');
  ...
  test.describe('반출 상세 페이지', () => {
    ...
    // FIXME: Superseded by Group A tests (a3-detail-page.spec.ts A-9)
    // Issue: Row click navigation may not work; section headings may differ from expectations
    test.fixme('반출 상세 페이지 구조 확인', async ({ page }) => {
  ...
  test.describe('반출 승인 관리 페이지 (기술책임자)', () => {
    ...
    // FIXME: /checkouts/manage now redirects to /admin/approvals?tab=checkout
    // Superseded by Group C tests (c1-permission-checks.spec.ts, c2-approval-flow.spec.ts)
    test.fixme('반출 승인 관리 페이지가 정상적으로 렌더링됨', async ({ page }) => {
  ...
  test.describe('반응형 디자인', () => {
    // FIXME: Table width assertion is fragile - table may overflow with horizontal scroll
    // This is a P3 responsive design test, not critical for deployment
    test.fixme('모바일 뷰에서 목록이 카드 형태로 표시', async ({ page }) => {
      // 모바일 뷰포트 설정
...
test.describe('권한 검증 테스트', () => {
  // FIXME: /checkouts/manage now redirects to /admin/approvals?tab=checkout
  // Superseded by Group F tests (f1-team-constraints.spec.ts, f2-role-permissions.spec.ts)
  test.fixme('기술책임자는 승인 관리 페이지 접근 가능', async ({ page }) => {
    await loginAs(page, TEST_USERS.technicalManager.role);
    await page.goto('/checkouts/manage');
