 ...
 *
 * Frontend 가 이미 sticky header height CSS 변수(SSOT: `CSS_VAR_NAMES.stickyHeaderHeight`)를
 * ResizeObserver 로 동적 갱신하므로 (apps/frontend/components/equipment/EquipmentDetailClient.tsx
 * 참조), e2e 도 동일 SSOT 를 읽어 sticky 영역 높이만큼 보정 스크롤을 수행한다.
 *
...
// SSOT: lib/design-tokens/css-variables.ts (CSS variable name 단일 진입점)
// e2e 는 `@/` path alias 미사용 — relative import 로 직접 참조
import { CSS_VAR_NAMES } from '../../../../lib/design-tokens/css-variables';
 ...
 * 2) `CSS_VAR_NAMES.stickyHeaderHeight` 변수를 읽어 element top 이 sticky 아래로 가도록 미세 보정
...
export async function clickBelowStickyHeader(page: Page, locator: Locator): Promise<void> {
  ...
  const stickyHeight = await page.evaluate((varName) => {
    const raw = getComputedStyle(document.documentElement).getPropertyValue(varName);
    const parsed = parseFloat(raw);
    // 변수 미설정 시 0 반환 — EquipmentDetailClient 언/마운트 시점에는
