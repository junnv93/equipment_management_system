...
test.describe('Incident → Non-Conformance → Equipment Status Update', () => {
  ...
  test('Issue 1: 부적합 등록 시 장비 상태가 즉시 non_conforming으로 변경되어야 함', async ({
    ...
    await techManagerPage.waitForResponse(
  ...
  test('Issue 3: 사고 이력 삭제 시 UI가 즉시 갱신되어야 함', async ({ techManagerPage }) => {
    ...
    const cardContainer = createdItem.locator('xpath=ancestor::div[contains(@class, "flex") and contains(@class, "justify-between")]');
    ...
    const [deleteResponse] = await Promise.all([
      techManagerPage.waitForResponse(
        (response) => response.url().includes('/incident-history/') && response.request().method() === 'DELETE',
        { timeout: 10000 }
      ).catch((e) => {
