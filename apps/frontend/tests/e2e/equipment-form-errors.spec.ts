...
test.describe('Equipment Form - Error Handling', () => {
  ...
  test('필수 필드 누락 시 에러 메시지 표시', async ({ page }) => {
    ...
    // 장비명 필수 에러 확인
    await expect(
      page.getByText(/장비명.*필수|장비명.*2자.*이상/i).first()
    ).toBeVisible({ timeout: 5000 });

    // 관리번호 필수 에러 확인
    await expect(
      page.getByText(/관리번호.*필수|관리번호.*2자.*이상/i).first()
    ).toBeVisible({ timeout: 5000 });
  });
...
test.describe('Equipment Form - Calibration History Errors', () => {
  ...
  test('교정 이력 추가 시 필수 필드 검증', async ({ page }) => {
    // 교정 이력 섹션으로 스크롤
    await page
      .getByRole('heading', { name: /교정 이력/i })
  ...
  test('교정일이 차기 교정일보다 이후인 경우 에러', async ({ page }) => {
    ...
    if (await addButton.isVisible()) {
      ...
      // 날짜 관련 에러 메시지 확인
      await expect(
        page.getByText(/차기 교정일.*이후|날짜.*오류/i)
      ).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('교정 이력 삭제 확인 다이얼로그', async ({ page }) => {
    ...
    if (await addButton.isVisible()) {
      ...
      if (await deleteButton.isVisible()) {
        ...
        // 삭제 확인 다이얼로그 확인
        await expect(
          page.getByText(/삭제.*확인|삭제.*시겠습니까/i)
        ).toBeVisible({ timeout: 5000 });
      }
    }
  });
...
test.describe('Equipment Form - API Error Simulation', () => {
  ...
  test('존재하지 않는 장비 수정 시 에러 처리', async ({ page }) => {
    ...
    await expect(
      page.getByText(/오류|찾을 수 없|not found|로딩|없습니다/i).first()
  ...
  test('네트워크 오류 시뮬레이션', async ({ page, context }) => {
    ...
    await expect(
      page.getByText(/네트워크|연결|오류|실패/i).first()
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('중복 관리번호 에러 처리', async ({ page }) => {
    ...
    // 중복 에러 메시지 확인
    await expect(
      page.getByText(/중복|이미.*등록|관리번호/i).first()
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('서버 에러 (500) 처리', async ({ page }) => {
    ...
    // 서버 에러 메시지 확인
    await expect(
      page.getByText(/서버|오류|실패|error/i).first()
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('권한 없음 (403) 에러 처리', async ({ page }) => {
    ...
    // 권한 에러 메시지 확인
    await expect(
      page.getByText(/권한|없습니다|forbidden/i).first()
    ).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe('Equipment Form - ErrorAlert Component', () => {
  ...
  test('ErrorAlert 닫기 버튼 동작', async ({ page }) => {
    ...
    if (await closeButton.isVisible()) {
      ...
      // Alert가 사라졌는지 확인
      await expect(errorAlert).toBeHidden({ timeout: 5000 }).catch(() => {
  ...
  test('다시 시도 버튼 동작', async ({ page }) => {
    ...
    // 에러 메시지 확인
    await expect(
      page.getByText(/서버|오류|실패/i).first()
    ).toBeVisible({ timeout: 10000 });
...
test.describe('Equipment Edit Form - Error Handling', () => {
  ...
  test('수정 페이지 로드 실패 시 에러 표시', async ({ page }) => {
    ...
    // 에러 상태 확인
    await expect(
      page.getByText(/오류|찾을 수 없|로딩|없습니다/i).first()
    ).toBeVisible({ timeout: 10000 });

    // 뒤로 가기 버튼 확인
    await expect(
      page.getByRole('button', { name: /뒤로|돌아가기/i })
