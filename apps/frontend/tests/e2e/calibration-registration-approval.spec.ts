...
test.describe('Calibration Registration and Approval Workflow', () => {
  ...
  test.describe('교정 기록 등록 폼 접근', () => {
    test('시험실무자는 장비 상세 페이지에서 교정 등록 버튼을 볼 수 있다', async ({
      ...
      await testOperatorPage.waitForLoadState('domcontentloaded');
      // 탭 패널 동적 로딩 대기
      await expect(testOperatorPage.getByRole('heading', { name: /교정 이력/i })).toBeVisible({ timeout: 15000 });
    ...
    test('기술책임자도 장비 상세 페이지에서 교정 등록 버튼을 볼 수 있다', async ({
      ...
      await techManagerPage.waitForLoadState('domcontentloaded');
      // 탭 패널 동적 로딩 대기
      await expect(techManagerPage.getByRole('heading', { name: /교정 이력/i })).toBeVisible({ timeout: 15000 });
  ...
  test.describe('승인 대기 목록 접근', () => {
    ...
    test('시험소장도 교정 승인 대기 목록 페이지에 접근할 수 있다', async ({
      siteAdminPage,
    }) => {
  ...
  test.describe('파일 업로드 필드', () => {
    test('교정성적서 파일 필드가 표시되고 파일 선택이 가능하다', async ({
      testOperatorPage,
    }) => {
      // 장비 목록 페이지로 이동
      await testOperatorPage.goto('/equipment');
    ...
    test('파일 미첨부 시 경고 메시지가 표시된다', async ({ testOperatorPage }) => {
      ...
      // 파일 미첨부 경고 메시지 확인
      await expect(
        testOperatorPage.getByText(/교정성적서 파일을 첨부해주세요/i)
      ).toBeVisible();
    });
  });
  ...
  test.describe('교정 이력 테이블', () => {
    test('교정 이력 탭에서 테이블 또는 빈 상태 메시지가 표시된다', async ({ testOperatorPage }) => {
      ...
      const tableOrEmpty = testOperatorPage.locator(
        'table, :text("등록된 교정 이력이 없습니다"), :text("교정 이력이 없습니다")'
      ).first();
