    ...
    <div className="space-y-6 p-6">
      ...
      <Tabs defaultValue="all" value={activeView} onValueChange={setActiveView}>
        ...
        <TabsContent value="equipment" className="space-y-6">
          <Card>
            <CardHeader>
              <h3 className="text-lg font-semibold leading-none tracking-tight" role="heading" aria-label="장비 현황">장비 현황</h3>
                  ...
                  <div className="space-y-2">
                    {isLoading ? (
                      ...
                      equipmentByTeam?.map((team) => (
                        <div key={team.id} className="flex items-center justify-between p-2 bg-card rounded border">
                          <span>{team.name}</span>
            ...
            <div className="space-y-2">
              {isLoading ? (
                ...
                overdueCalibrations?.map((calibration) => (
                  <div key={calibration.id} className="flex items-center justify-between p-2 bg-card rounded border">
            ...
            <div className="space-y-2">
              {isLoading ? (
                ...
                overdueRentals?.map((rental) => (
                  <div key={rental.id} className="flex items-center justify-between p-2 bg-card rounded border">
                    <span>{rental.name}</span>
                    <span className="text-sm text-muted-foreground">{rental.dueDate}</span>
                  </div>
                ))
              )}
            </div>
