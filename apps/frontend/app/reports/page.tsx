...
export default function ReportsPage() {
  ...
  const [lastGeneratedReport, setLastGeneratedReport] = useState<ReportGenerationResult | null>(null);
  ...
  const { mutate: generateReportMutation, isPending } = useGenerateReport({
    ...
    onError: (error: any) => {
      toast({
        title: "보고서 생성 실패",
        description: `보고서 생성 중 오류가 발생했습니다: ${error.message}`,
        variant: "destructive",
      });
    },
  });
  ...
  const getReportTypeLabel = (type: ReportType) => {
    switch (type) {
      case "equipment_inventory": return "장비 인벤토리";
      case "calibration_status": return "교정 상태";
      case "utilization_report": return "활용률";
      case "team_equipment": return "팀별 장비";
      case "maintenance_report": return "점검 보고서";
      default: return "";
    }
  };
  ...
  const getReportIcon = (type: ReportType) => {
    switch (type) {
      case "equipment_inventory": return <FileText className="mr-2 h-4 w-4" />;
      case "calibration_status": return <Clipboard className="mr-2 h-4 w-4" />;
      case "utilization_report": return <BarChart className="mr-2 h-4 w-4" />;
      case "team_equipment": return <FileText className="mr-2 h-4 w-4" />;
      case "maintenance_report": return <Clipboard className="mr-2 h-4 w-4" />;
  ...
  const getReportDescription = (type: ReportType) => {
    switch (type) {
      case "equipment_inventory": return "모든 장비의 현재 상태와 세부 정보가 포함된 인벤토리 보고서입니다.";
      case "calibration_status": return "모든 장비의 교정 일정과 상태가 포함된 교정 상태 보고서입니다.";
      case "utilization_report": return "장비별 사용 통계와 가용성 분석이 포함된 활용률 보고서입니다.";
      case "team_equipment": return "각 팀이 보유한 장비 목록과 현황이 포함된 팀별 장비 보고서입니다.";
      case "maintenance_report": return "장비 유지보수 일정 및 이력이 포함된 점검 보고서입니다.";
      default: return "";
    }
  };
  ...
  const getFormatIcon = (format: ReportFormat) => {
    switch (format) {
      ...
      case "csv": return <FileCSV className="mr-2 h-4 w-4" />;
      case "pdf": return <File className="mr-2 h-4 w-4" />;
      default: return <FileText className="mr-2 h-4 w-4" />;
    }
  };
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {reportType ? getReportDescription(reportType as ReportType) : "보고서 유형을 선택하면 자세한 정보가 표시됩니다."}
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <div className="border rounded-lg p-4 bg-slate-50">
                    ...
                    <ul className="space-y-2 text-sm">
                      <li className="flex justify-between">
                        ...
                        <span className="font-medium">{getReportTypeLabel(reportType as ReportType)}</span>
                      </li>
                      <li className="flex justify-between">
                      ...
                      {reportType === "equipment_inventory" && categoryId && (
                        <li className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {categoryId === "test" ? "테스트 장비" :
                             categoryId === "computer" ? "컴퓨터" :
                             categoryId === "network" ? "네트워크 장비" : categoryId}
                          </span>
                        </li>
                      )}
                      ...
                      {reportType === "team_equipment" && departmentId && (
                        <li className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {departmentId === "1" ? "연구개발팀" :
                             departmentId === "2" ? "품질관리팀" :
                             departmentId === "3" ? "생산팀" :
                             departmentId === "4" ? "시험팀" : departmentId}
                          </span>
                        </li>
                      )}
                    </ul>
