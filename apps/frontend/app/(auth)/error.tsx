        ...
        <Card className="border-0 shadow-xl rounded-2xl bg-card">
          <CardHeader className="text-center pt-8 pb-4">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-ul-orange/20">
              <AlertTriangle className="h-8 w-8 text-ul-orange" aria-hidden="true" />
            </div>
            <h1 className="text-2xl font-bold text-foreground">{t('errorBoundary.title')}</h1>
          </CardHeader>

          <CardContent className="px-8 pb-4">
            <p className="text-center text-muted-foreground" role="alert" aria-live="polite">
              {t('errorBoundary.description')}
              <br />
              {t('errorBoundary.descriptionRetry')}
            </p>
          ...
          <CardFooter className="flex flex-col gap-3 px-8 pb-8">
            <Button
              onClick={reset}
              className="w-full h-12 text-base font-medium bg-ul-midnight hover:bg-ul-midnight-dark motion-safe:transition-all motion-reduce:transition-none duration-200 hover:scale-[1.02] active:scale-[0.98]"
            ...
            <Button
              asChild
              variant="outline"
              className="w-full h-12 text-base font-medium border-border hover:bg-muted motion-safe:transition-all motion-reduce:transition-none duration-200"
              aria-label={t('errorBoundary.goToLogin')}
            >
              <Link href="/login">
