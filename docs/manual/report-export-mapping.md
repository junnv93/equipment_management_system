...
| 데이터 소스 | `equipment`                                                                                                                                                                    |
| 출력 형식   | XLSX (템플릿 기반)                                                                                                                                                             |
| 행 제한     | `EXPORT_QUERY_LIMITS.FULL_EXPORT`                                                                                                                                              |
| 시트명      | `SHEET_NAMES` SSOT (변종 fallback: `시험설비 관리대장` / `시험설비 관리 대장`)                                                                                                 |
| 아키텍처    | 3-way 분리: `EquipmentRegistryDataService` + `EquipmentRegistryRendererService` + `xlsx-helper` (2026-04-17 reports/layouts/equipment-registry.layout.ts SSOT)                 |
| SSOT 라벨   | `MANAGEMENT_METHOD_LABELS` (관리방법), `EQUIPMENT_AVAILABILITY_LABELS` (사용/고장/여분/불용), `INTERMEDIATE_CHECK_YESNO_LABELS` (O/X) — `packages/schemas/src/enums/labels.ts` |
| 양식 파일   | `docs/procedure/template/UL-QP-18-01(02) 시험설비 관리대장.xlsx` (2026-04-17 교체, 38KB 순수 템플릿)                                                                           |
...
| 아키텍처      | 3-way 분리: `IntermediateInspectionExportDataService` + `IntermediateInspectionRendererService` + `docx-xml-helper` (2026-04-17 layout SSOT) |
| SSOT 라벨     | `QP18_CLASSIFICATION_LABELS` (교정기기/비교정기기), `INSPECTION_JUDGMENT_LABELS` (합격/불합격) — `packages/schemas/src/enums/labels.ts`      |
| 양식 파일     | `docs/procedure/template/UL-QP-18-03(02) 중간점검표.docx` (2026-04-17 교체)                                                                  |
...
| 아키텍처      | 3-way 분리: `SelfInspectionExportDataService` + `SelfInspectionRendererService` + `docx-xml-helper` (2026-04-17 layout SSOT)                      |
| SSOT 라벨     | `QP18_CLASSIFICATION_LABELS`, `SELF_INSPECTION_RESULT_LABELS` (이상 없음/부적합/N/A) — `packages/schemas/src/enums/labels.ts`                     |
| Snapshot 컬럼 | `classification` + `calibration_validity_period` (drizzle 0031, 2026-04-17) — 장비 마스터 drift 방지. record 우선, 없으면 장비 마스터에서 derive. |
| 양식 파일     | `docs/procedure/template/UL-QP-18-05(01) 자체점검표.docx` (2026-04-17 교체)                                                                       |

**양식 파일 교체 절차:**

- **개발 환경**: `docs/procedure/template/` 파일 교체 → `pnpm --filter backend run db:reset` → `seedFromFilesystem`가 자동 self-heal (해시 불일치 감지 → 새 파일 재시드)
- **운영 환경**: `POST /api/form-templates/replace` (권한: `MANAGE_FORM_TEMPLATES`) + `changeSummary` 필수. 기존 storageKey 보존, 새 버전 레코드 추가.
...
### 3.10 UL-QP-18-11: 보정인자 및 파라미터 관리대장 (DOCX)

| 항목        | 값                                                          |
| ----------- | ----------------------------------------------------------- |
| 엔드포인트  | `GET /api/reports/export/form/UL-QP-18-11`                  |
| 데이터 소스 | `calibration_factors` + `equipment` + `users`               |
| 출력 형식   | DOCX                                                        |
| 구현 파일   | `modules/reports/services/calibration-factor-register-*.ts` |

**셀 매핑 (단일 테이블, 7열):**

| 열  | 헤더                 | DB 필드 / 변환                                                   |
| --- | -------------------- | ---------------------------------------------------------------- |
| 1   | 순번                 | export row sequence                                              |
| 2   | 장비관리번호         | `equipment.management_number`                                    |
| 3   | 장비명               | `equipment.name`                                                 |
| 4   | 보정인자 및 파라미터 | `factor_name: factor_value unit` + optional `parameters` summary |
| 5   | 적용일               | `calibration_factors.effective_date`                             |
| 6   | 확인                 | `users.name` (`approved_by`)                                     |
| 7   | 변경일               | `calibration_factors.updated_at`                                 |
...
| DOCX | QP-18-03, QP-18-05, QP-18-06, QP-18-07, QP-18-09, QP-18-11 |
