...
- **verify-hardcoding** — 하드코딩 탐지 (API paths, queryKeys, env, cache keys, token TTL, ErrorCode, Korean UI). Step 38: useUndoToast invalidateKeys `CheckoutCacheInvalidation.approvalWithDetailKeys(id)` 팩토리 강제 — `[detail(id), ...APPROVAL_KEYS]` 인라인 spread 금지 (2026-05-13)
- **verify-frontend-state** — TanStack Query (no onSuccess setQueryData, dynamic imports). Step 39·40: 프론트엔드 mutation version 전달 + useCasGuardedMutation + 2-step Dialog AP-4 confirm 진입 전 version 재조회 (2026-05-03 verify-cas 흡수). Step 41: toCsvParam SSOT — lib/api+lib/utils API 파라미터 조립 레이어 `.join(',')` 인라인 0건 강제 (2026-05-08). Step 42: RejectModal `mode='domain'` 위임 패턴 — 인라인 반려 UI 금지 (2026-05-09). Step 49: Context 안전 등급 — 보안 Context graceful no-op 금지 (ADR-0013, 2026-05-13)
...
- **verify-cache-events** — 이벤트 기반 캐시 무효화 아키텍처 검증 (ADR-0012). emit/emitAsync 일관성, 리스너 async Promise, registry 커버리지, SSOT 캐시 키 패턴, BFF 집계 체인. Step 7 dual-channel duplication 차단 + 양방향 mirror 검사. Step 8 proactive audit + pre-push 통합. Step 9: CACHE_KEY_PREFIXES 인라인 concat 차단. Step 10: DOMAIN_EVENTS 채널 — `{ async: true }` + try/catch + void emitAsync + domain prefix 전체 flush (2026-05-13 G-5 orphan cleanup 패턴).
