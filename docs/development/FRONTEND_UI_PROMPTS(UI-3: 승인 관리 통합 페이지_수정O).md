...
- technical_manager (기술책임자): 팀 내 장비/교정/반출/보정계수/소프트웨어 승인
- lab_manager (시험소장): 시험소 전체 승인 + 교정계획서 최종 승인

※ system_admin 역할은 존재하지 않음. lab_manager가 최고 권한.

요구사항:
1. 탭 기반 카테고리 분리
   - 장비 승인 (등록/수정/삭제)
   - 교정 승인
   - 반출 승인 (교정/수리/대여 목적 통합)
   - 보정계수 승인
   - 소프트웨어 승인
   - 부적합 종료 승인
   - 교정계획서 승인 (lab_manager 전용)
   - 각 탭에 대기 개수 뱃지 표시

2. 역할별 자동 필터링
   - technical_manager: 자신의 팀/사이트 관련 항목
   - lab_manager: 해당 시험소 전체 + 교정계획서 탭 표시
...
파일 (route group 구조 준수):
- apps/frontend/app/(dashboard)/admin/approvals/page.tsx (통합 승인 페이지 - Server Component)
- apps/frontend/app/(dashboard)/admin/approvals/loading.tsx (로딩 상태)
- apps/frontend/app/(dashboard)/admin/approvals/error.tsx (에러 핸들러 - 'use client')
...
기존 API 파일 활용 (개별 API 유지, 통합 파일 불필요):
- apps/frontend/lib/api/equipment-api.ts (장비 승인)
- apps/frontend/lib/api/calibration-api.ts (교정 승인)
- apps/frontend/lib/api/checkout-api.ts (반출 승인)
...
- [ ] app/(dashboard)/admin/approvals/page.tsx 구현됨 (Server Component)
- [ ] app/(dashboard)/admin/approvals/loading.tsx 생성됨
- [ ] app/(dashboard)/admin/approvals/error.tsx 생성됨 ('use client')
- [ ] app/(dashboard)/admin/approvals/layout.tsx 생성됨
...
**API 함수 (기존 파일 활용):**
- [ ] equipment-api.ts 장비 승인 메서드 확인됨
- [ ] calibration-api.ts 교정 승인 메서드 확인됨
- [ ] checkout-api.ts 반출 승인 메서드 확인됨
- [ ] dashboard-api.ts 대기 개수 조회 메서드 확인됨
...
// ⚠️ E2E_TEST_AUTH_GUIDE.md 참조: NextAuth 콜백 방식 로그인 사용
import { test, expect } from '@playwright/test';

// 로그인 헬퍼 (E2E_TEST_AUTH_GUIDE.md 패턴)
async function loginAs(page: Page, role: 'technical_manager' | 'lab_manager') {
...
test.describe('Approval Management - Basic', () => {
  test('탭 전환 및 항목 표시 (기술책임자)', async ({ page }) => {
    ...
    // 교정 탭으로 전환
    await page.getByRole('tab', { name: /교정/ }).click();
    await expect(page.getByTestId('approval-list')).toContainText('교정');
  ...
  test('역할별 탭 표시 - 기술책임자는 교정계획서 탭 미표시', async ({ page }) => {
    await loginAs(page, 'technical_manager');
    await page.goto('/admin/approvals');

    // 기술책임자는 교정계획서 탭 미표시 (lab_manager 전용)
    await expect(page.getByRole('tab', { name: /교정계획서/ })).not.toBeVisible();
  });

  test('역할별 탭 표시 - 시험소장은 교정계획서 탭 표시', async ({ page }) => {
    await loginAs(page, 'lab_manager');
    await page.goto('/admin/approvals');

    // 시험소장은 교정계획서 탭 표시
    await expect(page.getByRole('tab', { name: /교정계획서/ })).toBeVisible();
  });
});
...
test.describe('Approval Management - Accessibility', () => {
  ...
  test('뱃지에 aria-label 제공', async ({ page }) => {
    ...
    const badge = page.getByRole('tab', { name: /장비/ }).locator('[aria-label*="대기"]');
