...
- 필터 패널: 데스크톱은 상단 접이식, 모바일은 시트(Sheet) 형태
- 테이블은 줄무늬(stripe) 스타일, hover 효과
- 카드는 그림자 효과, 상태별 색상 뱃지
- 빈 상태 UI (검색 결과 없음)
- 스켈레톤 로딩
- 애니메이션:
  - 뷰 전환 시 부드러운 fade 트랜지션
  - 테이블 정렬 아이콘 rotate 애니메이션
  - 카드 hover 시 subtle lift 효과 (shadow + translateY)

제약사항:
- nuqs 또는 next/navigation useSearchParams 사용
- 서버 컴포넌트에서 초기 데이터 로드
- 필터 변경 시 URL 업데이트 및 데이터 재요청
- Next.js 16 searchParams Promise 패턴 준수 (아래 예시 참조)

Next.js 16 searchParams 패턴 (필수):
```typescript
// apps/frontend/app/equipment/page.tsx
export default async function EquipmentPage(props: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) {
  // ✅ searchParams는 Promise이므로 await 필수
  const searchParams = await props.searchParams;
  const page = searchParams.page ?? '1';
  const status = searchParams.status;

  // 서버에서 초기 데이터 로드
  const initialData = await fetchEquipment({ page, status });
...
- 긴 목록 시 가상화 또는 content-visibility 적용 검토
- 검색 입력 debounce로 불필요한 API 호출 방지 (300ms)
- 테이블 행 컴포넌트 React.memo 적용
- 무거운 컴포넌트(차트 등) next/dynamic 동적 import
...
**시각적 접근성 체크리스트** (WCAG 2.1 AA):

- [ ] 포커스 표시가 명확히 보이는가? (outline 또는 ring 스타일)
...
- [x] searchParams Promise 패턴 적용됨 (클라이언트 컴포넌트에 위임)
- [x] 검색 디바운스 300ms 적용됨 (EquipmentSearchBar.tsx)
- [x] 테이블 행 React.memo 적용됨 (EquipmentRow, HighlightText 등)
- [x] 긴 목록 가상화/content-visibility 검토됨 (VirtualizedEquipmentList.tsx)

#### 테스트

- [x] Playwright 테스트 작성됨 (equipment-list.spec.ts)
- [x] 에러 시나리오 테스트 통과됨
- [x] 교정기한 필터/정렬 테스트 추가됨 및 통과됨
- [x] 모든 테스트 통과됨 (33 tests passed)
