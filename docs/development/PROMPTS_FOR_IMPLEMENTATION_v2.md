...
각 프롬프트를 복사하여 새 대화에서 사용하세요. AGENTS.md와 API_STANDARDS.md를 항상 참조하세요.
...
```
AGENTS.md와 API_STANDARDS.md를 참조하여 장비-팀 스키마 일치화를 통해 팀별 권한 체크 로직을 근본적으로 개선해줘.
...
```
AGENTS.md와 API_STANDARDS.md를 참조하여 같은 시험소 내 대여 승인 프로세스를 구현해줘.
...
- [ ] equipmentId, calibrationId 필드 추가됨
- [ ] factorType, factorName, factorValue, unit 필드 추가됨
- [ ] parameters JSON 필드 추가됨
- [ ] effectiveDate, expiryDate 필드 추가됨
- [ ] approvalStatus, approverComment 필드 추가됨
- [ ] requestedBy, approvedBy 필드 추가됨
- [ ] CalibrationFactorApprovalStatusEnum 추가됨
- [ ] CalibrationFactorTypeEnum 추가됨
- [ ] 마이그레이션 파일 생성됨
- [ ] pnpm db:generate 성공
- [ ] pnpm db:migrate 성공
- [ ] pnpm tsc --noEmit 성공
