...
1. **frontend**
   ...
   - `@equipment-management/ui`와 `@equipment-management/api-client` 패키지 활용
...
2. **잘못된 의존성 방향(금지)**
   ```
   schemas → ui/api-client/frontend/backend
   backend → frontend/ui
   ui → frontend
   ```
...
3. **의존성 선언 방법**
   ...
   ```json
   {
     "dependencies": {
       ...
       "@equipment-management/ui": "workspace:*",
