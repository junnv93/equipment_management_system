...
### 🟢 LOW — sticky-helpers.ts 언/마운트 race 동작 주석 보강 (Mode 0)
...
> 
> **후속 부채 (tech-debt-tracker 등재 권장)**: ① frontend `getPendingOutgoing`에 `direction=outbound` 명시 → outgoing 탭에서 borrower-rental 완전 차단. ② `buildScopeCondition` 헬퍼 purpose-aware variant 검토.
