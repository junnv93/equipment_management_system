...
## 🟡 calibration-cert-phase-a-architecture-closure (Mode 2, atomic 4-갭)

> **컨텍스트**: 2026-05-06 commit `80e77488` 까지 Phase A 사용자-facing 가치 마감 완료 (controller + ErrorCode 5-layer + e2e + register/dialog 양쪽 PDF Uploader + filter chip + dialog managementNumber prop). 시니어 자기 감사 라운드 결과 *시스템 전반 일관성*에서 4 갭 식별 — 단편적 해결의 잔재.
>
> **갭**:
> 1. 🔴 `/equipment/[id]/calibration-history` sub-route 부재 (다른 도메인 패턴 `repair-history`/`calibration-factors`/`non-conformance` 와 불일치) → row 클릭 진입점이 `/calibration?equipmentId=X` filter URL과 sub-route 양쪽 의미 있는데 후자가 누락
> 2. 🔴 `<FilterChip>` reusable 컴포넌트 부재 (CalibrationContent.tsx inline 하드코딩 tailwind. checkouts/equipment 등 다른 도메인이 동일 deep-link 패턴 쓸 때 SSOT 없음)
> 3. 🟡 chip JSX 하드코딩 tailwind (`bg-muted/50 border border-border text-xs text-primary hover:underline`) — Design token 미사용
> 4. 🟡 chip이 `calibrationHistoryData.data[0]` 의존 (list 비면 fallback `—`. equipment 별도 fetch가 deterministic)
> 5. 🟢 AuditLog `Could not extract entityId for extract calibration_certificate` warn (controller `entityIdPath` 미설정 — dry-run audit 정합성)
> 6. 🟢 chip + dialog mismatch 검증 RTL spec 없음 (CalibrationContent.test.tsx + CalibrationRegisterDialog.test.tsx 회귀 보호 부족)
>
> **프롬프트 (새 세션 paste 가능, atomic single sprint)**:
> ```
> /harness mode2
>
> Sprint: calibration-cert-phase-a-architecture-closure
> Slug: calibration-cert-phase-a-architecture-closure
>
> ## 컨텍스트
> Phase A core (commits 20b8ed00 ~ 80e77488) 마감 완료. 시니어 자기 감사 6 갭 closure.
>
> ## 갭 (atomic single sprint, 모두 1 commit)
>
> 1. 🔴 `/equipment/[id]/calibration-history` sub-route 신설
>    - apps/frontend/app/(dashboard)/equipment/[id]/calibration-history/page.tsx + Client component
>    - 다른 도메인(repair-history, calibration-factors, non-conformance) 패턴 mirror
>    - CalibrationListTable 또는 CalibrationHistoryTab 재사용 + CalibrationRegisterDialog 통합
>    - 사용자 흐름: /calibration row 클릭 → 사용자 선택 (기존 `?equipmentId=X` OR sub-route)
>      현재 NavLink는 `?equipmentId` 유지하되 sub-route 추가로 의미적 진입점 둘 다 제공
>
> 2. 🔴 components/shared/FilterChip.tsx 추출 (SSOT)
>    - props: { label: string, value: ReactNode, onClear: () => void, ariaLabel?: string }
>    - design token 사용 (FILTER_CHIP_TOKENS or BADGE primitive)
>    - CalibrationContent.tsx의 inline chip → <FilterChip> 마이그레이션
>    - 향후 checkouts/equipment 등 다른 도메인 deep-link filter도 동일 컴포넌트 사용
>
> 3. 🟡 design token 정의 (lib/design-tokens/components/filter-chip.ts)
>    - container / clearButton / label classes
>    - 기존 BADGE_TOKENS 또는 SEMANTIC_BADGE 시스템과 정합
>
> 4. 🟡 chip data sourcing — equipment 별도 fetch
>    - useQuery(queryKeys.equipment.detail(equipmentId)) for chip display
>    - calibrationHistoryData[0] 의존 제거 → list 비어도 chip 정확 표시
>    - QUERY_CONFIG.EQUIPMENT_DETAIL or 동등 cache 정책 활용
>
> 5. 🟢 AuditLog entityIdPath 또는 dry-run 처리
>    - calibration-certificate.controller.ts @AuditLog 데코레이터에 entityIdPath
>      또는 dry-run audit 별도 처리 (response의 certificateNumber 사용)
>    - warn 로그 closure
>
> 6. 🟢 RTL spec 추가
>    - CalibrationContent.test.tsx — equipmentId chip 렌더 + clear 동작 + 다른 filter 보존
>    - CalibrationRegisterDialog.test.tsx — managementNumber mismatch toast + state 미보존
>
> ## 사용자 명시 제약
> - 다른 세션 도메인(dashboard/approvals/checkouts/commit-pipeline) 침범 금지
> - 하드코딩 X — design token + i18n key + SSOT helper 모두 경유
> - workflow 합성 — 기존 CalibrationListTable / CalibrationRegisterDialog / CalibrationHistoryTab 재사용
> - 시니어 자기검토 7대 영역 (state 분산 / prop drilling / destructive / SSOT / performance / cross-domain / mock-only)
>
> ## MUST 검증
> - schemas + shared-constants build PASS
> - backend tsc + frontend tsc PASS (sprint scope)
> - calibration module jest 회귀 0
> - 신규 RTL spec 4+ tests PASS (CalibrationContent + CalibrationRegisterDialog)
> - e2e 회귀 0 (calibration-certificate.e2e-spec)
> - cross-domain diff 0줄
> - AuditLog warn 사라짐 (backend log 확인)
> - FilterChip reusable — props만 다르게 다른 도메인에서 그대로 사용 가능 구조
>
> ## SHOULD
> - sub-route URL 패턴이 다른 도메인과 일관 (`/equipment/[id]/calibration-history`)
> - design token 신규 추가 시 BADGE/SEMANTIC 시스템 정합
> - tech-debt-tracker에 closure 항목 mark + archive 이동
>
> ## 후처리
> - atomic commit: `feat(calibration): close phase A architecture — sub-route + FilterChip SSOT + audit + RTL`
> - push (pre-push 검증)
> ```
>
> **예상 분량**: ~300 lines, 7~10 file (3~4 신규, 나머지 edit). Mode 2 atomic single commit.
