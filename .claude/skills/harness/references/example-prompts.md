...
> **마지막 정리일: 2026-04-02 (5차 정리 — 아키텍처 리뷰 이슈 해소)**
...
### PendingChecks 필터 미동작 (Mode 0) — 도메인 결정 필요

```
PendingChecksClient의 필터 버튼이 UI에만 존재하고 실제 쿼리에 적용되지 않아.

위치: apps/frontend/app/(dashboard)/checkouts/pending-checks/PendingChecksClient.tsx:63
수정 방안: A) 쿼리 반영  B) 클라이언트 필터링  C) 필터 UI 제거
```
