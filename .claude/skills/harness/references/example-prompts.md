...
> **마지막 정리일: 2026-04-05 (16차 — QP-18 양식 갭 분석 기반 중간점검표/케이블관리 추가 + 3-agent 스캔)**
...
```
문제:
equipment_test_software 중간 테이블이 존재하고 읽기(equipment→SW)는 구현되었으나,
링크 생성/삭제 API와 UI, 역방향 조회(SW→equipment)가 전부 없음.
현재 M:N 관계는 시드 스크립트로만 생성 가능.

검증됨:
- test-software.controller.ts — POST/DELETE 링크 엔드포인트 없음
- test-software.service.ts — linkEquipment/unlinkEquipment 메서드 없음
- api-endpoints.ts — LINK_EQUIPMENT/UNLINK_EQUIPMENT 없음
- TestSoftwareDetailContent.tsx — 연결된 장비 목록 섹션 없음
- SoftwareTab.tsx — 연결 추가/해제 버튼 없음 (읽기 전용)
- 역방향 findEquipmentBySoftwareId 서비스/컨트롤러/API 없음
- invalidateCache가 by-equipment 캐시 키를 무효화하지 않음

액션:
1단계 — 백엔드 API:
  - POST /api/test-software/:id/equipment — 장비 연결 (equipmentId, notes)
  - DELETE /api/test-software/:id/equipment/:equipmentId — 연결 해제
  - GET /api/test-software/:id/equipment — 소프트웨어별 연결 장비 목록 (역방향)
  - 캐시 무효화: by-equipment:{equipmentId} 포함
2단계 — 프론트엔드 API 레이어:
  - API 메서드 + queryKeys 추가
3단계 — 프론트엔드 UI:
  - SoftwareTab.tsx: "소프트웨어 연결" 버튼 + 선택 다이얼로그 + "연결 해제" 버튼
  - TestSoftwareDetailContent.tsx: "연결된 장비" 섹션 추가 (역방향)
4단계 — 이력카드 연동:
  - BasicInfoTab 항목 7 "관련 S/W": M:N JOIN 데이터 표시

검증:
1. POST/DELETE 링크 API 동작
2. 장비 상세 > SW 탭에서 연결/해제 동작
3. 소프트웨어 상세에서 연결 장비 목록 표시
4. 캐시 무효화 정상 (연결 후 새로고침 없이 반영)
```
...
### 🟠 HIGH — 장비 상세 "소프트웨어/매뉴얼" 탭 통합 재설계 (Mode 1)

```
문제:
장비 상세 페이지의 "소프트웨어" 탭이 이력카드 양식과 괴리됨.
시험설비이력카드에서는 "소프트웨어/매뉴얼 정보"를 하나의 칸에서 관리하지만,
현재 시스템에서는 정보가 분산되어 중복 표시되거나 누락됨.

현재 상태 (검증됨):
- apps/frontend/components/equipment/SoftwareTab.tsx:24-38:
    firmwareVersion만 표시, 없으면 "등록된 소프트웨어 정보가 없습니다" 출력
    매뉴얼 정보 없음, 관련 시험용 SW 목록 없음
- apps/frontend/components/equipment/BasicInfoTab.tsx:280-319:
    매뉴얼 파일 섹션 (documents API에서 equipment_manual 타입 조회 + 다운로드)
- apps/frontend/components/equipment/BasicInfoTab.tsx:321-345:
    "소프트웨어/매뉴얼 정보" 카드에 firmwareVersion + manualLocation 중복 표시
- packages/db/src/schema/equipment-test-software.ts: ✅ M:N 중간 테이블 존재
    (equipmentId, testSoftwareId, notes, createdAt)
- apps/backend/src/modules/equipment/equipment.service.ts: ❌ M:N JOIN 조회 없음
    장비별 연결된 시험용 소프트웨어 조회 API 미구현
- packages/shared-constants/src/api-endpoints.ts:211-216: ❌ 장비별 SW 조회 엔드포인트 없음
    TEST_SOFTWARE에 LIST/GET/CREATE/UPDATE/TOGGLE만 존재
- apps/frontend/lib/api/query-config.ts:433-439: ❌ 장비별 SW queryKey 없음
    testSoftware.list/detail만 존재, byEquipment 없음

데이터 소스 정리:
  ┌────────────────────────┬─────────────────────────────┬─────────────────┐
  │ 정보                   │ 현재 위치                    │ 목표 위치        │
  ├────────────────────────┼─────────────────────────────┼─────────────────┤
  │ 펌웨어 버전            │ BasicInfoTab + SoftwareTab   │ SoftwareTab만   │
  │ 매뉴얼 보관 위치       │ BasicInfoTab만               │ SoftwareTab만   │
  │ 매뉴얼 파일 첨부       │ BasicInfoTab만               │ SoftwareTab만   │
  │ 관련 시험용 SW (M:N)   │ 표시 안됨                    │ SoftwareTab에   │
  └────────────────────────┴─────────────────────────────┴─────────────────┘

이력카드 양식과 매핑:
  시험설비이력카드 항목 7 "관련 소프트웨어/매뉴얼":
  - 펌웨어 버전 (equipment.firmwareVersion)
  - 매뉴얼 보관 위치 (equipment.manualLocation) — 예: "A동 201호 캐비닛"
  - 매뉴얼 파일 (documents where type=equipment_manual)
  - 관련 시험용 SW (equipment_test_software M:N → test_software)

액션:
1단계 — 백엔드 API 추가 (장비별 연결 시험용 SW 조회):
  - API_ENDPOINTS에 추가:
    TEST_SOFTWARE.BY_EQUIPMENT: (equipmentId: string) => `/api/test-software/by-equipment/${equipmentId}`
  - test-software.controller.ts: GET /api/test-software/by-equipment/:equipmentId 엔드포인트
  - test-software.service.ts: findByEquipmentId() — equipment_test_software JOIN test_software
  - @RequirePermissions(Permission.VIEW_TEST_SOFTWARE) 적용

2단계 — 프론트엔드 API 레이어:
  - software-api.ts: testSoftwareApi.listByEquipment(equipmentId) 메서드 추가
  - query-config.ts: queryKeys.testSoftware.byEquipment(equipmentId) 키 추가

3단계 — SoftwareTab 리뉴얼 (통합 탭):
  SoftwareTab.tsx 전면 재작성 — 3개 섹션:
  
  섹션 A: 펌웨어 정보
    - equipment.firmwareVersion 표시 (기존 로직 유지)
  
  섹션 B: 매뉴얼
    - equipment.manualLocation 텍스트 표시 (예: "A동 201호 캐비닛")
    - documents API에서 equipment_manual 타입 조회 → 파일 목록 + 다운로드 버튼
    - BasicInfoTab의 매뉴얼 파일 로직 이동 (documentApi.getEquipmentDocuments 사용)
  
  섹션 C: 관련 시험용 소프트웨어
    - testSoftwareApi.listByEquipment(equipment.id) 호출 (TanStack Query)
    - 테이블: 관리번호(PNNNN) | 이름 | 버전 | 시험분야 | 가용여부
    - 각 행 클릭 → FRONTEND_ROUTES.SOFTWARE.DETAIL(id)로 링크
    - 빈 상태: "연결된 시험용 소프트웨어가 없습니다"
    - 하단: "소프트웨어 레지스트리 바로가기" → FRONTEND_ROUTES.SOFTWARE.LIST
  
  세 섹션 모두 비어있으면 (firmwareVersion 없음 + manualLocation 없음 +
  매뉴얼 파일 없음 + 연결 SW 없음) → 통합 empty state 표시

4단계 — BasicInfoTab 중복 제거:
  - BasicInfoTab.tsx:280-319 매뉴얼 파일 섹션 제거
  - BasicInfoTab.tsx:321-345 "소프트웨어/매뉴얼 정보" 카드 제거
  - 관련 import 정리 (BookOpen, Package, DOCUMENT_DISPLAY 등 미사용 시 제거)

5단계 — i18n 키 추가/수정:
  - softwareTab.manualTitle: "매뉴얼" / "Manual"
  - softwareTab.manualFiles: "매뉴얼 파일" / "Manual Files"
  - softwareTab.relatedSoftware: "관련 시험용 소프트웨어" / "Related Test Software"
  - softwareTab.emptyRelatedSoftware: "연결된 시험용 소프트웨어가 없습니다" / ...
  - softwareTab.goToRegistry: "소프트웨어 레지스트리 바로가기" / "Go to Software Registry"
  - softwareTab.empty 수정: "등록된 소프트웨어/매뉴얼 정보가 없습니다" / ...
  - softwareTab 기존 키 유지: title, firmwareVersion, manualLocation

변경 파일 목록 (~12개):
  백엔드:
  - packages/shared-constants/src/api-endpoints.ts (엔드포인트 추가)
  - apps/backend/src/modules/test-software/test-software.controller.ts (GET 엔드포인트)
  - apps/backend/src/modules/test-software/test-software.service.ts (findByEquipmentId)
  프론트엔드:
  - apps/frontend/lib/api/software-api.ts (listByEquipment 메서드)
  - apps/frontend/lib/api/query-config.ts (byEquipment queryKey)
  - apps/frontend/components/equipment/SoftwareTab.tsx (전면 재작성)
  - apps/frontend/components/equipment/BasicInfoTab.tsx (중복 섹션 제거)
  i18n:
  - apps/frontend/messages/ko/equipment.json (softwareTab 키 추가)
  - apps/frontend/messages/en/equipment.json (softwareTab 키 추가)

검증:
1. pnpm --filter backend run tsc --noEmit → 타입 에러 0
2. pnpm --filter frontend run tsc --noEmit → 타입 에러 0
3. GET /api/test-software/by-equipment/{equipmentId} → 연결된 SW 목록 반환
4. 장비 상세 > 소프트웨어 탭:
   - firmwareVersion 있으면 "펌웨어 정보" 섹션에 표시
   - manualLocation "A동 201호 캐비닛" 표시
   - 매뉴얼 파일 다운로드 동작
   - 연결된 시험용 SW 테이블에 관리번호/이름/버전 표시
   - SW 행 클릭 → /software/{id} 이동
5. 장비 상세 > 기본 정보 탭: 매뉴얼/펌웨어 섹션이 제거됨 (중복 해소)
6. 세 섹션 모두 비어있는 장비 → 통합 empty state 표시
7. BasicInfoTab에서 제거한 import 중 다른 곳에서 사용하지 않는 것 정리 확인
```
