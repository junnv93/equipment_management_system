...
> **마지막 정리일: 2026-04-22 (81차 정리: PR-1·PR-2·PR-21 완료 → archive 이동. active: PR-3~PR-20(일부)·PR-22~PR-24 + tech-debt low 1건 + UltraReview 3종. NC-P1~P4 완료 → archive)**
...
2. 번들 크기 baseline 측정 스크립트:
   scripts/measure-bundle.mjs (완료):
   - ANALYZE=true ANALYZE_MODE=json pnpm --filter frontend build 실행
   - .next/analyze/client.json (webpack-bundle-analyzer chartData JSON) 직접 파싱
   - chartData[].gzipSize 필드로 checkouts 청크 gzip 크기 집계
   - baseline 파일: scripts/bundle-baseline-checkouts.json (gzip kB 전용)
     ※ scripts/bundle-baseline.json(First Load JS kB, check-bundle-size.mjs 전용)과 분리
   - baseline 대비 증가분이 8KB gzipped 초과 시 경고 (--compare 플래그)

3. package.json에 script 추가 (완료):
   "scripts": {
...
- node scripts/measure-bundle.mjs --no-build --help → exit 0 (빌드 없이 도움말 확인)
```
