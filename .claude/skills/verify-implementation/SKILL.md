...
| #  | 스킬                    | 영역     | 설명                                                    |
|----|-------------------------|----------|---------------------------------------------------------|
| 1  | `verify-auth`           | backend  | 인증/인가 — req.user.userId, @RequirePermissions, Step 3 bash 스캔 — PermissionsGuard DENY 기본값 controller 데코레이터 전수 검증(2026-05-13 saved-views silent miss) |
| 2  | `verify-zod`            | backend  | Zod 검증 — ZodValidationPipe, Query targets, `.trim().min(N)` 경계 케이스 대칭성(Step 17, 2026-05-02), revocationReason fail-close SSOT(Step 16 cmd 3b, 2026-05-03), mapper I18N_VARS 숫자 하드코딩 0건(Step 16 cmd 5c, 2026-05-03), `z.string().uuid()` 직접 사용 금지 → `uuidString()` SSOT(Step 18, 2026-05-03), CAS DTO/서비스 검증(Step 19, 2026-05-03 verify-cas 흡수), Backend Zod issues array i18n routing(Step 22, 2026-05-08 ADR-0008), 도메인 mapper hub fallback ts-morph 정적 spec(Step 23, 2026-05-08), Sort-mapper default 회귀 spec 자동 등록 — 13 mapper count == covered (통합 11 + 별도 2) + PgDialect.sqlToQuery + JSON.stringify(queryChunks) 안티패턴 차단(Step 24, 2026-05-09 three-low-tech-debt-closure r2), `z.discriminatedUnion` spec 정렬 — `baseValid` discriminant 불일치 false-positive 차단 + discriminatedUnion 이관 후 service BadRequest dead test 제거(Step 25, 2026-05-09), `@Body('field')` 문자열 리터럴 추출 ZodValidationPipe 우회 차단 — undefined.trim() TypeError 500 방지(Step 26, 2026-05-13) |
| 3  | `verify-ssot`           | both     | SSOT 임포트 소스 — 로컬 재정의 금지, config 파생 boolean → 수치 SSOT(Step 32), isPurposeCompatibleWithEquipment USER_SELECTABLE_PURPOSES guard(Step 47), switch+assertNever exhaustiveness(Step 48), UI 도메인 타입 SSOT+위임 re-export(Step 49), Visual layer ↔ domain SSOT 분리 named constant(Step 52, Sprint 4.5 U-09), ESLint ImportExpression dynamic import selector(Step 22f), useRowSelection SSOT(Step 55), calculateDaysRemaining SSOT(Step 56), `EXTENDED_TEXT_MAX_LENGTH` SSOT — `.max(200)` 매직넘버 금지(Step 58, 2026-05-03), backend pagination default/max clamp SSOT(Step 59, 2026-05-03), drizzle-stub SSOT — backend spec fluent chain 로컬 정의 차단 (createDrizzleSelectChain/InsertChain/UpdateChain/createSequentialDrizzleStub 4 export 경유, Step 65, 2026-05-13 g4-g12-round3), env-sync-guard SSOT — `check-env-sync.mjs` parseEnvFile/checkFrontendEnvSync 인라인 재구현 차단 (Step 66, 2026-05-13) |
| 4  | `verify-hardcoding`     | both     | 하드코딩 탐지 — API 경로, queryKeys, 환경변수, optimistic fallback `pageSize`(Step 17b), `useOptimisticMutation` `invalidateKeys` 인라인 배열 → 클래스 SSOT(Step 33), useUndoToast invalidateKeys `CheckoutCacheInvalidation.approvalWithDetailKeys(id)` 팩토리 강제 — `[detail(id), ...APPROVAL_KEYS]` 인라인 spread 금지(Step 38, 2026-05-13) |
| 5  | `verify-frontend-state` | frontend | 상태 관리 — TanStack Query, 동적 import, ActiveDialog discriminated union(Step 34), runWithConcurrency worker pool + backend bulk endpoint delegation 예외(Step 35), sessionStorage TTL+try/catch+one-shot(Step 37, Sprint 4.5 U-07), useUndoableState SSOT — 인라인 pastRef/futureRef 선언 금지(Step 38, 2026-05-02), 프론트엔드 mutation version 전달(Step 39, 2026-05-03 verify-cas 흡수), useCasGuardedMutation + 2-step Dialog AP-4 confirm 진입 전 version 재조회(Step 40, 2026-05-03 verify-cas 흡수), RejectModal `mode='domain'` 위임 패턴 — 인라인 반려 UI 금지(Step 42, 2026-05-09), useSafeTimeout SSOT — 신규 수동 타이머 배열 패턴 금지(Step 44, 2026-05-09), ApprovalList 파일 로컬 `ApprovalRowItem = memo(...)` 4-layer 체인 — ApprovalsClient 인라인 람다 0건 강제(Step 45, 2026-05-10), Context 안전 등급 — 보안 Context graceful no-op 금지(Step 49, ADR-0013, 2026-05-13), QUERY_CONFIG 프리셋 스프레드 강제 — `staleTime: CACHE_TIMES.X` 단독 설정 금지(Step 5 강화, 2026-05-13) |
| 6  | `verify-nextjs`         | frontend | Next.js 16 패턴 — await params, useActionState, PPR-compatible layout(sync outer+Suspense, Step 9), session.user.role typeof guard — `extractValidRole(session)` SSOT 헬퍼 강제 + 직접 캐스팅 금지(Step 10, 2026-05-13) |
| 7  | `verify-filters`        | frontend | URL-driven 필터 SSOT, 교정 result 타입 SSOT chain(Step 13, 2026-05-09)                                     |
| 8  | `verify-design-tokens`  | frontend | Design Token 3-Layer, Dialog ARIA 입력 검증 패턴(Step 49), 내비게이션 list 시맨틱 `<ul role="list">` + `<li>`(Step 50), mono 헬퍼 `.text-mono`/`.text-mono-base` utility 경유(Step 54, 2026-05-13), brand-color-* oklch SSOT — hsl wrapper 0 + color-mix(in oklch) alpha 합성(Step 55, 2026-05-13 g4-g12-round3), raw amber/yellow → `text-brand-warning` SSOT — `text-amber-N dark:text-amber-N` 패턴 차단(Step 56, 2026-05-13) |
| 9  | `verify-security`       | both     | 보안 — Helmet CSP, Security Headers, scope 위반 ErrorCode 403+감사등록 3중 확인(Step 15), Step 15 강화: python3 invariant — `*ScopeForbidden|*ScopeOnly|*TeamScopeOnly|*Forbidden` 자동 등록 교차 검증(2026-05-13), enforceXxxAccess 컨트롤러 spec 뮤테이션 커버리지(Step 16, 2026-05-03), system_error_events PII deny-list(Step 17, 2026-05-06), common/security/* 레이어 PII deny-list + fire-and-forget + Symbol DI 토큰 + @Optional inject 패턴 일반화(Step 18, 2026-05-09 three-low-tech-debt-closure), SentryErrorSink captureException 강제 — captureMessage 0건 + err.name=errorCode(Step 19, 2026-05-09), MetricsService 보안 Counter ↔ alert.rules.yml + runbook + baseline measurement 4종 세트 정합성(Step 21, 2026-05-09) |
| 10 | `verify-i18n`           | frontend | i18n — en/ko 키 쌍, 동적 키 커버리지, audit SSOT enum 동기화, CROSS_CUTTING_NAMESPACES+checkStructuralNamespaces 스크립트 구조(Step 17), SHARED_COMPONENT_DOMAIN_NS_RULE ESLint 게이트(Step 18), typed linting block stories ignores(Step 19), domain mapper `errors.title/errors.genericError` baseline 키 존재(Step 20), ADR-0008 tErrors 3-param 주입 탐지 — 2-param `map*ErrorToToast` 호출 0건 강제(Step 21, 2026-05-09), i18n-parity automation spec 존재 + SUPPORTED_LOCALES SSOT 경유 — N-locale 자동 enumeration(Step 22, 2026-05-12 shortcuts-context-multi-tab-i18n-parity), spec 파일 inline type-only import babel-jest 호환 — `import { value, type Identifier }` 차단(Step 23, 2026-05-12) |
...
| 14 | `verify-cache-events`   | backend  | 이벤트 기반 캐시 무효화 — 레지스트리 커버리지, 리스너 async, SSOT 패턴, BFF 집계 무효화 체인(Step 6). Step 9: CACHE_KEY_PREFIXES 인라인 concat 차단. Step 10: DOMAIN_EVENTS 채널 — `{ async: true }` 리스너 + try/catch + void emitAsync + 다중 사용자 영향 캐시 domain prefix 전체 flush (2026-05-13 G-5) |
| 15 | `verify-handover-qr`    | both     | QR + Handover 통합 — QR URL/설정/액션 SSOT(qr-url.ts/qr-config.ts/qr-access.ts) + 직접 스캔 인수인계(confirm_handover 라우팅, PURPOSE_BY_STATUS mirror 금지, attachment fail-close 3-layer). 2026-05-03 verify-qr-ssot + verify-handover-security 통합, 2026-05-09 토큰 메커니즘 제거 |
...
| 27 | _(보안 S1) admin page 가드 필수_        | frontend | **[정적 검사]** `app/(dashboard)/admin/*/page.tsx` 파일은 반드시 `hasPermission` 또는 `APPROVAL_ROLES` 또는 `redirect(` 를 포함해야 한다. layout.tsx defense-in-depth 2중 방어 구조(T-2, 2026-05-13): (1) `admin/layout.tsx` APPROVAL_ROLES 1차 가드 (2) `admin/*/page.tsx` specific permission 2차 가드. 신규 admin page 추가 시 page-level guard 누락이 silent miss 됨을 방지. 검증: `grep -rL "hasPermission\|APPROVAL_ROLES\|redirect" apps/frontend/app/\\(dashboard\\)/admin/*/page.tsx 2>/dev/null \| grep -v "^$"` — 출력이 있으면 FAIL (가드 없는 admin page). 0건이면 PASS. 예외: deprecated-redirect 페이지(`redirect(getApprovalPageUrl(...))`)는 redirect 포함이므로 PASS. |
