...
| 4  | `verify-hardcoding`     | both     | 하드코딩 탐지 — API 경로, queryKeys, 환경변수, optimistic fallback `pageSize`(Step 17b), `useOptimisticMutation` `invalidateKeys` 인라인 배열 → 클래스 SSOT(정적 키) 또는 함수형 SSOT(파라미터 의존 키) 패턴(Step 33, 2026-05-09 함수형 패턴 승격) |
...
| 14 | `verify-cache-events`   | backend  | 이벤트 기반 캐시 무효화 — 레지스트리 커버리지, 리스너 async, SSOT 패턴, BFF 집계 무효화 체인(Step 6 — inbound-overview:* 포함 불변성, 2026-05-09) |
