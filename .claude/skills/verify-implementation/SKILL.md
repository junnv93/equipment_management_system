...
| 19 | `verify-checkout-fsm`   | both     | Checkout FSM — Dependency Inversion, assertFsmInvariants, CheckoutPermissionKey 동기화, NO_EQUIPMENT 가드(Step 19 ≥4건), rejectReturn checkTeamPermission unconditional(Step 20), firstEquip 취득 패턴 items[0] 기준(Step 22), rejectReturn reason 검증 순서 scope/FSM 이후(Step 23) |
