...
| 23 | `verify-lint-ruleset-parity` (script)   | tooling  | **[script SSOT]** `scripts/verify-lint-ruleset-parity.mjs` — lintstaged ↔ lint:ci(backend) / lint(frontend) glob coverage + critical rule 등록 정합성. `PARITY_SPEC.{backend,frontend}` SSOT. 신규 critical rule 추가 시 SSOT 만 갱신. pre-push 게이트 통합 (`.husky/pre-push`). 검증: `pnpm verify:lint-ruleset-parity` 18 checks PASS — 2026-05-06 commit-pipeline-safety sprint. 회귀 spec: `scripts/__tests__/verify-lint-ruleset-parity.spec.mjs` (5 cases, EMS_PARITY_DOMAINS=backend 격리). |
