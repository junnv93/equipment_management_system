...
| 17 | `verify-routing-origin` | both     | Same-Origin Reverse-Proxy(ADR-0006) 정합 — api-routing.ts SSOT / next.config.js / nginx lan.conf+template / proxy.ts, env 절대 URL 잠입, BACKEND ∩ NEXTAUTH disjoint, SW NetworkOnly `/api/*` 룰 |
...
| 19 | `verify-bulk-action-bar` | frontend | BulkActionBar 패턴 SSOT — count chip aria-live, role=toolbar, Esc clear, indeterminate Radix, focus management, IME guard, generic/common + approvals wrapper 분리 |
