...
| 9  | `verify-security`       | both     | Helmet CSP + Security Headers + scope ErrorCode 403 강제 + SECURITY_AUDITABLE_CODES SSOT + enforceXxxAccess spec 뮤테이션 커버리지(Step 16, 2026-05-03) + system_error_events PII deny-list(Step 17, 2026-05-06 system-health-data-source-ssot) | `helmet-config.ts`, `next.config.js`, `**/*.controller.ts`, `**/__tests__/*.controller.spec.ts`, `lib/config/api-config.server.ts`, `common/constants/security-auditable-codes.ts`, `common/filters/error.filter.ts`, `common/system-health/contract.ts` |
