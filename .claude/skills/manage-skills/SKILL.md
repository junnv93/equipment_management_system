...
| `verify-zod`            | Zod 검증 패턴 검증 (Step 13: `.default(N)` 보장 필드는 DTO 클래스 non-optional, Step 15: frontend `< N` 하드코딩 ↔ backend Zod `.min(N)` SSOT 동기화 강제 — REJECTION_REASON_MIN_LENGTH 동기화 + `.trim()`/`.max()` defense-in-depth 강제, Step 16: ErrorCode SSOT 강제 + service fail-close 비대칭 차단 + frontend mapper coverage — `code: '[A-Z_]+'` 인라인 string literal 0건 강제(disposal/calibration-plan 격상 도메인), service layer rejectionReason fail-close 도메인 간 강도 일관성, `lib/errors/<domain>-errors.ts` mapper SSOT 존재 + onError 호출처 적용 강제(한국어 backend 메시지 노출 0)) | `*/dto/**/*.dto.ts`, `apps/backend/src/common/pipes/*.ts`, `apps/backend/src/modules/{equipment,calibration-plans}/**`, `apps/frontend/lib/errors/{disposal,calibration-plan}-errors.ts`, `apps/frontend/components/{equipment/disposal,calibration-plans}/**` |
