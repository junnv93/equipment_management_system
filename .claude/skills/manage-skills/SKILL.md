...
|----|-------------------------|----------|-----------|----------------|
| 1  | `verify-auth`           | backend  | 서버사이드 인증/인가 + 라우트 선언 순서 + AuditInterceptor `__auditLogged` dedup 불변성. Step 3 강화: `PermissionsGuard` DENY 기본값 — 모든 controller class 또는 method 에 `@RequirePermissions/@SkipPermissions/@Public` 1건 이상 필수 (silent miss 차단, 2026-05-13 saved-views 회귀) | `apps/backend/src/**/*.controller.ts`, `*/dto/**/*.dto.ts`, `common/interceptors/audit.interceptor.ts`, `common/filters/error.filter.ts` |
...
| 3  | `verify-ssot`           | both     | SSOT import + 임계값/상수/Record satisfies 검증 (Phase 2.2 split → 4 references: permissions-roles / domain-status-literals / threshold-numbers / record-satisfies). Step 60: SystemHealthProvider 컨트랙트 우회 검출. Step 61: EquipmentTabFooterLink SSOT. Step 62: EQUIPMENT_STATUS_TONE exhaustive Record + TONE_TO_SEMANTIC 어댑터. Step 64: packages/schemas → shared-constants production import 0건 + SCHEMA_VALIDATION_RULES 단방향 wire. Step 65: drizzle-stub SSOT — backend spec fluent chain 로컬 정의 차단 (createDrizzleSelectChain/InsertChain/UpdateChain/createSequentialDrizzleStub 4 export 경유, equipment.service.spec.ts 예외 제거 2026-05-13). Step 66: env-sync-guard SSOT — `check-env-sync.mjs` parseEnvFile/checkFrontendEnvSync 인라인 재구현 차단 + setup-env.mjs SSOT import 강제 + dev-doctor.mjs envSync 포함 (2026-05-13) | `apps/backend/src/**/*.ts`, `apps/frontend/**/*.ts(x)`, `scripts/check-*.mjs`, `scripts/setup-env.mjs`, `scripts/dev-doctor.mjs`, `package.json`, `packages/schemas/src/**/*.ts`, `packages/shared-constants/src/validation-rules.ts`, `components/equipment/EquipmentTabFooterLink.tsx`, `components/ui/StatusBadge.tsx`, `apps/backend/src/common/__tests__/drizzle-stub.ts`, `apps/backend/src/modules/**/__tests__/*.spec.ts` |
| 4  | `verify-hardcoding`     | both     | API 경로/queryKeys/매직넘버/PII/도메인 경로 하드코딩 탐지 + cache-invalidation SSOT. Step 38: useUndoToast invalidateKeys 팩토리 메서드 강제 — `[detail(id), ...APPROVAL_KEYS]` 인라인 spread 금지, `CheckoutCacheInvalidation.approvalWithDetailKeys(id)` SSOT 경유 (2026-05-13) | `apps/frontend/lib/api/**`, `apps/backend/src/**/*.service.ts`, `apps/frontend/components/**`, `lib/brand-assets/**`, `lib/analytics/**` |
| 5  | `verify-frontend-state` | frontend | TanStack Query + dynamic import + cache invalidation + CAS frontend (Step 39/40, 2026-05-03 verify-cas 흡수) (Phase 2.4 split → 3 references: tanstack-query-cas / dynamic-import-ssr / cache-invalidation). Step 41: toCsvParam SSOT — lib/api+lib/utils API 파라미터 조립 레이어 `.join(',')` 인라인 0건 + 단일 export 보장 (2026-05-08). Step 42: RejectModal `mode='domain'` 위임 패턴 — 인라인 반려 UI(`showRejectInput`) 금지 + try-catch handleRejectConfirm 패턴 (2026-05-09). Step 43: useEquipmentCalibrations dual-variant hook SSOT — `calibrationApi.getEquipmentCalibrations`/`getCalibrationHistory` 직접 호출 차단 + cache config compete 회귀 가드 (2026-05-09 phase-c-followup-closure r2). Step 46: useOptimisticMutation `undoWindowMs` 호출자는 fire-and-forget 강제 — `await mutation.mutateAsync` 차단 + dialog state owner는 mutate 직전 즉시 close (race-safe) (2026-05-12 checkouts-sprint4-followups-s1-s3-s8). Step 47: pre-upload orphan cleanup `submittedRef`+`deleteOrphan` 3-piece 패턴. Step 48: SSR-safe `useReducedMotion` — useEffect 외부 matchMedia 직접 접근 금지 (2026-05-12 qr-visual-redesign). Step 49: Context 안전 등급 — 보안 Context graceful no-op 금지(ADR-0013, 2026-05-13) | `apps/frontend/components/**`, `apps/frontend/hooks/**`, `apps/frontend/lib/checkouts/**`, `apps/frontend/lib/api/query-csv.ts`, `apps/frontend/lib/utils/*-filter-utils.ts`, `lib/utils/checkout-return-context.ts`, `hooks/use-undoable-state.ts`, `components/approvals/RejectModal.tsx`, `hooks/use-equipment-calibrations.ts`, `components/checkouts/EquipmentConditionForm.tsx`, `components/checkouts/ReturnInspectionForm.tsx`, `components/mobile/AutoProgressCountdown.tsx`, `apps/frontend/lib/inspection/form-context.tsx`, `apps/frontend/lib/api/authenticated-client-provider.tsx` |
| 6  | `verify-nextjs`         | frontend | Next.js 16 패턴 (await params, useActionState, React 19 deprecated 이벤트 타입). Step 9: PPR-compatible layout — `export default async function Layout` FAIL (sync outer+Suspense+async inner 강제, 2026-05-13). Step 10: session.user.role typeof guard — `session.user.role as ` 직접 캐스팅 금지 + `extractValidRole(session)` SSOT 헬퍼 경유 강제 (`lib/auth/server-session.ts`, 2026-05-13) | `apps/frontend/app/**/page.tsx`, `layout.tsx`, `error.tsx`, `apps/frontend/components/**/*.tsx` |
...
| 8  | `verify-design-tokens`  | frontend | Design Token 3-Layer + ARIA/WCAG + motion (Phase 2.1 split → 4 references). Step 53: design-token 파일 내 `bg-brand-* text-white` 직접 조합 → `getSemanticSolidBgClasses()` SSOT 경유 필수 (2026-05-09). Step 54: mono 헬퍼는 `.text-mono`/`.text-mono-base` utility 경유 — `font-mono tabular-nums` 인라인 합성 0건. Step 55: brand-color-* oklch SSOT — hsl(var(--brand-color-*)) wrapper 0 + alpha 합성은 color-mix(in oklch, ... NN%, transparent) (2026-05-13 qr-visual-redesign-followups-g4-g12-round3) | `lib/design-tokens/**`, `apps/frontend/components/**`, `styles/globals.css`, `connection-banner.tsx` |
| 9  | `verify-security`       | both     | Helmet CSP + Security Headers + scope ErrorCode 403 강제 + SECURITY_AUDITABLE_CODES SSOT + enforceXxxAccess spec 뮤테이션 커버리지(Step 16, 2026-05-03) + system_error_events PII deny-list(Step 17, 2026-05-06 system-health-data-source-ssot). Step 18: common/security/* 레이어 PII deny-list + fire-and-forget + Symbol DI 토큰 + @Optional inject 패턴 일반화 (2026-05-09 three-low-tech-debt-closure). Step 21: MetricsService 보안 Counter ↔ alert.rules.yml + runbook + baseline measurement 4종 세트 정합성 — 신규 보안 counter 추가 시 alert rule 누락 차단 (2026-05-09). Step 15 강화: `*ScopeForbidden|*ScopeOnly|*TeamScopeOnly|*Forbidden` 패턴 ErrorCode 자동 등록 invariant — python regex 후보 추출 + SECURITY_AUDITABLE_CODES Set membership 양방향 교차 (2026-05-13 saved-views silent miss 회귀 차단) | `helmet-config.ts`, `next.config.js`, `**/*.controller.ts`, `**/__tests__/*.controller.spec.ts`, `lib/config/api-config.server.ts`, `common/constants/security-auditable-codes.ts`, `common/filters/error.filter.ts`, `common/system-health/contract.ts`, `common/security/**/*.ts`, `apps/backend/src/common/metrics/metrics.service.ts`, `infra/monitoring/prometheus/alert.rules.yml`, `docs/operations/prometheus-alert-rules.md` |
...
| 14 | `verify-cache-events`   | backend  | 이벤트 기반 캐시 무효화 SSOT (registry coverage, 리스너 async, emit/emitAsync 일관성, BFF 집계 무효화 체인 — Step 6). Step 9: CACHE_KEY_PREFIXES 인라인 concat 차단. Step 10: DOMAIN_EVENTS 채널 — `{ async: true }` 리스너 + try/catch + 다중사용자 영향 캐시 → domain prefix 전체 flush (per-user selective 금지) + `void emitAsync` fire-and-forget emit (2026-05-13 G-5) | `cache-event.registry.ts`, `cache-event-listener.ts`, `cache-invalidation.helper.ts`, `cache-key-prefixes.ts`, `modules/*/listeners/*.listener.ts` |
| 15 | `verify-handover-qr`    | both     | QR + Handover 통합 (2026-05-03 verify-qr-ssot + verify-handover-security 통합). Section A: QR URL/설정/액션 SSOT, Section B: 직접 스캔 인수인계 (confirm_handover 라우팅, PURPOSE_BY_STATUS mirror 금지, attachment fail-close 3-layer). 2026-05-09 토큰 메커니즘 제거. Step 16: QR_ACTION_GROUP/QR_ACTION_GROUP_ORDER SSOT 인라인 그룹 분기 금지. Step 17: HandoverItem Zod schema 다중 핸드오버 SSOT (packages/schemas/qr-handover.ts). Step 18: LABEL_SIZE_PRESETS.recommendedForKey i18n parity 7 preset (2026-05-12 qr-visual-redesign). Step 19: handoverCheckoutId deprecation 회귀 영구 차단 — 9 호출자 마이그레이션 후 backward-compat shim 0 + handoverCheckoutId/deprecatedHandoverCheckoutIdLogged/issueHandoverToken/handover_tokens 통합 grep (2026-05-13 qr-visual-redesign-followups-batch-1) | `qr-url.ts`, `qr-config.ts`, `qr-access.ts`, `EquipmentActionSheet.tsx`, `qr-access.service.ts`, `checkouts.service.ts`, `packages/schemas/src/qr-handover.ts`, `HandoverPickerSheet.tsx` |
