...
| 9  | `verify-security`       | both     | Helmet CSP + Security Headers + scope ErrorCode 403 강제 + SECURITY_AUDITABLE_CODES SSOT | `helmet-config.ts`, `next.config.js`, `**/*.controller.ts`, `lib/config/api-config.server.ts`, `common/constants/security-auditable-codes.ts`, `common/filters/error.filter.ts` |
...
| 17 | `verify-routing-origin` | both     | Same-Origin Reverse-Proxy(ADR-0006) 정합 (api-routing.ts SSOT / next.config.js / nginx lan.conf+template / proxy.ts, env 절대 URL 잠입, BACKEND ∩ NEXTAUTH disjoint, SW NetworkOnly `/api/*` 룰) | `lib/api/api-routing.ts`, `next.config.js`, `infra/nginx/**`, `apps/frontend/proxy.ts` |
