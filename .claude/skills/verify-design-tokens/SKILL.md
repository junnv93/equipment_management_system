...
**검사 대상 (78-1 추가, 2026-04-21 / 78-7 stepDot / 78-8 xs-tight+sm-tight 추가):**
...
```bash
# globals.css에서 6개 변수 값 추출
grep -E "(--text-2xs|--text-xs-tight|--text-sm-tight|--spacing-hairline|--spacing-pagination|--spacing-step-dot)" \
  apps/frontend/styles/globals.css

# primitives.ts에서 대응 상수 값 확인
grep -E "('2xs'|'xs-tight'|'sm-tight'|hairline|pagination|stepDot)" \
