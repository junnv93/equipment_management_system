...
description: Checkout FSM SSOT 아키텍처 검증 — Dependency Inversion 패턴(UserRole 직접 import 금지), assertFsmInvariants 호출 존재, CheckoutPermissionKey 로컬 string union 정합성. packages/schemas/src/fsm/** 변경 후 사용.
...
### Step 5: FSM 소비자 브릿지 패턴 — getPermissions 경유

컨트롤러/서비스에서 `canPerformAction`/`getNextStep`을 호출할 때 `userPermissions`를 올바르게 전달하는지 확인.

```bash
# canPerformAction/getNextStep 호출 위치 확인
grep -rn "canPerformAction\|getNextStep" \
  apps/backend/src apps/frontend --include="*.ts" --include="*.tsx" \
  | grep -v "checkout-fsm.ts\|checkout-fsm.test.ts\|import\|//"
```

```bash
# UserRole을 직접 전달하는 잘못된 패턴 탐지
# (role을 string[]으로 변환 없이 직접 전달하는 패턴)
grep -rn "canPerformAction.*role\b\|getNextStep.*role\b" \
  apps/backend/src apps/frontend --include="*.ts" --include="*.tsx" \
  | grep -v "//\|test\|spec"
# 결과: 0건 (PASS — role을 getPermissions 변환 없이 직접 전달하면 FAIL)
```

**PASS:** 소비자가 `getPermissions(role)` 또는 `userPermissions: string[]` 형태로 전달.
**FAIL:** `role` 값 자체를 `userPermissions` 인자로 직접 전달.
