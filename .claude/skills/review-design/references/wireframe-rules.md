...
생성 전 반드시 Glob 도구로 기존 파일을 탐색합니다:
...
1. **Read 도구로 기존 와이어프레임을 읽는다** — 레이아웃 구조, 섹션 배치, 디자인 결정을 파악
2. 리뷰 보고서(AP-01~10)에서 발견한 **약점만 개선**한 새 버전을 생성
3. 기존 파일의 강점(검증된 레이아웃, 사용자 피드백 반영 사항)은 유지
4. **버전 접미사**로 새 파일 생성 — 기존 파일을 덮어쓰지 않음

```
approval-redesign.html       ← 기존 (보존)
approval-redesign-v2.html    ← 리뷰 기반 개선 버전
```

같은 페이지에 여러 버전(`-v2`, `-v3`)이 존재하면 **가장 최신 버전**을 기반으로 개선합니다.

### 기존 파일이 없을 때

`references/wireframe-rules.md`의 규칙에 따라 새로 생성합니다.
파일명: `docs/wireframes/<page-name>-redesign.html`
