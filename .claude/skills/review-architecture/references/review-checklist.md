...
- 카운트 쿼리↔목록 쿼리 데이터 소스 일치: 대시보드/배지의 집계 쿼리와 클릭 후 연결되는 목록 페이지의 필터가 **동일 테이블/컬럼**을 참조하는가? (다른 테이블 기반 집계 → 카운트와 목록 건수 불일치)
...
**트랜잭션 경계:**
- 다중 테이블 변경에 `db.transaction()` 적용? (미적용 시 중간 실패로 부분 커밋 → 데이터 정합성 깨짐)
- CAS 단일 테이블 업데이트에 불필요한 트랜잭션 사용하지 않는가? (WHERE version=? 자체가 원자적 — 트랜잭션은 커넥션 홀드 시간만 증가)
- CAS 선점(상태 업데이트)이 실제 작업(장비 생성/수정 등)보다 **먼저** 실행되는가? (작업 후 CAS 체크 시 동시 요청이 모두 작업을 수행한 뒤 하나만 CAS 성공 — 보상 로직 없으면 중복 생성. `initiateReturn()`의 "CAS 선점 → 작업 → 보상" 패턴 참조)
...
4. **Navigate-Before-Invalidate 안티패턴**: `onSuccess`에서 `router.push()` 후 `onSettled`에서 캐시 무효화하면 대상 페이지에서 stale 데이터 표시. 특히 STATIC 프리셋(`refetchOnMount: false`)에서 심각. 올바른 순서: invalidation → navigation in `onSuccess` (`useFormSubmission` 훅 패턴 참조)
5. **카운트 쿼리↔목록 쿼리 데이터 소스 일치**: 대시보드 KPI/AlertBanner의 카운트 쿼리가 목록 페이지의 필터 기준과 **동일 테이블/컬럼**에서 집계되는가? (다른 테이블 기반 카운트 → 클릭 후 목록과 불일치)
...
- Pending 승인 목록 등 **별도 조회 엔드포인트**에 `@SiteScoped` 누락 → `findAll()`에는 적용되어 있어도, `/pending` 등 별도 엔드포인트는 누락될 수 있음. 목록에서 보이지만 403으로 승인 불가 → 사용자 혼란
...
mutation은 단순한 API 호출이 아니라 **캐시 무효화 → UI 업데이트 → 네비게이션** 전체 사이클의 일관성을 보장해야 합니다.

| 확인 항목 | 왜 중요한가 |
|---|---|
| `useOptimisticMutation` 사용 여부 | 직접 `useMutation` 시 VERSION_CONFLICT 처리, 롤백, 캐시 무효화를 모두 수동 구현해야 함 |
| `onSuccess`에서 `setQueryData` 미사용 | TData와 TCachedData 타입 불일치 75% — 서버 데이터를 클라이언트가 추측하면 stale 표시 |
| 캐시 무효화 → 네비게이션 순서 | `onSuccess`에서 invalidation 완료 후 `router.push()` (`useFormSubmission` 훅 참조) |
| `invalidateKeys` 교차 엔티티 포함 | 장비 수정 → 대시보드 캐시도 무효화? `EquipmentCacheInvalidation` 정적 메서드 활용 |

### URL-driven 필터와 클라이언트 네비게이션 호환성

App Router의 클라이언트 네비게이션 중 `useSearchParams()`가 일시적으로 이전 페이지의 params를 반영할 수 있습니다. Radix Select 등 controlled 컴포넌트가 이 일시 상태에 반응하여 spurious `onValueChange`를 발생시킬 수 있으므로:

- `updateURL` 함수에서 "명시적 변경 vs 이전 값 보존" 분기가 있는가?
- 필터 컴포넌트의 `onValueChange`에 중복 값 가드가 있는가? (값이 이미 같으면 URL 업데이트 스킵)
