...
### Step 8: routeMap labelKey ↔ navigation.json 교차 검증 *(Phase 3 자동화)*

**`pnpm --filter frontend run verify:route-metadata`** (pre-push 자동 실행 — `_t "route-map"`).
8a: labelKey → ko+en navigation.json 정방향. 8b: page.tsx → routeMap 역방향. 8c: orphan 키 탐지.
**PASS:** 61 routes, labelKeys, pages 전부 일치. **FAIL:** 누락/미등록/orphan 발견.
