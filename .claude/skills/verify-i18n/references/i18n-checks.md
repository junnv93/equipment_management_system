...
> **Phase 3 자동화 완료** — `apps/frontend/scripts/verify-route-metadata.mjs` 로 승격.
> pre-push gate 통합: `.husky/pre-push` `_t "route-map"` step.
> 수동 실행이 필요한 경우 아래 명령을 사용하세요.
...
`navigation.json`에 routeMap 참조 없는 죽은 키(orphan)도 탐지합니다.

### 자동화 실행 (권장)

```bash
# Phase 3 스크립트 직접 실행 (pre-push에서 자동 호출)
pnpm --filter frontend run verify:route-metadata
...
| 단계 | 내용 | FAIL 조건 |
|------|------|----------|
| **8a** | labelKey → ko + en navigation.json 정방향 검증 | `navigation.${key}` 가 양쪽 locale JSON에 없으면 브레드크럼 raw 키 노출 |
| **8b** | page.tsx → routeMap 역방향 검증 | page.tsx 존재하지만 routeMap 미등록 (EXCLUDED_ROUTE_PREFIXES 제외) |
| **8c** | navigation.json orphan 키 탐지 | routeMap이 참조하지 않는 죽은 i18n 키 — 양쪽 locale 동시 탐지 |

**EXCLUDED_ROUTE_PREFIXES** (routeMap 등록 불필요한 경로):
`/e/` (QR shortcode redirect), `/__visual__/`, `/visual-fixtures/`, `/~offline`, `/help`,
`/admin/calibration-approvals` 외 legacy individual approval redirect 6개.
예외 추가 시 `apps/frontend/scripts/verify-route-metadata.mjs` `EXCLUDED_ROUTE_PREFIXES` 배열에 추가.

**회귀 spec:** `scripts/__tests__/verify-route-metadata.spec.mjs` (9 cases — Step 8a/8b/8c + Integration)
