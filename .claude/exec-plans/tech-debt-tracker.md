...
- [x] **[2026-05-13 cache-wholesale-migration-inspection-templates] 🟡 MED inspection-template wholesale → specific** — Mode 2 harness PASS (commit `28c41b1a`). 9 wholesale → 5 specific sub-prefix × 3 events: intermediate `list-equip:*`/`list:*`/`detail:*` + self `list-equip:*`/`detail:*` (자기-도메인 INSPECTION_FORM_TEMPLATES wholesale 은 service-local invalidateAndEmit 으로 책임 이전). ★ HIGH 갭 동반 closure: `CACHE_KEY_PREFIXES.INTERMEDIATE_INSPECTIONS` SSOT mismatch (`intermediate-inspections:` dead → `calibration:inspections:` 실제 service prefix 정합) + service 인라인 concat 제거. LEGACY 21 → 0 완전 closure.
