...
- [x] ~~**[2026-05-10 checkouts-sprint4-ux-u02-u08 S-7] 🟢 LOW saved-views-team-share**~~ — closure (sprint `saved-views-team-share`, 2026-05-13). 새 backend 모듈 `saved-views` + DB 0059 manual SQL migration + scope 트리아드(PRIVATE/TEAM/GLOBAL) + CAS + RBAC + audit `saved_view` + SSOT 6위치 + frontend TanStack Query 마이그레이션 + 명시 import banner + 라운드 #2 자기검토 G-1~G-4 closure (rename UI / scope helper / banner dismiss localStorage / hasLocalBackup useMemo). 28/28 jest PASS, tsc EXIT=0.
