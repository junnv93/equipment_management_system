...
- [x] **[2026-05-13 round3 PR-2] 🟡 MED frontend-env-internal-backend-url-ssot** — DONE: `.env.local` ADR-0006 SSOT 정합화 (`ENABLE_LOCAL_AUTH=true` 추가 + "임시" 주석 제거 + 섹션 구조 정리). CI 환경은 모든 workflow에서 `INTERNAL_BACKEND_URL` 이미 정상 설정 확인. commit `cf296c13`.
- [x] **[2026-05-13 round3 PR-3] 🟡 MED equipment-service-spec-monolithic-mockdb-migration** — DONE: `equipment.service.spec.ts` monolithic `mockReturnThis` → `createDrizzleInsertWithReturningChain` + `createDrizzleUpdateWithReturningChain` + `createDrizzleSelectChain` SSOT 마이그레이션. `findAll` → cache-level 모킹(DB 순서 독립). 12/12 PASS. commit `cf296c13`.
