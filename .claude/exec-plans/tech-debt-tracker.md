...
- [ ] **[2026-05-02 error-codes-ssot] 🔴 HIGH audit-log-fail-close-integration** — 자기검토 갭 7. GlobalExceptionFilter (`apps/backend/src/common/filters/error.filter.ts`)에 AuditService DI 추가, fail-close BadRequestException(403/400 보안 차단) 발생 시 audit_logs entry 자동 기록. 모니터링 가능한 보안 이벤트 추적 — false security 잔존 closure. unit/integration test 추가 필수. 트리거: 보안 운영 sprint 또는 다음 시스템 강화 sprint.
- [ ] **[2026-05-02 error-codes-ssot] 🟡 MEDIUM tier-2-error-codes-equipment-domain-migration** — equipment 도메인 인라인 → ErrorCode enum 격상. **완료분(2026-05-02)**: service 51건(199dc407) + controller 7건(equipment-controller-errorcode-ssot sprint). **잔여**: `management-number-param.pipe.ts` 2건 + `form-data-parser.interceptor.ts` 1건 = 3건. ErrorCode enum 추가 + frontend mapper + i18n 동기화 필요. 트리거: equipment 도메인 강화 sprint.
...
- [ ] **[2026-05-02 equipment-controller-errorcode-ssot] 🟡 MEDIUM calibration-factor-approve-ui-approver-comment** — `apps/frontend/components/equipment/CalibrationFactorsClient.tsx` 승인 mutation이 `approverComment` 없이 API 호출 → 런타임 backend 400 발생 (Zod `z.string().min(1)` 강제). 현재 frontend DTO optional로 tsc 오류만 해소(commit: equipment-controller-errorcode-ssot sprint). 근본 수정: 승인 버튼 클릭 시 approverComment 입력 Dialog 추가. 트리거: calibration-factors 도메인 UX 강화 sprint.
