...
- [x] **[2026-04-17 qr-phase3] 🟠 HIGH Playwright E2E 3종** — ✅ 완료 2026-04-18 (부분). phase1 5/5 PASS, phase2 2/3, phase3 2/4. **남은 3건은 backend/UI 후속 task로 이연**: (1) `POST /api/checkouts/handover/verify` 500 Internal Server Error — 1회용+replay blocker (backend 디버깅 필요). (2) phase2 벌크 PDF 다운로드 — EquipmentList UI가 default로 bulk selection을 채우지 않아 BulkLabelPrintButton이 0 items → 드롭다운 미노출. (3) Phase 3 만료 토큰 — handover page의 `resolveErrorCode`를 statusCode 기반(401=expired, 409=consumed)으로 수정했으나 verify 500 이슈로 간접 확인.
