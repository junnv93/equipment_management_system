...
- [ ] **[2026-05-13 cache-wholesale-migration-equipment-imports] 🟠 MED equipment-imports wholesale → specific** — `${CACHE_KEY_PREFIXES.EQUIPMENT_IMPORTS}*` 3 entries (NOTIFICATION_EVENTS.IMPORT_CREATED/APPROVED/REJECTED). InboundOverview BFF가 EQUIPMENT_IMPORTS와 INBOUND_OVERVIEW 양쪽 의존 — broad sweep 정당화 가능성 있음. 분석: equipment-imports.service.ts cache key 구조 + InboundOverview 동기화 필요성 검토. 트리거: equipment-imports cache 갱신 빈도 측정 후.
