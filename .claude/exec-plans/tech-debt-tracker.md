...
- [ ] **[2026-04-29 button-loading] 🟢 LOW visual-double-spinner-auth-settings** — `AzureAdButton.tsx`, `LoginForm.tsx`, 3개 settings 파일에 `loading={isPending}` + 내부 `<Loader2 aria-hidden="true">` 공존. aria-hidden이 있어 SR double-announce 없음. 시각적으로는 이중 스피너. STRIP_MANUAL_SPINNERS=1 패스로 제거 가능. 트리거: auth/settings UX 개선 sprint.

### 2026-04-30 verify-implementation (tech-debt-batch-0429) 후속 — 세션 외 파일 발견

- [ ] **[2026-04-30 verify-impl-post-batch] 🟡 MEDIUM reason-field-trim-missing-3-dto** — `apps/backend/src/modules/checkouts/dto/reject-return.dto.ts:16`, `reject-checkout.dto.ts:16`, `borrower-reject-checkout.dto.ts:16`에서 `z.string().min(1, ...)` 앞에 `.trim()` 누락. 공백만 입력해도 Zod 검증 통과 가능. 처리: `z.string().trim().min(1, ...)` 패턴으로 통일. 트리거: 다음 checkouts DTO 수정 PR.
- [ ] **[2026-04-30 verify-impl-post-batch] 🟢 LOW manual-entry-fallback-react-form-event** — `apps/frontend/components/scan/ManualEntryFallback.tsx:46` `React.FormEvent<HTMLFormElement>` — React 19에서 deprecated. `React.SyntheticEvent` 또는 `React.FormEvent` (네임스페이스 없이) 로 교체 권장. 트리거: scan 도메인 작업 시.
