...
| **2026-05-03 batch** | **2026-05-03** | **12 sprints** | **완료** — validation-rules-domain-constants, validation-message-pagination-residual, revoke-approval-dto-trim-symmetry, calibration-created-linked-plan-item-sse, inspection-a11y-manual-checklist, inspection-mobile-760-e2e, frontend-error-enum-ssot-migration, user-error-backend-code-routing, frontend-mapper-unit-test, software-validation-pagesize-ssot, inbound-overview-query-ssot, equipment-import-invalidation-keys-ssot. 추가: trim/max defense-in-depth 시스템 전반 확대(28+ DTO), data-migration validator SSOT, verify-skill Phase 1 재편 (4 통합→2). frontend 41 suites/434 tests PASS |
| **quality-audit-ssot** | **2026-05-03** | **11 files** | **완료** (Mode 2 harness, 1 iter PASS) — `docs/operations/quality-audit-routes.json` 감사 라우트 SSOT 신설, Lighthouse/public a11y가 동일 레지스트리 사용. public a11y workflow가 인증 spec을 실행하지 않도록 `playwright.a11y.config.ts` testMatch 고정. workflow config path 버그(`apps/frontend/apps/frontend/...`) 수정. `playwright.config.ts`에 `PLAYWRIGHT_MANAGED_SERVER=1` opt-in webServer 추가. axe serious 위반(`div[aria-label]`) 발견 후 LoginPageContent loading skeleton을 `role="status"` + `aria-live`로 의미화. public a11y 2/2 PASS, frontend lint/type-check PASS. |
| **validation-rules-domain-constants** | **2026-05-03** | **12 files** | **완료** (Mode 1 harness, 1 iter PASS) — `VALIDATION_RULES`에 cable short/location/serial + bulk max 상수 추가, `VM.array.minCases/maxCases` + `VM.equipmentImport.dateRangeInvalid` 신설. cable DTO 20/50/100, bulk approve/reject max 50, equipment-import dateRange 영어 메시지 SSOT화. `create-measurement` dataPoints 메시지 VM화. 신규 DTO spec 3개 + bulk-reject 보강. 관련 Jest 4 suites/23 tests PASS, schemas/shared-constants build PASS, backend lint/type-check PASS. |
| **data-migration-preview-windowing** | **2026-05-03** | **6 files** | **완료** (Mode 2 harness, 1 iter PASS) — Excel preview 대용량 DOM 렌더링 방지: `DATA_MIGRATION_PREVIEW_PAGE_SIZE` SSOT 신설 + 장비/이력 preview table을 `visibleRows` window 렌더링으로 전환. 전체 선택/실행 payload는 전체 selectable rows 기준 유지. `/equipment?pageSize=999999` 과대 요청 방지를 위해 equipment filter URL `pageSize`를 `PAGE_SIZE_OPTIONS` SSOT로 제한. focused Jest 35/35 PASS, frontend lint/type-check PASS. |
| **validation-message-pagination-residual** | **2026-05-03** | **15 files** | **완료** (Mode 1 harness, 3 iter PASS) — `VM.checkout.duplicateEquipment` + `VM.handover.token.required` 신설로 checkout/handover DTO inline 메시지 제거. pagination canonical source를 `packages/schemas/src/pagination.ts`로 승격하고 `shared-constants/src/pagination.ts`는 재수출 전용으로 전환. schemas 내부 `paginationParamsSchema`/`equipmentFilterSchema`와 backend checkout/equipment-import/repair-history/audit/self-inspection query clamp가 `MAX_PAGE_SIZE` 사용. `SCHEMA_VALIDATION_RULES.EQUIPMENT_NAME_MAX_LENGTH` 신설로 target `.max(100)` 잔여 제거. schemas pagination test 2 + backend DTO query/message tests 6 PASS, schemas/shared-constants build PASS, backend lint/type-check PASS. |
| **revoke-approval-dto-trim-symmetry** | **2026-05-03** | **1 file** | **완료** (Mode 1 harness, 1 iter PASS) — `revokeApprovalSchema` 전용 DTO spec 추가. `REVOCATION_REASON_MIN_LENGTH` 기준 trim 후 accept/reject 양방향, 공백-only reject, `LONG_TEXT_MAX_LENGTH` max boundary, version required/positive boundary, `approverId` strip 동작 검증. focused Jest 9/9 PASS, backend type-check/lint PASS. |
| **security-auditable-revocation-comment** | **2026-05-03** | **0 files** | **완료** (Mode 0 확인) — `security-auditable-codes.ts`에 `RevocationReasonRequired`가 HTTP 400임에도 frontend ≥10자 룰 backend fail-close bypass 차단 감사 대상으로 포함된다는 인라인 주석이 이미 존재함을 확인. stale tracker 항목 정리. |
| **calibration-created-linked-plan-item-sse** | **2026-05-03** | **6 files** | **완료** (Mode 1 harness, 1 iter PASS) — `CALIBRATION_CREATED` notification payload에 `linkedPlanItemId` 추가, approval SSE sentinel이 linked plan item을 `entityType='calibrationPlanItem'`/`entityId`로 전달, frontend `useNotificationStream`이 해당 sentinel 수신 시 `queryKeys.calibrationPlans.all` 무효화. NotificationSseService focused spec 13/13 PASS, backend/frontend type-check PASS, frontend lint PASS. |
| **inspection-a11y-manual-checklist** | **2026-05-03** | **1 file** | **완료** (Mode 0 docs) — `docs/development/inspection-a11y-manual-checklist.md` 신설. WF-19/WF-20 점검 흐름의 NVDA 한국어 수동 검증, ToggleGroup `Tab → Arrow → Space`, template gallery/soft-fork dialog, 760px 모바일 스택 체크, evidence template과 failure handling 기준 문서화. |
| **inspection-mobile-760-e2e** | **2026-05-03** | **1 file** | **완료** (Mode 1 harness) — `wf-20-self-inspection-ui.spec.ts` Step 7 추가. `page.setViewportSize({ width: 760, height: 1024 })`에서 approved 자체점검 card/action 노출, document horizontal overflow 없음, card width viewport 이하 검증. |
| **frontend-error-enum-ssot-migration** | **2026-05-03** | **4 files** | **완료** (Mode 1 harness, 2 iter PASS) — `CALIBRATION_ERROR_CODES`를 schemas ErrorCode 기반 SSOT로 신설하고 frontend `calibration-errors.ts` 로컬 enum 제거. 교정 frontend i18n routing은 schema-sourced code set 사용. `equipment-errors.ts` backend code routing의 schema-owned raw key를 `ErrorCode.*` computed property로 전환(`NC_NOT_FOUND`/`VERSION_REQUIRED` evaluator FAIL 후 `NonConformanceNotFound`/`EquipmentImportVersionRequired`로 수정). schemas build PASS, frontend focused tests 2 suites/35 tests PASS, frontend type-check/lint PASS. |
| **user-error-backend-code-routing** | **2026-05-03** | **4 files** | **완료** (Mode 1 harness, 1 iter PASS) — `user-errors.ts mapBackendErrorCode()`가 `USER_ERROR_I18N_KEYS`를 경유하도록 수정해 legacy helper도 domain i18n routing과 동일 SSOT 사용. unknown/undefined는 `errors.unknown` i18n fallback으로 fail-close, ko/en users namespace parity 추가. 신규 `user-errors.test.ts` 5 tests 추가. focused frontend Jest 1 suite/5 tests PASS, frontend type-check/lint PASS. |
| **frontend-mapper-unit-test** | **2026-05-03** | **2 files** | **완료** (Mode 1 harness, 1 iter PASS) — `disposal-errors.ts` + `calibration-plan-errors.ts` mapper 회귀 테스트 추가. ErrorCode 매칭, nested response extraction, parameterized rejection reason min var, fallback Error message 경로 검증. 테스트는 backend string literal 대신 `ErrorCode.*` SSOT 사용. focused frontend Jest 2 suites/6 tests PASS, frontend type-check PASS. |
| **software-validation-pagesize-ssot** | **2026-05-03** | **1 file** | **완료** (Mode 1 harness) — `SoftwareValidationContent.tsx` optimistic fallback `pageSize: 20` 2건을 `DEFAULT_PAGE_SIZE` SSOT로 교체. query key/API/status transition 로직 변경 없음. focused grep 0건, frontend type-check PASS. |
| **inbound-overview-query-ssot** | **2026-05-03** | **3 files** | **완료** (Mode 1 harness, 1 iter PASS) — `buildInboundOverviewQuery()` SSOT 헬퍼 신설. `InboundCheckoutsTab.tsx`의 status/search/limit 조립 중복을 제거하고 queryKey/queryFn이 동일 query 객체를 공유하도록 전환. 명시 status 우선, subTab fallback, 빈 search undefined 정규화, `DEFAULT_PAGE_SIZE` 기본값 회귀 테스트 5건 PASS. frontend type-check/lint PASS. |
| **equipment-import-invalidation-keys-ssot** | **2026-05-03** | **2 files** | **완료** (Mode 1 harness) — `EquipmentImportCacheInvalidation.statusMutationInvalidateKeys`를 신설해 detail 상태 mutation 4곳의 `invalidateKeys` 중복을 제거. canonical key는 `queryKeys.equipmentImports.lists()` 유지. focused grep 0건, frontend type-check PASS. |
| **calibration-action-button-aria-labels** | **2026-05-03** | **4 files** | **완료** (Mode 1 harness) — 교정/보정계수 승인·반려 버튼 4곳에 i18n 기반 명시 `aria-label` 추가. 보정계수는 `factorName`, 교정 기록은 `calibrationDate` fallback `id`로 행별 대상 식별. ko/en equipment namespace parity 유지, frontend type-check PASS. |
...
- [ ] **[2026-05-02] 🟢 LOW document-file-frontend-error-mapper** — `DocumentNotFound = 'DOCUMENT_NOT_FOUND'`, `FileNotFound = 'FILE_NOT_FOUND'` — 문서 업로드/다운로드 뮤테이션에서 발생. `apps/frontend/lib/errors/document-errors.ts` 신설. 기존 `download-error-utils.ts` 패턴과 통합 고려.
- [ ] **[2026-05-02] 🟢 LOW inspection-template-frontend-error-mapper** — `InspectionTemplateNotFound` — 중간점검 mapper에 등록됨이나 inspection-form-templates 도메인 전용 mapper 미생성. `apps/frontend/lib/errors/inspection-template-errors.ts` 신설 권장 (form-templates.json 네임스페이스). 현재 intermediate-inspection-errors.ts에 임시 등록.

### 2026-05-03 verify-skill-restructure-phase1 (Mode 2 harness) SHOULD 후속

Mode 2 harness PASS 후 발견된 갭 (별도 sprint로 정리):

- [ ] **[2026-05-03 verify-skill-restructure-phase1] 🟢 LOW verify-implementation-orphan-skills-registration** — `verify-implementation/SKILL.md` 테이블에 `verify-bulk-action-bar` / `verify-click-feedback` / `verify-routing-origin` 3개 누락. CLAUDE.md / skills-index.md / manage-skills/SKILL.md에는 등록되어 있으나 verify-implementation 통합 실행 대상에서 빠짐. Phase 1 scope 외 정리 (orphan 일괄 등록 sprint).
- [ ] **[2026-05-03 verify-skill-restructure-phase1] 🟢 LOW frontend-state-cas-step-details** — verify-frontend-state Step 39·40 (CAS frontend) 본문이 SKILL.md에 직접 작성됨. verify-zod (cas-checks.md 위임) / verify-e2e (workflows-coverage.md 위임) 패턴과 불일치. references/step-details.md로 분리 시 일관성 확보 (Phase 2 mega-skill 정리에서 함께 진행 권장).
