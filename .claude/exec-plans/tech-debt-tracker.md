...
- [x] **[2026-04-22 checkout-lender-guard] 🟢 LOW rejectReturn LENDER_TEAM_ONLY 회귀 테스트 누락** — 완료: checkout-lender-guard-p1p3. FSM이 RENTAL reject_return을 차단함을 확인하여 LENDER_TEAM_ONLY dead code 제거 + FSM INVALID_TRANSITION 동작 문서화 테스트 추가. LENDER_TEAM_ONLY 블록은 rejectReturn에서 완전 제거됨.

### 2026-04-22 verify+review-architecture: approveReturn 패리티 후속

- [x] **[2026-04-22 review-arch] 🟡 MEDIUM approveReturn NO_EQUIPMENT 가드 누락** — 완료: checkout-lender-guard-p1p3 P1. `getCheckoutItemsWithFirstEquipment` 후 `firstEquipment === null` 시 BadRequestException(NO_EQUIPMENT) 추가, 트랜잭션 진입 전 배치.
- [x] **[2026-04-22 review-arch] 🟢 LOW rejectReturn checkTeamPermission guard 구조 불일치** — 완료: checkout-lender-guard-p1p3 P2. `approverClassification` 선언 호이스트 + for-loop 무조건 실행으로 approve 패턴과 통일.
...
- [ ] **[2026-04-22 nc-p4-guidance] 🟢 LOW pre-existing ⑧ FSM 리터럴 7건 (NC/IntermediateCheck/document 영역)** — `CreateNonConformanceForm.tsx:145`, `NCDocumentsSection.tsx:78`, `IntermediateCheckAlert.tsx:153,219`, `document-upload-utils.ts:59`, `CreateEquipmentContent.tsx:115`, `ResultSectionFormDialog.tsx:154`. `status === '<literal>'` 비교를 `CSVal.*` 상수로 전환 필요. 트리거: 해당 파일 수정 시. (checkout-arch-pr3-11 항목과 별개로 NC 세션에서 재확인)

### 2026-04-22 harness: checkout-subtab-ia + checkout-subtab-ssot-fix 후속 (verify + review 결과)

- [x] **[2026-04-22 subtab-ia] 🟡 MEDIUM QUERY_CONFIG 인라인 오버라이드 (4차 재발)** — ✅ 2026-04-22 해소. query-config.ts에 CHECKOUT_LIST/CHECKOUT_SUMMARY/CHECKOUT_DESTINATIONS 추가, CheckoutsContent.tsx + OutboundCheckoutsTab.tsx 모두 교체.
- [x] **[2026-04-22 subtab-ia] 🟡 MEDIUM Radix Select spurious onValueChange 가드 누락** — ✅ 2026-04-22 해소. handleStatusChange/handleLocationChange/handlePurposeChange/handlePeriodChange 각각 첫 라인에 `if (value === filters.X) return;` 가드 추가.
- [x] **[2026-04-22 subtab-ia] 🟡 MEDIUM role="tabpanel" 내부 role="tablist" (WCAG 4.1.2)** — ✅ 2026-04-22 해소. CheckoutListTabs를 tabpanel 외부 sibling으로 이동. tabpanel에는 목록+페이지네이션만.
- [x] **[2026-04-22 subtab-ia] 🟢 LOW handlePageChange URL SSOT 이중 경로** — ✅ 2026-04-22 해소. handlePageChange → filtersToSearchParams({ ...filters, page: newPage }) 일원화. (handleSubTabChange는 delete 로직 특수성으로 별도 tech-debt 등록)
- [x] **[2026-04-22 subtab-ia-followup] 🟢 LOW handleSubTabChange URL 직접 조작** — ✅ 2026-04-22 해소. filtersToSearchParams({ ...filters, subTab: newSubTab, status: 'all', page: 1 })으로 일원화. useSearchParams 의존성 제거.
- [x] **[2026-04-22 subtab-ia-followup] 🟡 MEDIUM InboundCheckoutsTab CACHE_TIMES.SHORT 직접 지정 3곳** — ✅ 2026-04-22 해소. inbound checkout → CHECKOUT_LIST, rental/internal import → EQUIPMENT_IMPORT_LIST 프리셋 교체. query-config에 EQUIPMENT_IMPORT_LIST 추가.
