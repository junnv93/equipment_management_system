...
- [ ] **[2026-04-20 nc-workflow-atomicity] S1 — NC close() previousEquipmentStatus 복원 단위 테스트 없음** — `non-conformances.service.spec.ts` 에 previousEquipmentStatus가 있을 때 올바른 상태로 복원되는지, EXCLUDED fallback이 동작하는지 케이스 추가 필요.
