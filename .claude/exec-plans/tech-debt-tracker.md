...
- [ ] **[2026-05-03] 🟢 LOW security-auditable-codes.ts 주석 오기재** — `apps/backend/src/common/constants/security-auditable-codes.ts:7` 주석 "AuditInterceptor 양쪽에서 참조"가 사실과 다름. AuditInterceptor는 자체 `isForbiddenError()` 로직으로 처리하며 SECURITY_AUDITABLE_CODES를 import하지 않음. 주석을 실제 동작에 맞게 수정 필요. 트리거: 문서 정합성 정리 sprint.
