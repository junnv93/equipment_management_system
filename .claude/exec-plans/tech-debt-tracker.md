...
- [ ] **[2026-05-02 tier-2-rejectmodal 후속] 🟡 MEDIUM tier-2-calibration-factor-reject-ui-build** — calibration-factor 도메인은 backend 5-layer (Zod + service + ErrorCode + mapper + i18n) 모두 완성됐으나 frontend reject UI 미구현 상태. `apps/frontend/components/equipment/CalibrationFactorsClient.tsx`의 pending factor 테이블에 approve/reject 버튼 + RejectModal `mode='domain'` 호출 + `mapCalibrationFactorErrorToToast` 사용. 추정: ~120 lines 신규 코드. 트리거: calibration-factor 운영 워크플로우 활성화 시.
...
- [ ] **[2026-05-02 error-codes-ssot] 🟡 MEDIUM tier-2-error-codes-equipment-domain-migration** — equipment 도메인(`equipment.service.ts` 12건 + `equipment-approval.service.ts` 24건 + `equipment-history.service.ts` 7건 + `repair-history.service.ts` 3건 + `equipment-attachment.service.ts` 1건 + `equipment.controller.ts` 7건 + `management-number-param.pipe.ts` 2건 + `form-data-parser.interceptor.ts` 1건 = ~57건) 인라인 → ErrorCode enum 격상. ErrorCode enum에 EquipmentRequest/EquipmentManager/Equipment 도메인 ~30 codes 추가. frontend equipment-errors.ts 매퍼 동기화. 트리거: equipment 도메인 강화 sprint.
