...
- [ ] **[2026-Q2 pr-17] 🟡 MEDIUM NEXT_PUBLIC_CHECKOUT_NEXT_STEP_PANEL 상시화** — 조건: E2E 11 시나리오 2 세션 연속 안정 + 개발 환경 1주 이상 운영. 제거 대상: `showNextStepPanel` 분기 전체(`CheckoutDetailClient`, `CheckoutGroupCard`), `LegacyActionsBlock`/`LegacyInlineActions` 컴포넌트, `.env.local`/`.env.local.example` 해당 줄. 트리거: PR-23 실행 시 (checkout-arch-pr3-11 tech-debt 연계).
