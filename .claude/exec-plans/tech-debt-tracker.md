...
- [ ] **[2026-04-21 verify-ssot] 🟠 HIGH `'calibration_certificate'` 리터럴 직접 사용** — `calibration.service.ts` (L316, L463, L1124, L1329, L1707, L1851) + `calibration.controller.ts` (L659). `DocumentTypeValues.CALIBRATION_CERTIFICATE` 경유 필요. 트리거: calibration 모듈 작업 시.
- [ ] **[2026-04-21 verify-hardcoding] 🟡 MEDIUM `queryKey: ['auth', 'providers']` 하드코딩** — `AuthProviders.tsx:24`. `queryKeys` 팩토리에 `auth.providers` 키 추가 후 경유 필요. 트리거: auth 캐시 전략 변경 시.
...
- [ ] **[2026-04-21 verify-hardcoding] 🟢 LOW `EXPORTABLE_STATUSES` sync 주석 없음** — `software-validation-export-data.service.ts:56`. `['submitted', 'approved', 'quality_approved'] as const` 리터럴에 `// sync: ValidationStatusValues.*` 주석 추가 또는 `ValidationStatusValues` 직접 경유.
...
### 2026-04-21 — docx 아키텍처 layer violation 해소
