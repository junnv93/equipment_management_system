...
- [x] **[2026-04-18 completion] 🟡 MEDIUM C1 이미지 썸네일 server-side 리사이징** — ✅ 완료 2026-04-18. `GET /documents/:id/thumbnail?size=sm|md|lg` (sharp WebP 200/400/800px), Cache-Control immutable. `API_ENDPOINTS.DOCUMENTS.THUMBNAIL` SSOT 추가, `fetchDocumentThumbnailBlobUrl` 신설, NCDocumentsSection `fetchDocumentObjectUrl` → `fetchDocumentThumbnailBlobUrl` 교체.
...
- [ ] **[2026-04-18 nc-permission-async-cache] 🟡 MEDIUM CACHE_EVENTS vs NOTIFICATION_EVENTS 분리** — `NC_ATTACHMENT_UPLOADED/DELETED` 이벤트가 현재 `NOTIFICATION_EVENTS`에 혼재. 이 이벤트들은 사용자 알림 없는 캐시 무효화 전용이므로 별도 `CACHE_EVENTS` 상수로 분리해야 의미 명확. **수정 방향**: (1) `common/cache/cache-events.ts` 신설 — `CACHE_EVENTS` 상수 + `CacheOnlyPayload` 타입, SSOT 교차 검증 미적용. (2) `CACHE_INVALIDATION_REGISTRY` 키 타입을 `string`으로 유지하므로 교체 가능. (3) `NOTIFICATION_TYPE_VALUES` + `NOTIFICATION_EVENTS`에서 attachment 2건 제거. **연관**: `invalidateAfterNonConformanceStatusChange` 재사용도 재검토 — NC 첨부 전용 `invalidateNcDerivedCaches` 메서드 신설 권장.
