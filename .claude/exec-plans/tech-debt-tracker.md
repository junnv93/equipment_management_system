...
- [ ] **[2026-05-07 phase-a-arch 🔴 HIGH] filter-chip-shared-component-b-apply** — `apps/frontend/app/(dashboard)/calibration/CalibrationContent.tsx`에 inline tailwind chip → `<FilterChip>` 마이그레이션. 트리거: query-r3-closure commit 후. 동시에 Gap 4 통합 처리.
...
- [ ] **[2026-05-07 phase-a-arch 🟡 MEDIUM] equipment-detail-separate-fetch-for-chip** — `useEquipment(filters.equipmentId)` 별도 fetch — list `[0]` 의존 제거. 트리거: 위 `filter-chip-shared-component-b-apply`와 함께 단일 commit.
...
- [ ] **[2026-05-07 phase-a-arch 🟢 LOW] calibration-content-rtl-specs** — `apps/frontend/app/(dashboard)/calibration/__tests__/CalibrationContent.test.tsx` (chip render + clear filter 보존). 본 sprint working tree에 spec 작성 완료(untracked). 트리거: `filter-chip-shared-component-b-apply` 적용 후 commit (component 변경 + spec 동시 commit).
