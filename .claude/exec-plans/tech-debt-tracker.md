...
- [ ] **[2026-04-19 sw-validation] 🔴 CRITICAL quality_approve 성공 후 FE testSoftware.detail 캐시 미무효화** — `SoftwareValidationContent.tsx:135-142`. `invalidateValidations()` 가 `testSoftware.detail(softwareId)` 를 누락. quality_approve 완료 후에도 재검증 배너가 사라지지 않는 UX 버그. `queryClient.invalidateQueries({ queryKey: queryKeys.testSoftware.detail(softwareId) })` 추가 필요.
- [ ] **[2026-04-19 sw-validation] 🟠 HIGH SoftwareValidationContent 6개 뮤테이션 useMutation 직접 사용** — `create/update/submit/approve/qualityApprove/reject` 6개가 `useOptimisticMutation` 대신 `useMutation` + 인라인 `onSuccess/onError` 패턴. VERSION_CONFLICT 에러 시 캐시 무효화가 일관성 없이 처리됨. `useOptimisticMutation` 패턴으로 전환 + `onSettled: invalidateValidations` 통일 필요.
- [ ] **[2026-04-19 sw-validation] 🟠 HIGH EN i18n 13개 키 누락** — `messages/en/software.json` (selfApprovalForbidden, dualApprovalForbidden, revalidationRequired, goToValidation 4개) + `messages/en/errors.json` (softwareValidation 섹션 9개). 빌드는 통과하나 영문 UI에서 키 그대로 노출.
- [ ] **[2026-04-19 sw-validation] 🟡 MEDIUM SoftwareValidationsModule에서 ExportData/Renderer Service export 누락** — `SoftwareValidationExportDataService`, `SoftwareValidationRendererService`가 `ReportsModule.providers`에 직접 등록됨. 올바른 위치는 `SoftwareValidationsModule.providers + exports` → `ReportsModule.imports`. 현재 기능 동작은 하나 모듈 경계 위반.
- [ ] **[2026-04-19 sw-validation] 🟡 MEDIUM calibration-plans review() assertIndependentApprover 미적용** — `calibration-plans.service.ts:560-606`. 3단계 승인 중 2단계(review)에 독립성 검사 없음. ISO 17025 §6.2.2 위반 소지. `assertIndependentApprover(plan.createdBy, reviewedBy)` 추가 필요.
- [ ] **[2026-04-19 sw-validation] 🟡 MEDIUM SoftwareValidationListener 과잉 캐시 무효화** — `deleteByPrefix(CACHE_KEY_PREFIXES.TEST_SOFTWARE)` 가 모든 test_software 캐시를 삭제. `cacheService.delete('test-software:detail:<event.testSoftwareId>')` + `deleteByPrefix('test-software:list:')` 타겟 무효화로 교체 권장.
...
- [ ] **[2026-04-19 sw-validation] 🔴 CRITICAL Phase 4 — 다중 항목 DOCX 렌더링 누락 (데이터 손실)** — `software-validation-renderer.service.ts` 가 `acquisitionFunctions[0]` / `processingFunctions[0]` / `controlFunctions[0]` 첫 번째만 렌더링. 절차서 템플릿이 단일 행 고정인지 실측 확인 필요. 2개 이상이면 나머지 데이터가 DOCX에서 유실됨. 절차서 원문 템플릿 XML 구조 확인 후 다중 행 렌더 또는 "외 N-1건" 주석 추가.
- [ ] **[2026-04-19 sw-validation] 🟠 HIGH Phase 3 — 글로벌 /software-validations 라우트 + Sidebar 메뉴 없음** — 현재 유효성 확인 목록 접근 경로: `소프트웨어 상세 > 유효성 확인 탭` 경유만 가능. 관리자/품질책임자가 전체 pending/submitted 건을 한눈에 볼 수 없음. `app/(dashboard)/software-validations/page.tsx` + `SoftwareValidationsListContent.tsx` + Sidebar 항목 추가 필요 (Permission 가드 포함).
...
- [ ] **[2026-04-19 sw-validation] 🟡 MEDIUM Phase 3 — FRONTEND_ROUTES.SOFTWARE_VALIDATIONS 상수 없음** — 소프트웨어 유효성 관련 경로가 `FRONTEND_ROUTES`에 미등록. 라우트 변경 시 하드코딩 문자열이 코드베이스 전반에 흩어질 수 있음. `LIST`, `DETAIL(id)`, `NEW`, `EDIT(id)` 항목 추가 필요.
- [ ] **[2026-04-19 sw-validation] 🟡 MEDIUM Phase 3 — error.tsx (Error Boundary) 없음** — `app/(dashboard)/software/[id]/validation/error.tsx` 미생성. 페이지 에러 시 전역 폴백으로 빠져 Sentry 캡처도 안 됨. `'use client'` + `useEffect(() => captureException(error))` 패턴 추가 필요.
...
- [ ] **[2026-04-19 sw-validation] 🟢 LOW Phase 4 — RFC 5987 content-disposition 헬퍼 미추출** — `reports.controller.ts` 의 파일명 인코딩 로직이 인라인으로 잔존. `apps/backend/src/common/http/content-disposition.util.ts` `makeAttachmentHeader(filename)` SSOT 헬퍼 신설 + 타 export 엔드포인트(이력카드, 케이블 등)에도 적용하면 RFC 5987 준수 일관성 확보.
