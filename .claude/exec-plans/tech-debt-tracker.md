...
- [ ] **[2026-04-22 PR-2] 🟡 MEDIUM approve (최초 승인) 엔드포인트 guard 과소제어** — `checkouts.controller.ts:395` `approve` 엔드포인트가 `VIEW_CHECKOUTS`만 요구 (APPROVE_CHECKOUT 아님). approve-return/reject-return이 올바른 guard로 강화된 상황에서 비대칭. `test_engineer`가 approve 시도 시 guard 통과 후 assertFsmAction에서 403 차단 — 다중 방어선 기준 허용이나 guard 일관성 개선 권장. 트리거: checkout 보안 감사 시.
...
- [x] **[2026-04-22 PR-2] 🟡 MEDIUM writeTransitionAudit 실패 명시적 로깅** — 완료 (db7f5be3)
