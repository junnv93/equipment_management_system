...
- [x] **[2026-05-13 cache-wholesale-migration-calibration-factors] 🟡 MED calibration-factors wholesale → specific** ✅ closure — 2 entries (CALIBRATION_FACTOR_APPROVED/REJECTED) wholesale `${CALIBRATION_FACTORS}*` → `list:*` + `registry:*` + `equipment:*` + `detail:*` (calibration-factors service의 4 sub-prefix와 정합). allowlist 제거. audit POTENTIAL 11→9.
- [x] **[2026-05-13 cache-wholesale-migration-other-legacy] 🟢 LOW other LEGACY wholesale (Disposal/IntermediateCheck)** ✅ closure — 2 entries 분해: DISPOSAL_REJECTED `${DISPOSAL_REQUESTS}*` → `list:*` + `detail:*` (no-op safety net); INTERMEDIATE_CHECK_COMPLETED `${CALIBRATION}*` → `list:*` + `pending:*` + `detail:*` (calibration cache 정합). allowlist 제거. audit POTENTIAL 9→6.
