...
- [ ] **[2026-04-22 subtab-ia] 🟡 MEDIUM QUERY_CONFIG 인라인 오버라이드 (4차 재발)** — `CheckoutsContent.tsx:141,149,165`: `staleTime: CACHE_TIMES.SHORT/LONG` 직접 지정. `QUERY_CONFIG`에 `CHECKOUT_LIST`, `CHECKOUT_SUMMARY`, `CHECKOUT_DESTINATIONS` 프리셋 추가 후 교체 필요. 트리거: checkout 쿼리 설정 수정 시.
- [ ] **[2026-04-22 subtab-ia] 🟡 MEDIUM Radix Select spurious onValueChange 가드 누락** — `CheckoutsContent.tsx` `handleStatusChange/handleLocationChange/handlePurposeChange/handlePeriodChange`: 현재 값과 동일한 경우 URL 업데이트 스킵 가드 없음. 네비게이션 중 spurious 발화 시 의도치 않은 필터 리셋 위험. `if (value === filters.X) return;` 가드 추가 필요. 트리거: CheckoutsContent 필터 핸들러 수정 시.
- [ ] **[2026-04-22 subtab-ia] 🟡 MEDIUM role="tabpanel" 내부 role="tablist" (WCAG 4.1.2)** — `OutboundCheckoutsTab.tsx:348-357`: `CheckoutListTabs`(tablist)가 `role="tabpanel"` div 내부에 위치. WCAG 2.1 Tab Pattern에서 tablist는 tabpanel의 sibling이어야 함. `CheckoutListTabs`를 tabpanel 외부로 이동하고 tabpanel에는 목록 콘텐츠만 남겨야 함. 트리거: OutboundCheckoutsTab 접근성 작업 시.
- [ ] **[2026-04-22 subtab-ia] 🟢 LOW handlePageChange URL SSOT 이중 경로** — `OutboundCheckoutsTab.tsx:132-148`: `handlePageChange`/`handleSubTabChange`가 `new URLSearchParams(searchParams.toString())`로 직접 조작. `CheckoutsContent.tsx`의 `updateUrl(filtersToSearchParams(...))` 패턴과 이중 경로. `filtersToSearchParams`로 일원화 권장. 트리거: 페이지네이션 관련 수정 시.
