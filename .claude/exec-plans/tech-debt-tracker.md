...
- [x] ~~**[2026-05-12 qr-visual-redesign G-2] verify-implementation-run-full-13-skills**~~ — closure `429f054c` (2026-05-12). verify-implementation 12/13 PASS (verify-bulk-action-bar N/A) + review-architecture HIGH 1건(equipmentApi 반환 타입 drift) 즉시 fix. `equipmentApi.getEquipmentByManagementNumber` 반환 타입에 `handovers?: HandoverItem[]` + `handoverCheckoutId?: string` 추가 → `EquipmentQRLanding` 와 type 정합. tsc EXIT=0.
- [x] ~~**[2026-05-12 qr-visual-redesign G-3] commit-da1dbc0e-isolate-software-files**~~ — closure (2026-05-12). **Cherry-pick verification 완료**: `git worktree add -b verify/qr-isolation /tmp/qr-isolation-verify c01452f3` → 4 clean cherry-pick + da1dbc0e split (`-n` + `git restore` software/* 6 files) + 후속 cherry-pick (1fcc34bf docs conflict skip). 결과: 8 isolated commits, 44 files changed, **software/* 0건**, backend tsc EXIT=0 + frontend tsc EXIT=0. 머지 시 software-design-review-p0-p1-p2 PR과 software/* 겹침은 동일 commit 내용이므로 충돌 0. worktree + branch cleanup 완료.
...
- [x] ~~**[2026-05-12 followups G-15] 🟡 MED shortcuts-context-api-consistency**~~ — closure `shortcuts-context-multi-tab-i18n-parity`. `ShortcutsSettingsContent.tsx` 가 `useKeyboardShortcutsContext` 훅 경유로 setOverride/clearOverride/resetAllOverrides 소비 (M-4/M-5).
- [x] ~~**[2026-05-12 followups G-16] 🟡 MED shortcut-overrides-multi-tab-sync**~~ — closure `shortcuts-context-multi-tab-i18n-parity`. `KeyboardShortcutsProvider` 가 `window.addEventListener('storage')` 등록 + cleanup + SSOT key guard + storageArea guard. 신규 spec 6 cases (M-6/M-7/M-8/M-10).
- [x] ~~**[2026-05-12 followups G-17] 🟢 LOW i18n-parity-automation-spec**~~ — closure `shortcuts-context-multi-tab-i18n-parity`. `apps/frontend/lib/__tests__/i18n-parity.test.ts` 신규 — 25 도메인 자동 enumeration + nested key recursion + describe.each 도메인별 케이스. 51 cases PASS (M-9).
...
- [x] ~~**[2026-05-10 large-component-refactor S-2] 🟢 LOW dynamic-import-location-ssot**~~ — **closure**: `section-autonomy-followup` sprint commit `4cc25ca5` (2026-05-12) — `StatusLocationStep.tsx` + `CalibrationStep.tsx` 신규 wrapper 도입 (HistoryAttachmentStep 패턴), EquipmentForm `next/dynamic` 0건 달성.
- [x] ~~**[2026-05-10 large-component-refactor S-5] 🟢 LOW inspection-basic-info-context-coupling**~~ — **closure**: commit `4cc25ca5` — `inspectionCycleIsPrefilled` + `calibrationValidityPeriodIsPrefilled` boolean props 전달, section provider-free testable. VisualTableEditor는 graceful no-op SSOT 의도된 예외 — F-3 후속 ADR.
- [x] ~~**[2026-05-10 large-component-refactor S-1] 🟢 LOW section-file-line-count-reduction**~~ — **closure**: commit `4cc25ca5` — InspectionItemsSection 225→127 (InspectionItemCard 추출), NCInfoCards 260→50 (NCBasicInfoCard + NCRepairCard + NCCalibrationCard 분리). **정정**: tracker entry "MeasurementEquipmentSection(103)" stale fact — 실측 107 lines, 200 미만이므로 no-action.
