...
- [ ] **[2026-04-28 supply-chain-gate-completion] 🟢 LOW main-residual-lint-errors** — `apps/backend/src/modules/dashboard/__tests__/dashboard.service.spec.ts:330` (return type 누락) + `apps/backend/src/modules/software-validations/software-validations.service.ts:388` (unused arg) — origin/main에 이미 존재. pre-push hook이 `tsc + test`만 게이트하고 lint는 미포함이라 침투. 별도 PR로 정리 + pre-push hook lint 추가 검토 필요.
...
- [ ] **[2026-04-28 software-validations] 🔴 IMMEDIATE software-validation-approve-comment-silent-loss** — `apps/backend/src/modules/software-validations/software-validations.service.ts:388` `approve()` 메서드가 `approvalComment?: string` 인자를 받지만 본문에서 사용 안 함. DTO(`approve-validation.dto.ts:10`) + Controller(`software-validations.controller.ts:175`)는 정상 전달하지만 Service silent loss. 사용자 입력 승인 코멘트가 어디에도 저장되지 않음. 처리 옵션: (a) `software_validations` 테이블에 `approval_comment` 컬럼 신설 + Drizzle 마이그레이션 (b) audit_logs metadata에 코멘트 저장 (c) 의도된 placeholder라면 DTO에서 제거. 본 세션은 lint 통과 위해 `_approvalComment` underscore prefix만 적용 — 의도 보존이지만 silent loss 미해결. 트리거: software-validations 도메인 sprint 또는 ISO/IEC 17025 §6.2.2 audit 보강 시.
