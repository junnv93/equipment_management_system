...
- [~] Export UI 다운로드 동선 미검증 양식 — 부분 해결: 2026-04-08 — `test/export-ui-download-spec`. SSOT helper `expectFileDownload(page, trigger, {filenamePattern})` 신규 추가(`apps/frontend/tests/e2e/shared/helpers/download-helpers.ts`). 새 spec `wf-export-ui-download.spec.ts` 가 QP-18-01 / 07 / 08 세 양식의 사용자 클릭 동선 + Content-Disposition `filename*=UTF-8''...` 슬러그/확장자 가드. 새 양식 export 버튼이 추가될 때마다 본 describe 에 한 케이스만 추가하면 동일 가드 자동 적용. 미커버: QP-18-03/05/06/10 (UI 진입점 부재 — backend-only), QP-18-09 (validation 상세 진입 필요로 별도 fixture). 토스트 에러 피드백은 별도 finding 으로 분리 — 2026-04-08
...
- [~] CAS 충돌 캐시 무효화 훅 점진적 채택 — 11개 서브클래스 중 8개 처리 완료, 3개는 구조적 이유로 보류 — 2026-04-08
  ...
  - **[DEFER] calibration-plans**: 테이블이 `version`(개정 번호)과 `casVersion`(CAS 잠금)을 **의도적으로 분리** 사용 (calibration-plans.service.ts:74-77 주석 참조). `VersionedBaseService.updateWithVersion()`는 단일 `version` 전제이므로 호출 불가 → hook 트리거 경로 자체가 없음. 현재는 raw `tx.update()` + ConflictException throw 직전 `delete(detail:uuid)` 인라인 무효화로 처리 중. 채택하려면 베이스 클래스에 `casVersion` 컬럼 인식 옵션 추가 또는 도메인 모델 통합 필요 — 별건 아키텍처 PR로 분리
  - **[DEFER] disposal**: 5개 CAS 경로 모두 raw `tx.update()` 직접 구현 (`updateWithVersion()` 미사용) → hook 트리거 불가. 현재 `deleteByPattern(CACHE_PREFIX + '*')` 광범위 무효화로 stale cache 문제 실질적으로 없음. 채택하려면 5개 트랜잭션 구조 재작성 필요 — 비용 대비 효익 낮음, 별건 PR 후보
  - **결론**: 11개 중 채택 가능했던 8개 모두 처리. 잔여 3개는 단순 boilerplate가 아닌 도메인 설계 차이 → 이 항목은 사실상 종결, 후속은 각 모듈별 별건 부채로 추적
