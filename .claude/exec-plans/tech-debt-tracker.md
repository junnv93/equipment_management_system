...
- [ ] **[2026-05-06 commit-pipeline S-1] 🟡 MEDIUM frontend-lintstaged-lint-parity** — frontend `eslint.config.mjs`(ESLint 9 flat config)와 `.lintstagedrc.json`의 frontend glob `apps/frontend/**/*.{ts,tsx}` 정합성 자동 검증. backend는 본 sprint에서 closure됐으나 frontend는 별도 분석 깊이 필요 (flat config + Next.js eslint 통합). `scripts/verify-lint-ruleset-parity.mjs`에 frontend 영역 추가 또는 별도 스크립트. 트리거: frontend lint 회귀 incident 또는 ESLint upgrade sprint.
