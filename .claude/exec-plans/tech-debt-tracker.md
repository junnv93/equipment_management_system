...
- [ ] **[2026-05-02 inspection-pr2] 🟢 LOW inspection-checkitem-row-base-token-split** — `SelfInspectionFormDialog.tsx:638` 인라인 `'rounded-md border bg-card px-2 py-1.5 transition-colors'`가 `INSPECTION_CHECKITEM_ROW_STATE.rowBase`의 레이아웃 제거 버전을 중복. 해결: `rowBaseGrid` 변형 토큰 신설 (flex/gap 제거, grid 전용) + 인라인 → 토큰 경유. 트리거: 자체점검 row 토큰 수정 시 드리프트 감지.
- [ ] **[2026-05-02 inspection-pr2] 🟢 LOW toggle-group-dark-mode-primitive-color** — `INSPECTION_OVERALL_RESULT_TOGGLE.itemPass/itemFail` + `INSPECTION_CHECKITEM_ROW_STATE.judgmentToggle`에 `emerald-100/rose-100` Tailwind primitive 사용 — dark mode에서 미적응(밝은 색 유지). `dark:bg-emerald-950/30 dark:text-emerald-400` 형태로 dark 오버라이드 추가 필요. 기존 SelfInspectionFormDialog.tsx:495 `dark:bg-rose-950/30` 패턴 참조. 트리거: dark mode QA 또는 접근성 sprint.
