...
>
> 자기검토 #3 라운드 5건 (4 valid + 1 STALE) — **2026-05-08 sprint `phase-c-followup-closure` 통합 closure** (Mode 2 Full harness, commit `0587277c`) + **2026-05-09 라운드 #2 사용자 압력 대응 systemic gap 2건 closure** (commit `e410f275` — sentinel `_all` 통일 + BasicInfoTab hook 마이그레이션). archive batch `phase-c-followup-closure`.
>
...
- [ ] **[2026-05-09 phase-c-r2 후속] 🟢 LOW tab-footer-link-rtl-spec** — `MaintenanceHistoryTab.test.tsx` / `CalibrationFactorsTab.test.tsx` / `IncidentHistoryTab.test.tsx` footer link href + i18n 키 RTL 검증. 본 sprint는 e2e wf-equipment-calibration-history-sub-route Case 3 + verify-hardcoding SKILL grep 으로 footer raw path 0건 정합 — RTL 추가는 testing surface 증가. 트리거: footer 링크 동작 회귀 또는 디자인 토큰 추출 sprint.
- [ ] **[2026-05-09 phase-c-r2 후속] 🟢 LOW main-calibration-sentinel-unification** — 메인 `/calibration` (`useCalibrationFilters`) 의 site/team/approval/result/calibrationDueStatus 5 필터 sentinel 일관성 — 일부는 `_all` (line 321), 일부는 `'all'` (line 79 useFilterSelect 명시 인자) 혼재. 본 sub-route 는 `_all` 통일 완료. 메인도 `_all` SSOT 통일 가능 — 큰 변경 (5 SelectItem value + 5 useFilterSelect 인자 + URL parser 정합). 트리거: 메인 calibration 도메인 일관성 sprint 또는 사용자 메인↔sub URL 호환 issue 보고.
