...
- [ ] **[2026-04-21 tech-debt-batch-0421c] 🟢 LOW SecurityService 단위 스펙 미작성** — `apps/backend/src/modules/security/__tests__/security.service.spec.ts` 없음. `saveReport()` DB INSERT 경로 + DB 오류 시 throw 금지 동작 미검증. 트리거: security 모듈 수정 시.
- [ ] **[2026-04-21 tech-debt-batch-0421c] 🟢 LOW renderer MERGED_TEXT_COL 상수 스펙 미작성** — `checkout-form-renderer.service.ts`, `equipment-import-form-renderer.service.ts` 확인 문장 행이 `MERGED_TEXT_COL` 상수를 사용하는지 renderer spec에 byte-level 검증 없음. 트리거: docx renderer 수정 시.
