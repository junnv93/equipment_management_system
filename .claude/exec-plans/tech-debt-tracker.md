...
- [x] **[2026-04-27 sprint4-3-to-5] 🔴 HIGH bulk-approve-endpoint** — ✅ DONE 2026-04-27 harness: checkout-sprint4-3-to-5 (eval PASS). `bulk-approve.dto.ts` + controller M8 + service bulkApprove(Promise.allSettled + CAS). 947 tests PASS.
- [ ] **[2026-04-27 sprint4-3-to-5] 🔴 HIGH rejection-presets-endpoint-seed** — `rejection_presets` 테이블·컨트롤러·서비스 완료(M9 PASS). **미완**: seed 데이터 미삽입 — UL-QP-18 절차서 기준 반려 사유 프리셋 5건 사용자 확인 후 `seed-test-new.ts`에 추가 필요. 마이그레이션(`0047_add_rejection_presets.sql`) 존재 — `db:reset`/`db:migrate` 실행 시 자동 적용. 트리거: 반려 사유 프리셋 UI 개발 착수 시.
- [x] **[2026-04-27 sprint4-3-to-5] 🟡 MEDIUM destinations-recent-endpoint** — ✅ DONE 2026-04-27 harness: checkout-sprint4-3-to-5 (M10 PASS). userId scope + 60s TTL + selectDistinct + limit 5.
- [x] **[2026-04-27 sprint4-3-to-5] 🔴 HIGH revoke-approval-endpoint** — ✅ DONE 2026-04-27 harness: checkout-sprint4-3-to-5 (M11 PASS). fail-close(scope→FSM→domain) + CAS + REVOCATION_WINDOW_EXPIRED + AuditLog extraInfo.
- [x] **[2026-04-27 sprint4-3-to-5] 🟢 LOW backend-lint-unused-vars** — ✅ DONE 이미 수정됨. `pnpm --filter backend run lint` exit 0 확인 (2026-04-27).
...
- [x] **[2026-04-27 approvals-ui-r2] 🟡 MEDIUM approvals-mini-stepper-e2e-spec** — ✅ DONE 2026-04-27 harness: approvals-ar7-ar6-e2e. `wf-ap01-approvals-mini-stepper.spec.ts` 생성. `role="progressbar"` 균일 렌더 + aria-valuemax + 단일단계 분수레이블 미노출 E2E 커버.
- [x] **[2026-04-27 approvals-ui-r2] 🟡 MEDIUM approvals-bulk-reject-e2e-spec** — ✅ DONE 2026-04-27 harness: approvals-ar7-ar6-e2e. `wf-ap02-approvals-bulk-reject.spec.ts` 생성. BulkActionBar aria-hidden 토글 + RejectModal bulk 모드 + 일괄 반려 완료 + 선택 해제 시퀀스 커버.
