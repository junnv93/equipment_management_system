...
### 2026-04-27 verify-implementation: tech-debt-0427-open 후속 WARN 항목

- [ ] **[2026-04-27 verify-auth] 🟢 LOW self-inspections-role-literal-ssot** — `self-inspections.controller.ts:284` role 값을 문자열 리터럴(`'system_admin'`, `'technical_manager'`)로 직접 비교. `UserRoleValues.SYSTEM_ADMIN` / `UserRoleValues.TECHNICAL_MANAGER` SSOT 경유로 교체. 트리거: self-inspections 컨트롤러 수정 시.
- [ ] **[2026-04-27 verify-hardcoding] 🟢 LOW revocation-error-message-dynamic** — `checkouts.service.ts:3209` 에러 메시지 `'Approval can only be revoked within 5 minutes of approval'` 에 시간 값 하드코딩. `${APPROVAL_REVOCATION_WINDOW_MS / 60_000} minutes` 동적 계산으로 교체. 트리거: checkouts.service.ts 수정 시.
