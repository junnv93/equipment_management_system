...
- [ ] **[2026-05-08 sub-route-nav 🟡 MEDIUM] equipment-detail-sub-route-navigation-link** — equipment 상세 페이지(`/equipment/[id]`)에서 모든 sub-route (`repair-history` / `calibration-factors` / `non-conformance` / `calibration-history`)로 진입하는 *primary navigation link* 부재. 현재 시스템 전반 패턴이 link 0건이라 본 sprint 단편 closure 부적합 — 별도 architecture sprint로 *전 sub-route 통합 navigation* 처리 권장. 후보 위치: EquipmentDetailClient에 tab/sidebar entry, IncidentHistoryTab의 contextual link 패턴 확장. 트리거: equipment detail UX 통합 sprint 또는 사용자 navigation flow 개선 요청.
