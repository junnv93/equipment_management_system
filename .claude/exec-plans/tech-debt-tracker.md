...
- [ ] **[2026-04-29 nextauth-csrf §S3] 🟡 MEDIUM docker-compose-onprem-prod-manual-verification** — `pnpm compose:onprem` 후 `curl "$ONPREM_PUBLIC_ORIGIN/api/auth/csrf"` → 200 + JSON 응답 확인. nginx NextAuth handler 분기(`infra/nginx/lan.conf`: legacy 파일명, onprem compose에서 사용) 운영 환경 manual 검증. 트리거: 다음 온프레미스/prod 배포 직전.
