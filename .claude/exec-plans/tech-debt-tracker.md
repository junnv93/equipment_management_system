...
- [ ] **[2026-04-30 sprint-4.5 SHOULD 후속] 🟢 LOW verify-bulk-action-bar-step-8-group-header** — `verify-bulk-action-bar/SKILL.md` 현재 Step 7. 그룹 헤더 indeterminate 패턴(`getGroupRowIds` SSOT + Radix `data-state="indeterminate"` + `aria-checked="mixed"` 자동 매핑) Step 8로 추가. 본 세션 격리 fixture 패턴(부모 통합 없이 검증)도 Step 9 candidate. 트리거: BulkActionBar 도메인 SKILL 정비 sprint.
