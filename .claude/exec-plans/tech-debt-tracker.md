...
- [ ] **[2026-04-20 e2e-api-endpoints-migration] pre-existing E2E 실패 13건** — auth(site 시드 미설정/profile 500), checkouts(create/approve DTO), calibration-plans(confirm item), site-permissions(사이트 조회), incident-nc-integration(5건). 경로 변환과 무관한 서비스/시드 레벨 이슈. 별도 수정 필요.
- [ ] **[2026-04-20 review-architecture] BE: CalibrationPlanSyncListener 이중 갱신** — `calibration-plan-sync.listener.ts:24`. `linkActualCalibrationToPlanItem` 실행 후 리스너가 `isNull(actualCalibrationId)` 조건 없이 동일 항목 재갱신. `payload.linkedPlanItemId`가 있으면 직접 id로 업데이트, 없을 때만 자동 탐색하도록 분기 추가 필요.
- [ ] **[2026-04-20 review-architecture] BE: CALIBRATION_UPDATED/UPLOADED/REVISED 이벤트 미발행** — `calibration.service.ts:574`. 레지스트리 등록만 있고 `update()` + 인증서 업로드/수정 경로에 `emitAsync` 없음. 레지스트리 기반 아키텍처 SSOT 위반. 이벤트 발행 추가 또는 레지스트리에서 제거.
- [x] **[2026-04-20 review-architecture] FE: ValidationDetailContent `useCasGuardedMutation` 교체** — ✅ 확인 종결 2026-04-20. `ValidationDetailContent.tsx:124` 이미 `useCasGuardedMutation` 사용 중. tracker 항목만 closed.
- [ ] **[2026-04-20 review-architecture] FE: VersionHistory staleTime 미지정** — `VersionHistory.tsx:42`. `staleTime: CACHE_TIMES.MEDIUM` 이상 적용 권장.
...
- [x] **[2026-04-19 docx-append-section-sectpr] S1 — insertBeforeSectPr fallback의 replace('</w:body>', ...) 패턴** — ✅ 완료 2026-04-20. `docx-template.util.ts` fallback을 `lastIndexOf('</w:body>')` + slice+concat 방식으로 교체.
...
- [x] **[2026-04-17 qr-phase3] 🟢 LOW Lighthouse/axe/bundle 배포 게이트** — ✅ 완료 2026-04-20. `.github/workflows/performance-audit.yml` + `accessibility-audit.yml` + `bundle-size.yml` 3종 신규. `docs/operations/performance-budgets.md` SSOT (Lighthouse/CWV/axe/bundle 임계값). 실측은 PWA 완결 시점.
...
- [x] **[2026-04-19 sw-validation] 🟡 MEDIUM wf-14b E2E 미업데이트** — ✅ 완료 2026-04-20. Steps 12-15(기존 유지) + Step 16(배너 표시) + Step 17(quality_approve 후 배너 소멸) 추가. FRONTEND_ROUTES.SOFTWARE.DETAIL 경유 브라우저 assertion.
- [x] **[2026-04-19 sw-validation] 🟢 LOW 재검증 배너 i18n 메시지 미구분** — ✅ 완료 2026-04-20. ko: "유효성 확인이 필요합니다. (ISO/IEC 17025 §6.4.13)". en: "Software validation is required. (ISO/IEC 17025 §6.4.13)" 신규 추가. 두 시나리오(최초 미검증/버전 변경) 모두 정확한 중립 문구.
...
- [x] **[2026-04-19 sw-validation] 🟡 MEDIUM Phase 3 — FE exportability 유틸 없음** — ✅ 완료 2026-04-20. `ValidationDetailContent.tsx` ExportFormButton에 `disabled={!isValidationExportable(validation.status)}` 바인딩 + `@/lib/utils/software-validation-exportability` import 연결.
...
- [x] **[2026-04-19 sw-validation] 🟢 LOW Phase 7 — renderer spec 없음** — ✅ 완료 2026-04-20. `__tests__/software-validation-renderer.service.spec.ts` 신규. PizZip in-memory 9테이블 픽스처 + 9케이스: smoke/T0 vendorName/T4 name,method/T6 slicing(4th excluded)/SSOT 상수/빈배열/storage mock. 9 PASS.
...
- [ ] **[2026-04-20 cplan-export-audit] 🟡 MEDIUM Phase 2 — year "_all" 필터** — `CalibrationPlansContent.tsx` "모든 연도" 옵션 (ErrorState onRetry는 ✅ 완료 2026-04-20).
...
- [x] S3: CACHE_EVENTS.CALIBRATION_CREATED payload에 linkedPlanItemId 포함 미구현 — ✅ 완료 2026-04-20. `CalibrationCachePayload.linkedPlanItemId?: string | null` 필드 추가 + `calibration.service.ts` emit 시 `planItemId ?? null` 주입.
...
- [x] 고아 SQL 파일 정리: 0033_software_validation_constraints.sql, 0034_audit_log_append_only.sql, 0035_test_software_latest_validation.sql, 0036_software_validations_composite_idx.sql — ✅ 완료 2026-04-20. journal 검증 후 4개 orphan 삭제. canonical(0034/0035/0036/0037) 보존.

### 2026-04-20 nc-detail-design-fix review-architecture 이연

- [x] **[W-FE1] NCDetailClient.tsx:329 편집 권한 명시성** — ✅ 완료 2026-04-20. `canEditNC = canCloseNC` alias 추가 + UL-QP-18 §14 근거 주석으로 의도 명확화.
- [x] **[W-BE2] calibration-plan-renderer.service.ts:72 confirmedSignature 로직** — ✅ 완료 2026-04-20. `confirmedBy: 각 항목의 확인자(plan 작성자와 별개). UL-QP-19-01 §4: 항목별 확인 서명` 주석 추가.
- [x] **[W-BE3] calibration-plans-export.service.spec.ts:173 테스트 하드코딩** — ✅ 완료 2026-04-20. `'연간교정계획서'` 리터럴 → `FORM_CATALOG['UL-QP-19-01'].name.replace(/\s+/g, '')` SSOT 경유로 교체.
