...
- [ ] **[2026-04-19 qr-sampler-review] 🟢 LOW W-3: LABEL_CONFIG.cell 관심사 혼합** — `qr-config.ts`. `cell` 객체가 레이아웃 파라미터(qrPaddingLeftMm, fontStack 등)와 스케일 기준값(referenceLabelHeightMm)을 혼합. 장기적으로 `LABEL_CONFIG.scaling = { referenceLabelHeightMm }` 네임스페이스 분리 권장.
- [ ] **[2026-04-19 qr-sampler-review] 🟢 LOW pt→mm 매직 넘버 0.353** — `generate-label-pdf.worker.ts:737` `sampler.headerFontPt * 0.353`. `const PT_TO_MM = 0.353` named 상수로 추출 또는 `LABEL_SAMPLER_CONFIG`에 편입 권장.
- [ ] **[2026-04-19 qr-sampler-review] 🟢 LOW i18n size.* 치수 하드코딩** — `ko/qr.json`, `en/qr.json`의 `qrDisplay.size.*` 값에 mm 치수가 문자열로 박힘. `sampler.header.*`처럼 `{widthMm}/{heightMm}` 보간 토큰으로 교체하면 LABEL_SIZE_PRESETS 변경 시 자동 반영.
