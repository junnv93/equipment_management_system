...
- [ ] **[2026-04-22 p1p3] 🟢 LOW approveReturn scope 체크 비대칭** — `approveReturn`(L1883)은 `enforceScopeFromCheckout`(추가 DB 쿼리) 사용, `approve`/`rejectReturn`은 `enforceScopeFromData`(쿼리 0). `checkTeamPermission`은 PR-20에서 완료. 잔여: `enforceScopeFromCheckout` → `enforceScopeFromData`로 교체 시 firstEquip.site/teamId 활용. 트리거: approveReturn 리팩토링 시.
...
- [ ] **[2026-04-22 pr22] 🟡 MEDIUM approvals-api.ts UASVal 하드코딩 3건** — `mapCheckoutToApprovalItem`(L1142: `status: 'pending_review'`), `mapNonConformanceToApprovalItem`(L1165: `status: 'pending'`), `mapInspectionToApprovalItem`(L1192: `status: 'pending'`) — `UASVal`이 이미 import되어 있으나 raw 리터럴 사용. 현재 값이 SSOT와 우연히 일치하여 런타임 버그 없음. 수정: `UASVal.PENDING_REVIEW`, `UASVal.PENDING`으로 교체. verify-ssot Step 24에 탐지 명령어 추가 완료. 트리거: approvals-api.ts 수정 시.
