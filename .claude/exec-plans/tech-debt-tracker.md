...
- [ ] **[2026-04-19 qr-review] 🟡 MEDIUM sql 템플릿 → eq() 전환 (enum 필터)** — `form-template-export.service.ts:803-810`(cables.site/connectorType/status), `equipment-registry-data.service.ts:70`(equipment.status). 타입 안전한 `eq()` 대신 `` sql`${col} = ${val}` `` raw 사용 → 잘못된 enum 값 입력 시 400 아닌 빈 결과 반환. `eq(cables.site, filter.site as Site)` 패턴으로 교체.
- [ ] **[2026-04-19 qr-review] 🟡 MEDIUM HandoverQRDisplay appUrl 빈 문자열 가드** — `HandoverQRDisplay.tsx:58-78` `issueAndRender`에 `if (!appUrl) return` 가드 없음. `EquipmentQRCode.tsx:66`의 `enabled: !!appUrl` 패턴과 대칭성 부재. SSR hydration 경쟁 시 error 화면 순간 표시 가능.
- [ ] **[2026-04-19 qr-review] 🟡 MEDIUM refetchOnMount QUERY_CONFIG 통합** — `EquipmentListContent.tsx:226-232` `...QUERY_CONFIG.EQUIPMENT_LIST, refetchOnMount: 'always'` 인라인 오버라이드. `QUERY_CONFIG.EQUIPMENT_LIST`에 이유 주석과 함께 통합하여 SSOT 단일화.
- [ ] **[2026-04-19 qr-review] 🟢 LOW appUrl getAppUrl() 유틸 추출** — EquipmentQRCode/HandoverQRDisplay/BulkLabelPrintButton 3곳 중복 로직. `apps/frontend/lib/utils/app-url.ts` 유틸 신설.
