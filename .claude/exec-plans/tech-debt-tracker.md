...
- [x] **[2026-04-27 dashboard-design] 🟢 LOW dark-text-brand-prefix** — ✅ DONE 2026-04-27 harness: tech-debt-0427-cleanup. `dashboard.ts:101,105` dark:text-brand-info → text-brand-info 교체 완료.
- [ ] **[2026-04-27 tech-debt-0427-cleanup] 🟢 LOW equipment-ts-dark-text-brand-info** — `equipment.ts:92` `dark:text-brand-info` 잔존. CSS 변수 자동 전환 체계 위반. 트리거: design-token 감사 시.
...
- [x] **[2026-04-27 vi] 🟢 LOW guidance-urgency-normal-i18n** — ✅ DONE 2026-04-27 harness: tech-debt-0427-cleanup. ko `"일반"` / en `"Normal"` 값 확인 완료 — 이미 설정됨.

### 2026-04-27 harness: Sprint 4.1+4.2 NextStepPanel+Row3Zone SHOULD 이연 항목

- [x] **[2026-04-27 sprint-4.1] 🟡 MEDIUM actor-variant-role-mapping-gap** — ✅ DONE 2026-04-27 harness: fsm-terminal-actor-variant. `ActorVariant` + `roleToActorVariant` SSOT 승격(schemas), `isMyTurn` + `YourTurnBadge` 구현, `data-my-turn` 전 variant 적용.
...
### 2026-04-27 harness: equipment-pwa-audit-i18n SHOULD 후속

- [ ] **[2026-04-27 equipment-pwa-audit-i18n] 🟢 LOW formatFilters-compound-i18n** — `PrintableAuditReport.tsx` `formatFilters()` 내부 `대상=`, `액션=`, `사용자=`, `시작일=`, `종료일=`, `전체` 6개 compound 문자열 하드코딩 잔존. format message 패턴(`{label}: {value}`)으로 리팩토링 필요. 트리거: audit 보고서 i18n 리뷰 시.
...
- [x] **[2026-04-24 sprint-1.5] 🟡 MEDIUM fsm-terminal-step-index-semantics** — ✅ DONE 2026-04-27 harness: fsm-terminal-actor-variant. `terminatedFromStatus` DB 컬럼(nullable) + `computeReachedStepIndex` + `NextStepDescriptor.reachedStepIndex` 구현. reject/borrowerReject/cancel 시 `terminatedFromStatus` 저장, `buildNextStep`에서 전달.
...
- [ ] **[2026-04-27 dashboard-phase4-6] 🟢 LOW row4-use-translations-extraction** — `DashboardRow4.tsx`에서 `useTranslations('dashboard')` 직접 호출 (t('srOnly.recentActivity')). Row 파일은 순수 presentational이어야 하므로 `ariaLabel?: string` prop으로 추출하고 DashboardClient에서 전달하는 패턴으로 전환 권장. 트리거: Row 파일 i18n 감사 시.
...
### 2026-04-27 page-container-ssot: FRONTEND_ROUTES 인라인 경로 하드코딩 (pre-existing)

- [ ] **[2026-04-27 page-container-ssot] 🟡 MEDIUM frontend-routes-inline-href** — `EquipmentStickyHeader.tsx:156` `href="/equipment"` → `FRONTEND_ROUTES.EQUIPMENT.LIST`. `CreateCalibrationPlanContent.tsx:175,358` `href="/calibration-plans"` (2곳) → `FRONTEND_ROUTES.CALIBRATION_PLANS.LIST`. `NonConformanceManagementClient.tsx:288,385,507,521` 직접 경로 4곳 → `FRONTEND_ROUTES.EQUIPMENT.DETAIL(id)` 등. 총 7곳. 트리거: 해당 파일 다음 수정 시 함께 교정. verify-hardcoding Step 28으로 추적.

### 2026-04-27 harness: checkout-sprint4-3-to-5 Phase 4 Backend APIs 이연

- [ ] **[2026-04-27 sprint4-3-to-5] 🔴 HIGH bulk-approve-endpoint** — `POST /checkouts/bulk-approve` 미구현. DTO(Zod, ids uuid[] min1 max50, commonReason?), Controller(@Post('bulk-approve'), @RequirePermissions(APPROVE_CHECKOUT)), Service(Promise.allSettled + 개별 CAS + cross-team reject), 응답({approved, failed}), approverId=extractUserId(req), cache invalidate(view:*+resource:*). 트리거: 관리자 대량 승인 UX 필요 시.
- [ ] **[2026-04-27 sprint4-3-to-5] 🔴 HIGH rejection-presets-endpoint** — `GET /checkouts/rejection-presets` 미구현. 신규 Drizzle 테이블 `rejection_presets` 스키마(packages/db/src/schemas/rejection-presets.ts), 마이그레이션(db:generate+db:migrate), Controller(@Get, @RequirePermissions(REJECT_CHECKOUT)), Seed 5건(도메인 데이터는 반드시 사용자 확인 후 추가 — 임의 생성 금지). 트리거: 반려 사유 프리셋 UI 필요 시.
- [ ] **[2026-04-27 sprint4-3-to-5] 🟡 MEDIUM destinations-recent-endpoint** — `GET /checkouts/destinations/recent` 미구현. Controller(@Get('destinations/recent'), @RequirePermissions(VIEW_CHECKOUTS)), Service(getRecentDestinations(userId, limit=5), userId scope, Drizzle groupBy+desc), 캐시(1분 TTL, key에 userId 포함). 트리거: 반출 신청 폼 목적지 자동완성 필요 시.
- [ ] **[2026-04-27 sprint4-3-to-5] 🔴 HIGH revoke-approval-endpoint** — `POST /checkouts/:id/revoke-approval` 미구현. Controller(@Post(':uuid/revoke-approval'), @RequirePermissions(APPROVE_CHECKOUT)), Service: fail-close(scope→FSM(status===approved+5분 이내)→domain(approvedBy===approverId)), CAS(version 불일치→409), ErrorCode.REVOCATION_WINDOW_EXPIRED 신규 등록(packages/schemas/src/error-codes.ts), AuditLog(revokedBy/revokeReason/previousApprovedAt). 트리거: 승인 취소 UX 필요 시.
- [ ] **[2026-04-27 sprint4-3-to-5] 🟢 LOW backend-lint-unused-vars** — `apps/backend/src/modules/checkouts/checkouts.service.ts:11` `Site` import, `data-migration.service.ts:685` `chunkOffset`, `data-migration.service.ts:527` `siteCode2` — unused vars. lint 5errors 중 3건. 트리거: 해당 파일 작업 시 함께 수정.

### 2026-04-27 harness: approvals-ui-r2 SHOULD 이연 항목

- [ ] **[2026-04-27 approvals-ui-r2] 🟢 LOW reject-modal-ux-min-length-inconsistency** — RejectModal에 "10자 이상 필수" UX 힌트 미표시. 프론트엔드는 min(1) 검증, 백엔드는 REJECTION_MIN_LENGTH(10) 검증. 사용자가 1~9자 입력 시 서버 에러로 반려 → 혼란 가능. 수정: 입력창 helper text에 "10자 이상 입력" 안내 추가. 트리거: 반려 UX 개선 Sprint 시.
- [ ] **[2026-04-27 approvals-ui-r2] 🟢 LOW bulk-reject-modal-no-auto-close** — 일괄 반려 성공 시 모달 자동 닫힘 미구현. 현재는 성공 후 수동 닫기 필요. 수정: `bulkReject` resolve 후 `onClose()` 자동 호출. 트리거: 반려 워크플로우 UX 개선 Sprint 시.
...
### 2026-04-27 verify-implementation: approvals-ui-r2 검증 이슈

- [ ] **[2026-04-27 approvals-ui-r2] 🟡 MEDIUM approval-card-border-tokens-dead-code** — `APPROVAL_CARD_BORDER_TOKENS`(approval.ts:53) + `getApprovalCardBorderClasses()`(approval.ts:583)가 컴포넌트에서 미사용. 내부 함수 참조만 존재하여 export는 유효하나 실질 소비처 없음. 수정: 컴포넌트에서 실제 card-view 레이아웃 도입 시 연결하거나 card-view 미도입 확정 시 제거. 트리거: approvals card-view 레이아웃 Sprint 시.
- [ ] **[2026-04-27 approvals-ui-r2] 🟡 MEDIUM approvals-mini-stepper-e2e-spec** — `approvals-mini-stepper.spec.ts` 미생성. `ApprovalRowMiniStepper.tsx` `role="progressbar"` ARIA + valuenow/valuemax + dot 상태 전환 E2E 미커버. 트리거: approvals E2E 확장 Sprint 시.
- [ ] **[2026-04-27 approvals-ui-r2] 🟡 MEDIUM approvals-bulk-reject-e2e-spec** — `approvals-bulk-reject.spec.ts` 미생성. `RejectModal mode='bulk'` + 일괄 사유 입력 + Promise.allSettled 부분 실패 케이스 미커버. 현재 `approvals-bulk-bar.spec.ts`는 BulkActionBar 가시성만 검증. 트리거: approvals E2E 확장 Sprint 시.

### 2026-04-27 verify-implementation: checkout-sprint4-3-to-5 pre-existing 이슈

- [ ] **[2026-04-27 checkout-sprint4-3-to-5] 🟡 MEDIUM i18n-audit-borrower-actions** — `apps/frontend/messages/ko/audit.json` + `apps/frontend/messages/en/audit.json` 에서 `actions.borrower_approve`, `actions.borrower_reject` 키 누락. `CheckoutAuditLog` 표시 시 "actions.borrower_approve" 원문 노출. 수정: ko="대여자 승인"/"대여자 반려", en="Borrower Approve"/"Borrower Reject" 키 추가. 트리거: audit log UI 작업 시.
- [ ] **[2026-04-27 checkout-sprint4-3-to-5] 🟡 MEDIUM i18n-qr-micro-size** — `apps/frontend/messages/ko/qr.json` + `apps/frontend/messages/en/qr.json` 에서 `qrDisplay.sampler.header.micro`, `qrDisplay.size.micro` 키 누락. QR 사이즈 선택기에서 micro 옵션 레이블 원문 노출. 수정: ko="초소형", en="Micro". 트리거: QR 설정 UI 작업 시.
- [ ] **[2026-04-27 checkout-sprint4-3-to-5] 🟡 MEDIUM stale-time-query-config-presets** — 다수 파일(CheckoutHistoryTab.tsx, CheckoutListClient.tsx 외)에서 `staleTime: CACHE_TIMES.*` 직접 지정, `QUERY_CONFIG.*` 프리셋 미사용. REFETCH_STRATEGIES SSOT 위반. 수정: `staleTime` 직접값 → `...QUERY_CONFIG.NORMAL` 등 프리셋으로 교체. 트리거: 해당 파일 다음 수정 시 함께 교정. verify-hardcoding Step 28으로 추적.
- [ ] **[2026-04-27 checkout-sprint4-3-to-5] 🟡 MEDIUM checkout-history-purpose-raw-literals** — `CheckoutHistoryTab.tsx:273,276,279` SelectItem value에 purpose raw 리터럴(`'calibration'`, `'repair'`, `'rental'`) 하드코딩. `CheckoutPurpose` enum → `@equipment-management/schemas` import 후 치환 필요. 트리거: CheckoutHistoryTab 다음 수정 시.
