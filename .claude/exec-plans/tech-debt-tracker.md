...
- [x] **[2026-05-08 reject-modal S-2 후속] 🟢 LOW reviewDisposalSchema-discriminatedUnion** — 완료 (다른 세션 tech-debt-5items-closure + 본 세션 테스트 정렬 2026-05-09). disposal.service.ts 수동 check 제거 + 테스트 baseValid.decision approve→reject 수정 완료.
...
> **2026-05-09 sprint `zod-hub-should-s4-followups` closure (Round #2 확장)**: 1라운드 alert-rule + e2e-toast ✅ + ESLint [SKIP-trigger-not-met]. **시니어 자기검토 라운드 #2** 결과 운영 maturity 갭 3건 추가 closure (alert annotations actionability + Prometheus 운영 절차 문서 + promtool 정적 검증 SUCCESS 13 rules). 분리 등록 4건 (en parity / shared fixture / 실 실행 검증 / threshold baseline) 아래 명시. archive batch `zod-hub-should-s4-followups`.
...
### 2026-05-09 zod-hub-should-s4-followups 라운드 #2 후속

- [ ] **[2026-05-09 zod-hub-r2 후속] 🟢 LOW en-locale-zod-toast-e2e** — 본 sprint e2e (`zod-fail-toast.spec.ts`) 는 ko 단일 locale 검증. ADR-0008 ko/en parity 운영 전제이나 런타임 ICU 변수 보존 (`{minimum}`/`{format}`/`{expected}`) 은 ko 한정 검증. 분리 등록 사유: en locale 검증은 NEXT_LOCALE 쿠키 setLocale + storageState 분기 + en messages parity 추가 → 별도 sprint 적합. 트리거: en 사용자 운영 incident 발생 또는 다국어 운영 강화 sprint.
- [ ] **[2026-05-09 zod-hub-r2 후속] 🟢 LOW nc-seed-reset-shared-fixture** — `resetNcToCorrected()` (zod-fail-toast.spec.ts) 와 `resetNcForRejectionTest()` (nc-rejection-flow.spec.ts) SQL 90% 동일 (UPDATE non_conformances + cache-clear). DRY 위반 — `apps/frontend/tests/e2e/shared/helpers/nc-seed-helpers.ts` 추출 후보 (signature: `resetNcToStatus(ncId, status, options)`). 트리거: 3번째 NC reset 헬퍼 사용 spec 추가 시 또는 NC 시드 schema 변경 발생 시.
- [ ] **[2026-05-09 zod-hub-r2 후속] 🟡 MEDIUM zod-fail-toast-actual-playwright-execution** — 본 sprint 는 tsc PASS + grep 검증 + promtool 정적 검증까지 완료, **실제 playwright 실행 검증 미수행**. 환경 부담 (backend + 시드 + cache-clear endpoint 의존) 로 본 sprint 분리. CI nightly 또는 manual run 필요. 트리거: 다음 e2e 통합 sprint 또는 i18n 회귀 incident 발생 시. 실행 명령: `pnpm exec playwright test zod-fail-toast --project=chromium --workers=1`.
- [ ] **[2026-05-09 zod-hub-r2 후속] 🟢 LOW zod-issues-alert-threshold-baseline** — `> 0.1 req/s` (warning) / `> 1 req/s` (critical) 임계값의 운영 baseline 측정 + SLO 도출 과정 미문서화. 현재는 ADR-0008 §4 reconsideration 트리거 로직 기반 추정값. 운영 데이터 수집 (1-2주 통상 운영 후 p95/p99 측정) → 임계값 보정. 트리거: 본 alert 가 무효 발생 (false positive) 3회 이상 또는 1주 운영 후 baseline metric 수집 가능 시.
