...
- [x] **[2026-04-30 deps-supply-chain] 🟡 MEDIUM file-upload-form-template-spec-신설** — ✅ 2026-04-30 완료 (tech-debt-batch-0430d Phase A N/A). 두 spec 모두 이미 존재 + 16 tests PASS. `file-upload.service.spec.ts` magic-bytes/SHA-256 커버리지 확인.
...
### 2026-04-30 verify-implementation 발견 — tech-debt-batch-0430d 후속

- [ ] **[2026-04-30 verify-impl-batch-0430d] 🟢 LOW display-preferences-select-ssot** — `DisplayPreferencesContent.tsx` SelectItem value를 `@equipment-management/schemas` SSOT 배열 미경유, 리터럴 하드코딩. `SUPPORTED_LOCALES`(`'ko'/'en'`), `ITEMS_PER_PAGE_OPTIONS`(`'10'/'20'/'50'`), `DATE_FORMAT_OPTIONS`(`'YYYY-MM-DD'/'YYYY.MM.DD'`), `EQUIPMENT_SORT_OPTIONS`(`'managementNumber'/'name'/'updatedAt'`)을 import 후 `.map()`으로 렌더링 교체 필요. `CalibrationSettingsContent.tsx`의 `CALIBRATION_ALERT_DAYS_OPTIONS.map()` 패턴이 선례. 트리거: schemas에서 옵션값 추가/제거 시 SelectItem 불일치 방지 위해 즉시 처리 권장.
...
- [x] **[2026-04-28 software-validation-comment] 🟢 LOW service-param-underscore-prefix-static-check** — ✅ 2026-04-30 완료 (tech-debt-batch-0430d). `verify-implementation` SKILL.md Step 21 (경고 레벨 W2) 등록. `grep -rnE "\(_[a-z][a-zA-Z0-9]*[,)]" apps/backend/src/modules --include="*.service.ts"` 탐지 룰 추가.
