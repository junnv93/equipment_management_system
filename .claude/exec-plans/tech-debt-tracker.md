...
- [ ] **[2026-05-09 zod-hub-r3 후속] 🟢 LOW nc-seed-reset-shared-fixture-full-migration** — 라운드 #3 partial closure 후 잔여: `apps/frontend/tests/e2e/shared/helpers/nc-seed-helpers.ts` 신설 + `zod-fail-toast.spec.ts` 마이그레이션 완료. **`nc-rejection-flow.spec.ts` 미마이그레이션** — NC_007 의 partial-reset semantics (corrector 필드 보존) 와 helper 의 full-reset (corrector 필드 항상 override) 충돌 가능. 안전한 마이그레이션 위해 helper 시그니처 확장 필요 (`{ preserveCorrectionFields?: boolean }` option 추가). 인접 sprint 코드 회귀 위험으로 본 sprint 보류. 트리거: NC seed schema 변경 또는 3번째 NC reset spec 추가 시.
- [ ] **[2026-05-09 zod-hub-r2 후속] 🟡 MEDIUM zod-fail-toast-actual-playwright-execution** — 본 sprint 는 tsc PASS + grep 검증 + promtool/amtool 정적 검증까지 완료, **실제 playwright 실행 검증 미수행**. 환경 부담 (backend + 시드 + cache-clear endpoint 의존) 로 본 sprint 분리. CI nightly 또는 manual run 필요. 트리거: 다음 e2e 통합 sprint 또는 i18n 회귀 incident 발생 시. 실행 명령: `pnpm exec playwright test zod-fail-toast --project=chromium --workers=1` (4 cases: ko too_small + invalid_format + 다중 issue + en too_small).
