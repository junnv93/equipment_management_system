...
- [ ] **[2026-04-26 nc-verify] 🟡 MEDIUM nceditdialog-form-field-tokens-barrel** — `NCEditDialog.tsx:10` `REQUIRED_FIELD_TOKENS` / `REQUIRED_FIELD_A11Y` import가 `@/lib/design-tokens/form-field-tokens` 서브패스 직접 사용. verify-ssot Step 3 위반 (design-tokens는 barrel 경유 필수). `index.ts`에 두 토큰 barrel re-export 후 NCEditDialog import 수정 필요. 트리거: NC 다음 편집 세션 또는 form-field 토큰 정리 시.
...
- [ ] **[2026-04-26 nc-verify] 🟡 MEDIUM statusAlert-dark-prefix** — `non-conformance.ts:96` `statusAlert` 토큰에 `dark:border-brand-critical/30 dark:bg-brand-critical/10` CSS 변수 자동 전환 체계 위반. 제거하면 `.dark` CSS 변수만으로 자동 처리됨. 트리거: NC 디자인 토큰 정리 세션.
