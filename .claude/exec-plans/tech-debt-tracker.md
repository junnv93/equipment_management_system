...
- [ ] **[2026-04-19 verify] 🟢 LOW verify-hardcoding — getTemplateBuffer() 6곳 문자열 리터럴** — `form-template-export.service.ts` L344/529/681/828/1138/1355 `getTemplateBuffer('UL-QP-18-NN')` 직접 문자열 전달. 메서드 상단에 `entry = FORM_CATALOG[...]` 선언이 이미 있으므로 `entry.formNumber` 로 교체해야 양식번호 개정 시 drift-safe.
- [ ] **[2026-04-19 review-arch] 🟢 LOW exportSoftwareValidation site 스코프 post-filter** — L640-645 `filter.site` 체크가 DB 쿼리 이후 수행 (404 반환, 정보 노출 없음). `WHERE eq(testSoftware.site, filter.site)` 조건을 쿼리에 직접 추가하면 early-reject 패턴과 일관성 확보 + 불필요 쿼리 비용 제거.
