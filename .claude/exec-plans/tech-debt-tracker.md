...
- [ ] `reports.service.ts:scopeToEquipmentConditions` 새 SSOT 에 대해 drift — 같은 4-case 정책 상태기계(`all`/`site`/`team`/`none` + `sql\`FALSE\``) 를 사전 SSOT 승격(8d7c8971) 직전에 인라인 구현. `buildScopePredicate(policy, userCtx, { site, team })` 호출로 마이그레이션하여 SSOT 통합 권장. 단, 본 함수는 `ResolvedDataScope` 를 인자로 받으므로 호출자 측에서 `userCtx + policy` 로 한 단계 올리거나 dispatchScopePredicate 의 resolved 변형을 추가하는 옵션 검토 — `apps/backend/src/modules/reports/reports.service.ts:102` — 2026-04-08 (review-architecture finding)
