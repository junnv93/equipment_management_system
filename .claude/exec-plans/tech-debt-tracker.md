...
- [ ] reports filter ALL 센티널 통일 — `reports-filter-utils.ts`는 `'_all'`을, `ReportsContent.tsx`는 `'__all__'`을 사용. 실동작 충돌은 없으나 calibration(`'_all'`) 컨벤션과 표기 차이로 추후 혼동 소지. utils에서 센티널 상수 export → 컴포넌트 재사용 — `apps/frontend/lib/utils/reports-filter-utils.ts:97` + `apps/frontend/app/(dashboard)/reports/ReportsContent.tsx:57` — 2026-04-08
- [ ] reports `customDateRange` Date↔string 직렬화 KST 하루 밀림 위험 — `new Date('yyyy-MM-dd')`는 UTC 파싱이라 KST 표시 시 하루 전으로 보일 수 있음. `parseISO` 또는 `new Date(y, m-1, d)` 로 전환 권장 (계산 영향 없음, DatePicker 표시 회귀만) — `apps/frontend/app/(dashboard)/reports/ReportsContent.tsx:84-91` — 2026-04-08
- [ ] `lib/design-tokens/components/audit.ts:37` 의 audit action label record 가 `access_denied` 키 누락 → tsc 에러 (pre-existing, 본 세션 변경 무관). access_denied AUDIT_ACTION 추가 시 UI label record 동시 갱신이 누락된 것으로 추정. label/color/i18n 4-way 동기화 회귀 — `apps/frontend/lib/design-tokens/components/audit.ts:37` — 2026-04-08
- [ ] reports-filter-utils 단위 테스트 부재 — calibration-filter-utils 의 `__tests__` 패턴을 따라 parse round-trip / default / count / ALL_SENTINEL strip 케이스 추가 권장 (harness reports-url-ssot SHOULD S1) — `apps/frontend/lib/utils/__tests__/reports-filter-utils.test.ts` (신규) — 2026-04-08
- [ ] 0006 마이그레이션 USING 캐스트 운영 사전 검증 가드 (hotfix 0008) — `0006_gray_sersi.sql`의 backfill UPDATE는 `NOT IN users.id::text` 만 NULL 처리하므로 비-UUID 형식 string 잔존 시 `ALTER COLUMN ... USING ::uuid` 가 abort. dev 통과(20행)했으나 운영 적용 전 regex 가드 hotfix 0008 + DRIZZLE_MIGRATIONS.md 절차 보강 권장. example-prompts.md 36차 후속 HIGH 항목 — `apps/backend/drizzle/0006_gray_sersi.sql` — 2026-04-08
...
- [~] CAS 충돌 캐시 무효화 훅 점진적 채택 — 부분 해결: 2026-04-08 (commit f8ad419b). 11개 후보 중 단일 detail 캐시 키 패턴이 동일한 6개 서비스 (cables/test-software/calibration-factors/calibration/intermediate-inspections/software-validations) 마이그레이션 완료, 17개 try/catch boilerplate 제거. 남은 5개 follow-up: (1) equipment + checkouts — compound cache key (`{uuid}` + `{uuid, includeTeam}`) 사용으로 override 가 두 키 모두 처리 필요; (2) self-inspections — 현재 catch 자체 부재 (NEW behavior 도입 의사결정 필요); (3) calibration-plans — VersionedBaseService 비사용 (raw `db.update().where(version)` + manual createVersionConflictException) 으로 hook 적용 불가, 별도 마이그레이션; (4) disposal — 본 세션 미조사 — 2026-04-08
