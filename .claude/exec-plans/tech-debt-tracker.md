...
- [ ] **[2026-04-24 rental-phase5-8] 🟢 LOW borrowerReject dialog onErrorCallback reason 미초기화** — `borrowerRejectMutation.onErrorCallback`에서 `setBorrowerRejectReason('')` 미호출. 오류 후 재오픈 시 이전 입력 잔존. 기존 rejectMutation 패턴도 동일 (일관성 유지 중). `onOpenChange`에서도 미초기화. 트리거: dialog UX 개선 작업 시 (PR-18).
- [ ] **[2026-04-24 rental-phase5-8] 🟢 LOW REJECTED 상태 카드에 borrowerRejectionReason 미표시** — `CheckoutDetailClient.tsx` L997 REJECTED 카드가 `checkout.rejectionReason`만 참조. borrower 1차 반려 시 `borrowerRejectionReason`은 표시 안 됨. 트리거: 반출 상세 UX 개선 시.
