...
### 2026-05-01 disposal-zod-defense-in-depth 후속 — Tier 2 도메인 페어링 sprint (Mode 1 harness 발견)

본 세션은 frontend가 이미 `≥ 10` 강제 중인 disposal/calibration-plan 2개 도메인만 backend Zod 격상 (Tier 1 안전). 이하 Tier 2 도메인은 frontend가 `≥ 10` 미강제이므로 backend 격상 시 기존 사용자 입력(1~9자) 거부 회귀. 각 후속 sprint는 **frontend ≥ 10 룰 추가 + backend 격상**을 페어링해야 안전. 모두 `verify-zod` Step 15 검증 명령 #2 결과로 자동 식별 가능.

- [ ] **[2026-05-01 disposal-zod] 🟡 MEDIUM tier-2-equipment-imports-zod-defense-in-depth** — `equipment-imports/dto/reject-equipment-import.dto.ts:9` `rejectionReason: z.string().min(1, ...)` + frontend `EquipmentImportDetail.tsx:567` `!rejectionReason.trim()` 만 검증. ① frontend `>= REJECTION_REASON_MIN_LENGTH` 룰 추가 (toast/disabled 동기) ② backend `.trim().min(REJECTION_REASON_MIN_LENGTH).max(LONG_TEXT_MAX_LENGTH)` 격상 ③ i18n `{min}` 파라미터화. 트리거: 후속 sprint 또는 보안 강화 시.
- [ ] **[2026-05-01 disposal-zod] 🟡 MEDIUM tier-2-calibration-rejectionReason** — `calibration/dto/approve-calibration.dto.ts:28` + `CalibrationApprovalActions.tsx:108` 패턴. frontend 직접 length 체크 없음, backend `.min(1)`. 페어링 sprint 필요.
- [ ] **[2026-05-01 disposal-zod] 🟡 MEDIUM tier-2-calibration-factors-rejectionReason** — `calibration-factors/dto/approve-calibration-factor.dto.ts:14,28` (`approverComment` + `rejectionReason` 둘 다 `.min(1)` 약함). 페어링 sprint.
- [ ] **[2026-05-01 disposal-zod] 🟡 MEDIUM tier-2-self-inspections-rejectionReason** — `self-inspections/dto/approve-self-inspection.dto.ts:15`. 페어링 sprint.
- [ ] **[2026-05-01 disposal-zod] 🟡 MEDIUM tier-2-software-validations-rejectionReason** — `software-validations/dto/approve-validation.dto.ts:31`. 페어링 sprint.
- [ ] **[2026-05-01 disposal-zod] 🟡 MEDIUM tier-2-intermediate-inspections-rejectionReason** — `intermediate-inspections/dto/approve-inspection.dto.ts:15` + `IntermediateInspectionList.tsx:263` (`!rejectionReason` 단순 체크). 페어링 sprint.
- [ ] **[2026-05-01 disposal-zod] 🟡 MEDIUM tier-2-nc-reject-correction-rejectionReason** — `non-conformances/dto/reject-correction.dto.ts:15` `.trim().min(1)` (trim은 OK, min만 약함) + `NCDetailClient.tsx:614` `!rejectionReason.trim()`. 페어링 sprint.
- [ ] **[2026-05-01 disposal-zod] 🟢 LOW disposal-comment-reject-fail-close-service-layer** — `disposal.service.ts:379` reject 분기에서 `approveDto.comment || '승인 단계에서 반려'` fallback. backend Zod에서 conditional .min은 의도적으로 피했으나(D1) service layer에서도 fallback 사용 → frontend bypass 시 audit log에 fallback 메시지로 들어감. service layer fail-close (decision='reject' && !comment.trim() → throw 400) 추가 검토. 트리거: 보안 감사 sprint.
