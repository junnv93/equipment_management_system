...
- [x] **[2026-04-18 arch-review] 🟠 HIGH H1 — data-migration 케이블 INSERT 후 캐시 무효화 누락** — ✅ 완료 2026-04-19 (tech-debt-batch-0419). `${CACHE_KEY_PREFIXES.CALIBRATION}cables:` 복합 프리픽스로 `deleteByPrefix` 추가. 별도 CABLES 상수 없음 — cables.service.ts:24 기존 패턴 그대로 사용.
...
- [x] **[2026-04-18 verify] 🟡 MEDIUM verify-e2e — networkidle + waitForTimeout** — ✅ 완료 2026-04-19 (tech-debt-batch-0419). `phase2-scanner-ncr.spec.ts` networkidle→load 교체. `seed-view-form.spec.ts` waitForTimeout(3000/1000)→`waitFor({state:'visible',timeout:10000/5000})` 교체. Tailwind utility selector 2건은 별도 이연.
