...
- [ ] **[2026-05-13 cache-wholesale-migration-checkouts] 🟠 MED checkouts wholesale → specific sub-prefix** — `CACHE_KEY_PREFIXES.CHECKOUTS}*` 사용 10 entries (NOTIFICATION_EVENTS.CHECKOUT_CREATED/APPROVED/REJECTED/BORROWER_APPROVED/BORROWER_REJECTED/STARTED/RETURNED/RETURN_APPROVED/RETURN_REJECTED/OVERDUE). 분석 필요: checkout service의 `cache-keys.ts` SSOT가 `list:*`/`detail:*`/`pending:*`로 세분되어 있는지 — 세분 prefix가 정의되어 있으면 registry pattern 분해, 없으면 cache key 재구조화 선행. 트리거: dashboard/checkouts cache churn p99 latency 회귀 또는 정기 architectural sweep.
