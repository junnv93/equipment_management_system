...
- [ ] 프론트엔드 API 클라이언트 site/teamId 명시 송출 일관성 — failLoud 전환 후 follow-up. 대상: `calibration-factors-api.ts`, `notifications-api.ts`(admin), `software-api.ts`. 기능 영향 없음(backend enforceScope가 값 강제 주입), UX/디버깅 명확성 및 SSOT 일관성 개선용 — 2026-04-09
- [ ] `UserQueryDto.teamId` 필드 추가 — `USER_DATA_SCOPE.test_engineer=team` policy가 team 레벨이나 DTO에 teamId 없어 service가 team 필터 미적용. failLoud 마이그레이션 회귀는 아니나 scope 정책과 DTO shape 불일치 — 2026-04-09
