...
- [ ] **[2026-04-21 verify-cache-events] 🟡 MEDIUM SW 검증·테스트소프트웨어 캐시 하이브리드 패턴 문서화** — `software-validations.service.ts`가 `SOFTWARE_VALIDATIONS:*` + `TEST_SOFTWARE:*` 캐시를 서비스 레이어에서 직접 무효화(hybrid: registry=dashboard 전용, service=domain 캐시). verify-cache-events가 패턴 삭제 미등록 경고. 설계 의도를 `cache-event.registry.ts` 주석으로 보강하거나 registry 패턴으로 통합.
- [ ] **[2026-04-21 verify-ssot] 🟢 LOW `scope.type` 인라인 타입 리터럴** — 체크아웃 스코프 관련 `scope.type` 필드에서 `'equipment' | 'team'` 등 인라인 리터럴 사용. `@equipment-management/schemas` 공유 타입으로 통합 필요.
- [ ] **[2026-04-21 verify-auth] 🟡 MEDIUM `equipment.controller.ts` ParseUUID 라우트 순서 충돌** — 동적 라우트(`:uuid`)와 정적 라우트(`disposal-requests` 등) 순서 미정렬. `@Param('uuid', ParseUUIDPipe)` 선적용 컨트롤러에서 정적 경로를 먼저 선언해야 함. 파일: `apps/backend/src/modules/equipment/equipment.controller.ts`
- [ ] **[2026-04-21 verify-auth] 🟡 MEDIUM `EquipmentImportDetail` role 리터럴 하드코딩** — `'ADMIN' | 'MANAGER'` 역할 리터럴을 `UserRole` enum 대신 인라인 사용. `@equipment-management/schemas`의 `UserRole`로 교체 필요.
- [ ] **[2026-04-21 verify-frontend-state] 🟢 LOW `CheckoutHistoryTab` invalidate SSOT 미사용** — `queryClient.invalidateQueries()` 호출 시 `queryKeys` SSOT 상수 대신 문자열 직접 사용. `lib/api/query-config.ts` queryKeys로 교체 필요.
- [ ] **[2026-04-21 verify-i18n] 🟢 LOW 빈 번역 키 2건** — ko/en 중 한 쪽이 빈 문자열인 번역 키 2건 존재. 확인 후 번역 추가 또는 키 제거.
- [ ] **[2026-04-21 verify-i18n] 🟢 LOW routeMap 4개 페이지 미등록** — 존재하는 라우트 4개가 i18n routeMap에 누락. 등록 필요.
