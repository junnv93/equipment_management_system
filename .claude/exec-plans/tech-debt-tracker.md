...
- [x] **[2026-04-18 arch-review] 🟡 MEDIUM M2 — data-migration 세션 상태 메모리 의존** — 2026-04-21 완료: `executionStartedAt` 타임스탬프 + `MIGRATION_EXECUTION_TIMEOUT_MS` (10분) stale 판정으로 서버 재시작 후 재시도 가능. 완전한 Redis/DB 영속화는 프로덕션 사용자 발생 시 재검토.
...
- [x] **[2026-04-19 sw-validation] 🟡 MEDIUM Phase 3 — 10개 컴포넌트 미추출** — 2026-04-21 4개 핵심 컴포넌트 추출 완료: `ValidationCreateDialog`, `ValidationActionsBar`, `ValidationFunctionsTable`, `ValidationControlTable`. `SoftwareValidationContent.tsx` 1033→404줄. 잔여(`ValidationStatusBadge`, `ValidationWorkflowTimeline` 등)는 해당 UI 수정 시 단계적 추출.
...
- [x] **[2026-04-19 sw-validation] 🟢 LOW Phase 7 — 운영 가이드 문서 없음** — 2026-04-21 완료: `docs/references/software-validation-workflow.md` 생성. 상태 전이 다이어그램, ISO §6.2.2/§6.4.13, validationType 분기, assertIndependentApprover 설명 포함. SLA/보존연한은 TBD.

### 2026-04-19 verify-implementation + review-architecture (api-ssot-e2e-serial-export-batch 후속)

- [x] **[2026-04-19 review-arch] 🟢 LOW form-templates-api GET 패턴 불일치** — 2026-04-21 완료: `docs/references/frontend-patterns.md`에 "API GET 응답 패턴 선택" 섹션 추가. envelope vs 비-envelope 패턴 기준 문서화.
- [x] **[2026-04-19 verify-e2e] 🟢 LOW wf-35:103 토스트 .first() 직접 사용** — 2026-04-21 완료: `expectToastVisible(pageB, /데이터 충돌|.../)` 로 교체.

### 2026-04-20 — calibration-phase4-7 verify-* 스킬 갭 업데이트

- [x] **[2026-04-20 skill-gap] 🟢 LOW calibration-plan-renderer.service.ts iCell.alignment 직접 조작** — 2026-04-21 완료: `calibration-plan.layout.ts`에 `ALIGNMENT.CENTER_MIDDLE` / `CENTER_MIDDLE_SHRINK` 토큰 추출 + renderer에서 `Layout.ALIGNMENT.*` 경유 사용.
...
- [x] DocumentTable.tsx: 213줄 (SHOULD ≤120 초과) — 2026-04-21 완료: DocumentUploadButton + DocumentTableRow 분리 → DocumentTable.tsx 85줄

### 2026-04-21 verify-ssot Step 19 — 프론트엔드 ValidationStatus/ValidationType 리터럴 잔여 위반

- [ ] **[2026-04-21 ssot-step19] 🟡 MEDIUM ValidationActionsBar.tsx:43,75,104,123** — `status === 'draft'/'submitted'/'approved'/'rejected'` → `ValidationStatusValues.*` 로 교체
- [ ] **[2026-04-21 ssot-step19] 🟡 MEDIUM ValidationDetailContent.tsx:116** — `validation.status === 'draft'` → `ValidationStatusValues.DRAFT`
- [ ] **[2026-04-21 ssot-step19] 🟢 LOW ValidationEditDialog.tsx:108** — `validation.validationType === 'vendor'` → `ValidationType` 타입 안전 비교 또는 `VALIDATION_TYPE_VALUES.VENDOR`
- [ ] **[2026-04-21 ssot-step19] 🟢 LOW ValidationCreateDialog.tsx:121** — `createForm.validationType === 'vendor'` → 동일

### 2026-04-21 verify-e2e SHOULD 미충족 항목

- [ ] **[2026-04-21 verify-e2e] 🟢 LOW wf-35-cas-ui-recovery.spec.ts:18** — `@playwright/test` 직접 import → `auth.fixture`로 교체
