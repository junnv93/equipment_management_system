...
- [x] **[2026-05-13 cache-wholesale-migration-checkouts] 🟠 MED checkouts wholesale → specific sub-prefix** ✅ closure (commit 진행중) — 10 entries 모두 wholesale `${CHECKOUTS}*` → `list:*` + `summary:*` + `detail:*` specific sub-prefix로 분해. CACHE_INVALIDATION_WHOLESALE_LEGACY_ALLOWLIST에서 10 entries 제거 → audit VIOLATIONS 0. 28/28 jest PASS. 분석: checkouts service `buildCacheKey`가 이미 `list:t:<teamId>` / `list:g:` / `summary:t:<teamId>` / `summary:g:` / `detail:<uuid>` sub-prefix 구조 — 재구조화 불필요. dot prefix (`.destinations`/`.rejection-presets`/`.recent-destinations`)는 reference data로 status 전이 무효화 대상 외.
- [x] **[2026-05-13 cache-wholesale-migration-equipment-imports] 🟠 MED equipment-imports wholesale → specific** ✅ closure — 3 entries (IMPORT_CREATED/APPROVED/REJECTED) 모두 wholesale `${EQUIPMENT_IMPORTS}*` → `list:*` + `detail:*` specific sub-prefix로 분해. helper `invalidateAllEquipmentImports` + `invalidateEquipmentImportsWithEquipment`도 분해 정합. INBOUND_OVERVIEW는 `t:*` sub-prefix로 변경 (cache key가 `t:<teamId>:s:<status>:q:<search>:l:<limit>` 사용). allowlist에서 3 entries 제거. **분석 결과**: equipment-imports service는 현재 cache 미사용 (no-op 상태) — 본 sprint는 향후 cache 도입 시 자동 정합되는 안전망 역할.
...
- [ ] **[2026-05-13 cache-wholesale-migration-calibration-factors] 🟡 MED calibration-factors wholesale → specific** — `${CACHE_KEY_PREFIXES.CALIBRATION_FACTORS}*` 2 entries (NOTIFICATION_EVENTS.CALIBRATION_FACTOR_APPROVED/REJECTED). calibration-factors.service의 list/pending/detail prefix 사용 패턴 분석 + registry 분해. 트리거: 정기 architectural sweep 또는 calibration-factors cache 회귀 발견 시.
- [ ] **[2026-05-13 cache-wholesale-migration-other-legacy] 🟢 LOW other LEGACY wholesale (Disposal/IntermediateCheck)** — `${CACHE_KEY_PREFIXES.DISPOSAL_REQUESTS}*` (DISPOSAL_REJECTED) + `${CACHE_KEY_PREFIXES.CALIBRATION}*` (INTERMEDIATE_CHECK_COMPLETED) 2 entries. 사용 빈도 낮음 → 마지막 마이그레이션 그룹. 트리거: 위 4 sprint closure 후.
