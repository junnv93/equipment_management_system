...
- [ ] **[2026-04-22 PR-2] 🟢 LOW rejectReturn 스코프 검증 무조건 실행** — `if (rejectReturnDto.approverTeamId)` 조건부로 스코프 검증이 실행됨. `enforceScopeFromCheckout`를 조건 밖으로 이동하여 다른 전이 메서드와 방어선 일관성 확보. 트리거: rejectReturn 관련 작업 시.
- [ ] **[2026-04-22 PR-2] 🟢 LOW reject 엔드포인트 반려 사유 검증 이중 위치** — controller.ts L465-470 + service.ts L1531-1535 중복. controller 측 제거하여 서비스 단일 경로로 통일. 트리거: checkouts 컨트롤러 수정 시.
