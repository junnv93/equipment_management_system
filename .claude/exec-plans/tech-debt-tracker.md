...
### 2026-05-03 entropy-pass 발견 항목

- [ ] **[2026-05-03] 🔴 HIGH zod-trim-max-system-wide-residual** — 2026-05-03 entropy 점검에서 step-12/15 잔존 확인. checkouts(destination/reason/conditionBefore/After), teams(name/description), cables(lossDb/optional fields), notifications(title/content), equipment-imports(reason/name/vendorName/ownerDepartment) DTO에 `.trim()` 또는 `.max()` 미적용. 30+ 건. DoS 방어 + 공백 우회 방지. 트리거: zod-trim-max-system-wide sprint (Mode 2). **우선순위 HIGH** — user-facing text fields unbounded.
- [ ] **[2026-05-03] 🟡 MEDIUM revoke-approval.dto.ts `.max()` 미적용** — `apps/backend/src/modules/checkouts/dto/revoke-approval.dto.ts` 철회 사유에 `.max(LONG_TEXT_MAX_LENGTH)` 없음 (VM.required 수정은 2026-05-03 entropy pass에서 완료). 무제한 text 입력 허용. 트리거: zod-trim-max-system-wide sprint에 포함.
- [ ] **[2026-05-03] 🟡 MEDIUM bulk-reject.dto.ts REJECTION_REASON_MIN_LENGTH 미적용** — `bulk-reject.dto.ts`의 reason은 단건 reject와 동등 수준이어야 하나 현재 `.min(1)`. `.trim()`은 entropy pass에서 추가됨. UL-QP-18 정책상 bulk reject도 동일 최소 길이 적용 필요. 트리거: revoke-approval REVOCATION_REASON_MIN_LENGTH 정책 확인 sprint에 병행.

### 2026-05-03 backend-errorcode-full-closure verify-security WARN

- [ ] **[2026-05-03] 🟡 MEDIUM equipment-imports.service.ts BadRequestException 의미론 불일치** — `apps/backend/src/modules/equipment-imports/equipment-imports.service.ts:753` — `ErrorCode.EquipmentImportOnlyRequesterCanCancel` (소유권/scope 위반)을 `BadRequestException`으로 throw. GlobalExceptionFilter가 403으로 보정하므로 런타임 영향 없으나 `ForbiddenException`이 의미론적으로 정확. 트리거: equipment-imports 도메인 정리 sprint.

### 2026-05-03 backend-errorcode-full-closure SHOULD 이연

- [ ] **[2026-05-03] 🟢 LOW cable-errors.ts thin-wrapper 문서화** — `cable-errors.ts`는 `cables-errors.ts`(복수형)로 re-export하는 thin wrapper. 동일 기능이 두 파일에 존재해 독자 혼란 가능. `cables-errors.ts`를 SSOT로 명확화하거나 단일 파일로 통합. 트리거: errors/ 디렉토리 정리 sprint.
- [ ] **[2026-05-03] 🟢 LOW user-errors.ts mapBackendErrorCode 라우팅 없음** — `mapBackendErrorCode`가 domain key map을 무시하고 `code ?? 'UNKNOWN_ERROR'` 반환. 호출자가 `mapBackendErrorCode` 대신 `mapUserErrorToToast` 사용해야 하지만 함수 시그니처로는 구별 불가. 트리거: frontend error mapper 일관성 강화 sprint.
- [ ] **[2026-05-03] 🟢 LOW non-conformance-errors.ts mapBackendErrorCode 미노출** — 5개 신규 mapper 중 non-conformance-errors.ts만 `mapBackendErrorCode` export 없음. 호출자 인터페이스 일관성 결여. 트리거: frontend error mapper 일관성 강화 sprint.
...
### 2026-05-02 backend-errorcode-full-closure 스코프 외 (frontend mapper 미완)

Sprint 2 Phase A에서 신규 추가된 14개 ErrorCode 중 frontend 6개 도메인은 mapper 파일 미생성/미등록 상태.
각 도메인은 현재 `error.message` 또는 generic fallback으로 처리 중. 도메인별 mutation 사용 빈도에 따라 우선순위 결정.

- [ ] **[2026-05-02] 🟡 MEDIUM checkout-frontend-error-mapper** — `CheckoutNotFound = 'CHECKOUT_NOT_FOUND'` — `apps/frontend/lib/errors/checkout-errors.ts` 신설. 호출자 `useTranslations('checkouts')` 패턴. `checkouts.json`에 `errors.notFound` 추가. checkout 뮤테이션 에러 핸들러에 연결.
- [ ] **[2026-05-02] 🟡 MEDIUM team-frontend-error-mapper** — `TeamNotFound = 'TEAM_NOT_FOUND'`, `LeaderNotFound = 'LEADER_NOT_FOUND'` — `apps/frontend/lib/errors/team-errors.ts` 신설. 호출자 `useTranslations('teams')` 패턴. `teams.json`에 `errors.notFound` / `errors.leaderNotFound` 추가.
- [ ] **[2026-05-02] 🟢 LOW user-frontend-error-mapper** — `UserRequesterNotFound = 'USER_REQUESTER_NOT_FOUND'`, `UserTargetNotFound = 'USER_TARGET_NOT_FOUND'` — 사용자 관리 도메인에서 뮤테이션 중 발생. `apps/frontend/lib/errors/user-errors.ts` 신설 또는 기존 disposal-errors.ts의 UserNotFound 패턴 확장.
...
- [ ] **[2026-05-02] 🟢 LOW notification-frontend-error-mapper** — `NotificationNotFound = 'NOTIFICATION_NOT_FOUND'` — 알림 읽음처리/삭제 뮤테이션에서 발생. `apps/frontend/lib/errors/notification-errors.ts` 신설. `notifications.json`에 `errors.notFound` 추가.
