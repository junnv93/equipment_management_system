...
- [ ] **[2026-04-30 verify-impl-batch-0430] 🟢 LOW fetchers-status-literal-ssot** — `apps/frontend/lib/api/approvals/fetchers.ts:96-97,217` URLSearchParams 인자에 raw 리터럴 3건: `'pending'`→`CheckoutStatusValues.PENDING`, `'return_to_vendor'`→`CheckoutPurposeValues.RETURN_TO_VENDOR`, `'due'`→`IntermediateCheckStatusValues.DUE`. 프론트엔드 ESLint BinaryExpression 룰 적용 범위 밖이라 현재 lint 위반 아님. SSOT 관점에서는 교체 필요. 트리거: approvals/fetchers.ts 다음 변경 PR.
