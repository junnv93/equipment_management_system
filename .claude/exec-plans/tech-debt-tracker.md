...
- [ ] **[2026-04-29 nextauth-csrf §J3] 🟢 LOW verify-routing-origin-pre-commit-hook** — `.claude/skills/verify-routing-origin/SKILL.md`가 4-layer SSOT (TypeScript / next.config.js / nginx / proxy.ts) 동기화를 검증하나 자동 실행 hook이 없음 (수동 의존). `.husky/pre-commit` 또는 `.husky/pre-push`에 path-based gate 추가 검토 — `infra/nginx/`, `next.config.js`, `proxy.ts`, `api-routing.ts` 변경 시 verify-routing-origin 자동 실행. 트리거: 본 SSOT 회귀 1회 발생 시 즉시 실행.
