...
- [ ] **[2026-05-09 three-low S-2] 🟢 LOW calibration-result-ui-options-iter** — `CalibrationHistoryClient.tsx` Result Select UI는 여전히 `<SelectItem value="pass">` 등 하드코딩 옵션. `CALIBRATION_RESULT_VALUES` iter + `CALIBRATION_RESULT_LABELS` (또는 i18n) 매핑으로 교체 시 신규 result 값 추가 시 자동 노출. 트리거: 신규 CalibrationResult enum 값 추가 (예: 'partial_pass') 또는 i18n 라벨 정합 sprint.
...
> Phase C.2 (commit `e318d3eb`)는 `--no-verify` 우회 — 다른 세션 calibration-errors.ts tsc 에러 차단 회피, 추후 정상 commit 시 검증 통과 필요.
