...
- [ ] **[2026-04-22 PR-2] 🔴 HIGH checkout-fsm E2E 격리 검증 필요** — `apps/backend/test/checkouts.fsm.e2e-spec.ts` 18개 케이스, 각 it()이 전용 장비 생성(격리 수정 완료). 라이브 DB 환경(`docker compose up -d` 후 `pnpm --filter backend run test:e2e -- checkouts.fsm`)에서 전체 PASS 확인 필요. evaluation 1차(iteration 1)에서 장비 공유 문제로 17/18 실패 — 격리 수정 후 미재검증 상태.
- [ ] **[2026-04-22 PR-2] 🟢 LOW checkout service spec mockReq permissions 패턴 문서화** — `derivePermissionsFromRoles`를 `mockReq`에 포함해야 한다는 규칙이 암묵적. FSM assertFsmAction 사용 서비스 테스트의 표준 픽스처 패턴을 `docs/references/backend-patterns.md`에 추가. 트리거: 다음 checkout 관련 서비스 작업 시.
...
- [ ] **[2026-04-22 PR-2] 🟡 MEDIUM CHECKOUT_FORBIDDEN을 400→403으로 수정** — `checkouts.service.ts:assertFsmAction`에서 `check.reason === 'permission'` 시 `BadRequestException(400)` 대신 `ForbiddenException(403)` 반환. HTTP 시맨틱 위반. E2E 테스트(`checkouts.fsm.e2e-spec.ts`) 기대값 400→403 동시 수정 필요. PR-3 진행 전 수정 권장.
- [ ] **[2026-04-22 PR-2] 🟡 MEDIUM approve-return/reject-return 컨트롤러 가드 FSM과 정렬** — `approve-return`: `VIEW_CHECKOUTS` → `APPROVE_CHECKOUT`, `reject-return`: `APPROVE_CHECKOUT` → `REJECT_CHECKOUT`. FSM requires와 HTTP 가드 계층 비대칭 해소. E2E 테스트 기대값(400→403) 수정과 함께 처리.
- [ ] **[2026-04-22 PR-2] 🟡 MEDIUM writeTransitionAudit 실패 명시적 로깅** — 7개 전이 메서드에서 `writeTransitionAudit` 실패 시 조용히 누락. `catch (auditErr)` + `this.logger.error()` 추가. 장기적으로 dead-letter 큐 연동 검토 (규제 요구사항).
- [ ] **[2026-04-22 PR-2] 🟢 LOW CheckoutErrorCode enum 도입** — `checkouts.service.ts` 에러 코드 인라인 문자열 28건을 `CheckoutErrorCode` enum/const로 상수화. 도입 후 `checkouts.service.ts` + 연관 테스트 일괄 교체.
