...
- [ ] **[2026-05-09 srp-final S-3] 🟢 LOW use-approval-row-transitions-safe-timeout** — `apps/frontend/hooks/use-approval-row-transitions.ts`가 `useRef<ReturnType<typeof setTimeout>[]>` + `useEffect` cleanup 패턴을 수동 구현. `useSafeTimeout()` SSOT(`apps/frontend/hooks/use-safe-timeout.ts`) 경유로 교체하면 중복 패턴 제거. verify-frontend-state Step 44 등록 완료 (신규 코드 회귀 차단). 트리거: approvals 훅 리팩토링 세션.
