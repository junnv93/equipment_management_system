...
- [x] ~~**[2026-05-02 inspection-pr2] 🟢 LOW inspection-checkitem-row-base-token-split**~~ — **완료 (a951015d)** — `rowBaseDecoration` 토큰 신설 + `SelfInspectionFormDialog.tsx:638` 인라인 → 토큰 경유. SSOT 드리프트 제거.
- [x] ~~**[2026-05-02 inspection-pr2] 🟢 LOW toggle-group-dark-mode-primitive-color**~~ — **완료 (a951015d)** — `judgmentToggle.itemPass/Fail/Na` + `INSPECTION_OVERALL_RESULT_TOGGLE.itemPass/Fail`에 `dark:data-[state=on]:bg-emerald-950/30 + text-emerald-400 + ring-emerald-800` 추가. outOfSpec 기존 패턴과 일관성 확보.
...
- [x] ~~**[2026-05-02 system-arch] 🟡 MEDIUM tier-2-rejectmodal-ssot-integration**~~ — **완료 (Mode 2 harness, 2 iter PASS, 2026-05-02)** — 7 backend 도메인 reject DTO Zod 격상 (`.trim().min(REJECTION_REASON_MIN_LENGTH).max(LONG_TEXT_MAX_LENGTH)`) + service-layer fail-close (7 ErrorCode enum 신설) + 6 frontend reject Dialog → RejectModal SSOT(`mode='domain'`) 통합 + 7 frontend mapper(per-domain) + i18n 4 namespace 통합(`errors.rejectionReasonRequired` + `{min}` 파라미터). **iter 2 보강**: 인접 inline string code 시스템 전반 격상 (FSM 상태 전이 7개 ErrorCode 추가 — `EquipmentImportOnlyPendingCanReject`, `CalibrationOnlyPendingCanReject`, `CalibrationFactorOnlyPendingCanReject`, `SoftwareValidationInvalidStatusTransition`, `IntermediateInspectionInvalidStatusTransition`, `SelfInspectionInvalidStatusTransition`, `NonConformanceInvalidTransition`). 시니어 자기검토 갭 1건 closure: reject method의 단편적 ErrorCode 누수. 결과: backend 1119/1119 tests + frontend 405/405 tests + tsc 0. **Out of Scope (별도 sprint)**: ① calibration-factor frontend reject UI 신규 구축 (현재 백엔드만 존재). ② 다른 흐름(approve/create 등)의 인라인 코드 격상은 도메인별 별도 sprint.
- [x] ~~**[2026-05-02 tier-2-rejectmodal 후속] 🟡 MEDIUM tier-2-calibration-factor-reject-ui-build**~~ — **완료 (2026-05-02)** — `CalibrationFactorsClient.tsx` pending 테이블에 `APPROVE_CALIBRATION_FACTOR` 권한 기반 approve/reject 버튼 추가. `useOptimisticMutation + RejectModal mode='domain' + mapCalibrationFactorErrorToToast + getCalibrationActionButtonClasses` 5-layer 패턴 적용. `calibration.errors.title` i18n 키 누락도 동시 수정. tsc 0 errors, pre-push 497 tests PASS. commit: `dea5c0db`
...
- [x] ~~**[2026-05-01 disposal-zod] 🟢 LOW disposal-comment-reject-fail-close-service-layer**~~ — **완료 (disposal-service-fail-close, 2026-05-02)** — `approveDisposal()` reject 분기에 BadRequestException(code: DISPOSAL_REJECT_COMMENT_REQUIRED) 추가, `'승인 단계에서 반려'` fallback 제거, `approvalComment` trim+null 정규화. 신규 `disposal.service.spec.ts` 31 tests로 회귀 차단. defense-in-depth 의미적 완결성 확보.
...
- [x] **[2026-05-02 error-codes-ssot] ✅ DONE audit-log-fail-close-integration** — `GlobalExceptionFilter`에 `AuditService` DI, `SECURITY_AUDITABLE_CODES` Set(31개 ErrorCode), `maybeAuditSecurityEvent` fire-and-forget 비동기 감사 로그, `__auditLogged` dedup 플래그, `audit-entity-id.util.ts` UUID 추출 헬퍼, `error.filter.spec.ts` unit test — **이미 구현됨** (commit `c8faf22f feat(security): guard-level 403 audit log gap — APP_FILTER DI + SSOT`). tech-debt 항목 생성(error-codes-ssot sprint) 시점보다 선행 구현. 2026-05-02 현황 확인으로 closure.
- [x] **[2026-05-02 error-codes-ssot] ✅ DONE tier-2-error-codes-equipment-domain-migration** — equipment 도메인 인라인 → ErrorCode enum 격상 완전 종결. service 51건(199dc407) + controller 7건(equipment-controller-errorcode-ssot) + interceptor/pipe 3건(equipment-domain-errorcode-closure) = 61건. 5-layer defense-in-depth 완성.
- [ ] **[2026-05-02 verify-implementation] 🟡 MEDIUM equipment-reject-request-dto-zod-hardening** — `apps/backend/src/modules/equipment/dto/reject-request.dto.ts:8` `rejectionReason: z.string().min(1, ...)` — `.trim()` 누락 + `REJECTION_REASON_MIN_LENGTH` 미사용 (disposal/calibration-plan과 비대칭). verify-zod Step 15b+12 이중 위반. 트리거: equipment 도메인 Zod 강화 sprint.
- [ ] **[2026-05-02 verify-implementation] 🟡 MEDIUM equipment-approval-service-fail-close-asymmetry** — `apps/backend/src/modules/equipment/equipment-approval.service.ts:559` `rejectionReason.trim().length === 0` 체크만 존재 (disposal의 `>= REJECTION_REASON_MIN_LENGTH` 대비 약한 fail-close). ErrorCode enum은 정상 사용 중. 트리거: equipment-approval 도메인 강화 sprint.
- [ ] **[2026-05-02 equipment-controller-errorcode-ssot] 🟡 MEDIUM versioned-base-service-notFoundCode-type-upgrade** — `apps/backend/src/common/base/versioned-base.service.ts:154` `notFoundCode = 'ENTITY_NOT_FOUND'` 기본값이 plain string. 모든 호출부가 기본값 사용 중이므로 `ErrorCode` 타입 파라미터로 변경 시 `EntityNotFound` ErrorCode enum 추가 필요. 트리거: versioned-base 공통 유틸 강화 sprint.
...
- [x] ~~**[2026-04-30 batch-0501 SHOULD] 🟢 LOW charscounter-unit-test**~~ — **완료 iter 3** — `apps/frontend/components/common/__tests__/CharsCounter.test.tsx` 11 case PASS (임계값 5 + warningRatio override 1 + 텍스트 fallback 2 + a11y 2 + className composition 1).
...
- [x] ~~**[2026-04-30 sprint-4.5 SHOULD 후속] 🟢 LOW verify-bulk-action-bar-step-8-group-header**~~ — **완료 (tech-debt-batch-0501)** — Step 8(group header indeterminate, `getGroupRowIds` + `deriveGroupSelectionState` + `toCheckboxCheckedProp` SSOT + Radix `data-state="indeterminate"` 자동 `aria-checked="mixed"` 매핑) + Step 9(격리 fixture page 패턴, `app/(dashboard)/__visual__/<scenario>/page.tsx` 인라인 시드 데이터 + 네트워크/세션 의존 0) 신설.
...
- [x] ~~**[2026-04-30 setqueryd-purge SHOULD] 🟢 LOW charsCount 5곳 SSOT 통합**~~ — **완료 (tech-debt-batch-0501)** — `<CharsCounter>` SSOT(`apps/frontend/components/common/CharsCounter.tsx`, memo + 글로벌 `common.charCountRatio` i18n + 80%/100% threshold 자동 토글). NCEditDialog 하드코딩 제거(CAUSE_MAX_LENGTH 상수 + `<CharsCounter>` 호출, children 생략 → 컴포넌트 내부 i18n). RejectModal 인라인 삼항분기 제거(`<CharsCounter>` + children으로 `t('rejectModal.charsRemaining')` 위임). **설계 결정**: Disposal 3개는 "min-required hint" 시맨틱(`charCountMin`)으로 별도 네임스페이스 정리 — `<CharsCounter>`(ratio+warning 시맨틱)와 다름. 결과적으로 SSOT 통합 대상은 2곳(NCEditDialog + RejectModal)이고 Disposal은 i18n 키 정리만 수행.
...
- [x] ~~**[2026-04-30 sprint-4.5 SHOULD] S7 Sidebar pendingCount 분석 이벤트**~~ — **완료 (tech-debt-batch-0501)** — `NavRowWithSecondaryAction.tsx`에 `track('sidebar.checkouts.click', { pendingCount })` 추가, `href.startsWith('/checkouts')` prefix gate로 다른 nav 클릭에 영향 없음. 두 분기(sibling-anchor + single-anchor) 메인 NavLink에 `onClick={handlePrimaryClick}` 연결. PII deny-list 위반 0(pendingCount는 숫자).
...
- [x] **[2026-04-29 nextauth-csrf §S1] 🟡 MEDIUM legacy-sw-unregister-e2e-verification** — `LegacyServiceWorkerCleanup` 컴포넌트 E2E 검증. **완료 2026-04-30 (tech-debt-batch-0430e)**: `tests/e2e/features/pwa/legacy-sw-cleanup.spec.ts` TC-01(getRegistrations 0건) + TC-02(localStorage 플래그) + TC-03(재로드 후 유지 + reload 정책 문서화) 신설. 강제 reload 없음 = ADR-0006 확인.
