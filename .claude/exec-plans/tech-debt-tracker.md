...
- [x] **[2026-04-27 checkout-sprint4-3-to-5] 🟡 MEDIUM stale-time-query-config-presets** — 36건 → 12건 배치 교체 완료(2026-04-27). 교체된 파일: ApprovalsClient, PendingChecksClient, use-equipment(2건), use-equipment-kpi(3건), use-prefetch-detail, use-breadcrumb-metadata(2건), BasicInfoTab(2건), EquipmentFilters, DocumentRevisionDialog, LeaderCombobox, equipment/test-software/user-combobox(각1건), NonConformanceManagementClient(2건), ReportsContent, CalibrationFactorsRegistryContent. 잔여 12건 의도적 보존: StorageImage(gcTime:SHORT load-bearing), combobox 4개(SHORT search-freshness preset 없음), use-equipment:84(backend협력 주석), use-management-number-check(gcTime:LONG+retry:1), use-notifications:93(preferences LONG), providers.tsx(GlobalQueryClient default). tsc 0 error.

### 2026-04-27 verify-implementation: 세션 종료 발견 항목

- [ ] **[2026-04-27 tech-debt-0427-cleanup] 🟡 MEDIUM revoke-approval-workflow-e2e** — `revokeApproval` (5분 내 승인 취소, `REVOCATION_WINDOW_EXPIRED` 에러) 서비스 구현 + M11 PASS 완료. 미완: (1) `docs/workflows/critical-workflows.md`에 WF-AP-03 워크플로우 미등록, (2) `wf-ap03-revoke-approval.spec.ts` E2E 스펙 미작성. 트리거: approvals E2E 확장 Sprint 시.
