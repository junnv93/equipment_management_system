...
- [ ] **[2026-05-09 phase-c-r2 후속] 🟢 LOW use-equipment-calibrations-hook-spec** — `apps/frontend/hooks/__tests__/use-equipment-calibrations.test.ts` (renderHook + queryKey SSOT 검증 + 두 variant export 시그니처). 본 sprint는 e2e Case 3 + tsc + verify-* SKILL grep 3단 보호로 systemic 검증 충분 — over-testing 회피로 SKIP. 트리거: hook signature 회귀 발견 또는 신규 caller 추가 시 정합 검증 필요.

### 2026-05-09 phase-c-verify-implementation-pre-existing 후속 (verify-implementation 발견)

> 본 세션 13 파일 verify-implementation 결과: 12 issues (pre-existing 11 + introduced 1 — isError 누락 commit `7945d6f3` closure). 잔여 pre-existing 11건은 본 세션 변경 영역 외부 — 별도 sprint trigger 등록.

- [ ] **[2026-05-09 verify-impl pre-existing] 🟡 MEDIUM calibration-select-item-domain-literal-ssot** — verify-ssot Step 27. CalibrationHistoryClient (L256-262, L273-276) + CalibrationContent (L364-370) SelectItem `value="approved"|"rejected"|"pending_approval"|"pass"|"fail"|"conditional"` 도메인 리터럴 직접 사용. `CalibrationApprovalStatusValues` (schemas) + `CALIBRATION_RESULT_VALUES` SSOT 경유 필요. 본 세션은 sentinel 변경만 수정 — pre-existing 리터럴은 수정 영역 외. 트리거: calibration enum SSOT 일관성 sprint 또는 새 status 추가.
- [ ] **[2026-05-09 verify-impl pre-existing] 🟢 LOW basic-info-tab-domain-literal-comparison** — verify-ssot Step 19. `BasicInfoTab.tsx:142-230` — `equipment.specMatch === 'match'/'mismatch'`, `equipment.calibrationRequired === 'required'/'not_required'`, `equipment.site === 'suwon'/'uiwang'/'pyeongtaek'` raw 리터럴 비교 7건. `SpecMatchValues` / `SiteValues` SSOT 경유 필요. 본 세션은 hook 마이그레이션만. 트리거: equipment domain enum SSOT sprint.
- [ ] **[2026-05-09 verify-impl pre-existing] 🟢 LOW upcoming-days-magic-number-to-ssot** — verify-ssot Step 35. `CalibrationHistoryClient.tsx:57` `const UPCOMING_DAYS = 30` 로컬 상수. `CALIBRATION_THRESHOLDS.CALIBRATION_WARNING_DAYS` (business-rules.ts) SSOT 동일 값 존재. `CalibrationContent` 이미 SSOT 사용. 트리거: dashboard threshold 일관성 sprint.
- [ ] **[2026-05-09 verify-impl pre-existing] 🟡 MEDIUM frontend-routes-ssot-coverage** — verify-hardcoding Step 28. (a) `CalibrationHistoryClient.tsx:186` `backUrl={\`/equipment/${equipmentId}\`}` → `FRONTEND_ROUTES.EQUIPMENT.DETAIL(equipmentId)` (b) `IncidentHistoryTab.tsx:667` `href={\`/equipment/${equipmentId}/repair-history\`}` → `FRONTEND_ROUTES.EQUIPMENT.REPAIR_HISTORY(equipmentId)` (exec-plan out-of-scope #6 명시) (c) `CalibrationContent.tsx:226` `router.push('/calibration/register')` → `FRONTEND_ROUTES.CALIBRATION.REGISTER`. 트리거: routing SSOT enforcement sprint.
- [ ] **[2026-05-09 verify-impl pre-existing] 🟢 LOW design-token-bg-brand-ok-and-page-container-variant** — verify-design-tokens Step 28+39. (a) `MaintenanceHistoryTab.tsx:326` `bg-brand-ok` 인라인 → `getTimelineNodeClasses('bg-brand-ok')` SSOT (b) `CalibrationHistoryClient.tsx:182` + `CalibrationContent.tsx:219` `getPageContainerClasses()` 빈 호출 → `'list'` variant 명시. 트리거: design token coverage sprint.
- [ ] **[2026-05-09 verify-impl pre-existing] 🟢 LOW calibration-history-page-server-filter-parsing** — verify-filters Step 4. `app/(dashboard)/equipment/[id]/calibration-history/page.tsx` 서버 사이드 `parseFiltersFromSearchParams` 미호출 — Server Component prefetch에서 필터 미반영, SSR 초기 필터 상태 미지원. 본 세션 sub-route 신설 sprint(`calibration-cert-phase-a-architecture-closure`)에서 도입된 page.tsx — 본 세션 변경 영역 외부. 트리거: SSR-driven filter prefetch 요구 또는 deep-link share 성능 issue.
- [ ] **[2026-05-09 verify-impl pre-existing] 🟢 LOW calibration-history-client-filter-utils-extraction** — verify-filters Step 8. `CalibrationHistoryClient.tsx:118-130` `URLSearchParams + params.set/delete` 인라인 조작. exec-plan 명시 의도적 결정 (premature util 회피, 단일 caller 4 필드). 트리거: 신규 caller 추가 또는 메인 `/calibration` `calibration-filter-utils.ts` 통합 sprint.
...
### ~~2026-05-08 backend-zod-error-i18n-ssot 후속~~ ✅ CLOSED 2026-05-08

> `zod-i18n-mapper-hub-closure` Mode 2 Full harness PASS (iter 3, MUST 21/21). 12 domain mapper hub 통합 + ts-morph spec + Prometheus Counter 완전 종결.

### 2026-05-08 zod-i18n-mapper-hub-closure 후속 (SHOULD S-4)

- [ ] **[2026-05-08 zod-hub S-4 후속] 🟢 LOW zod-validation-issues-alert-rule** — `zod_validation_issues_total` Counter 등록 완료. 운영 단계에서 `issue_count_bucket="11+"` rate p95 지속 발생 시 Prometheus AlertManager rule 추가 (ADR-0008 Trigger Conditions §4 신규 reconsideration trigger). 트리거: `issue_count_bucket="11+"` bucket rate p95 임계값 초과 감지 시.
- [ ] **[2026-05-08 zod-hub S-4 후속] 🟢 LOW zod-fallback-eslint-custom-rule-migration** — ts-morph static spec(`zod-fallback-coverage.test.ts`)이 SSOT 역할 중. 신규 도메인 mapper 추가 시 ESLint custom rule로 마이그레이션하면 빌드타임 대신 편집기 실시간 피드백 가능. 현재 ts-morph spec으로 충분 — 신규 도메인 mapper 추가 빈도 높아지면 전환 검토. 트리거: 신규 도메인 mapper 추가 3회 이상.
- [ ] **[2026-05-08 zod-hub S-4 후속] 🟢 LOW e2e-zod-fail-toast-verification** — 백엔드 Zod 검증 실패 → 프론트엔드 toast i18n 렌더링을 실제 브라우저에서 검증하는 e2e spec 미보유. 현재 ts-morph 정적 + backend unit 테스트로 커버하나 런타임 toast 노출 경로 미검증. 트리거: i18n errors.validation 키 누락 운영 incident 발생 시.
...
### 2026-05-09 ADR-0008 tErrors chain 후속 (SHOULD)

- [ ] **[2026-05-09 tErrors S-1] 🟡 MED errors.genericError baseline key 추가** — disposal/notifications/teams/users 4개 도메인 × ko/en 8파일에 `errors.genericError` 키 누락. 현재 mapper들이 직접 호출하지 않아 런타임 영향 없음. 단, 추후 fallback 경로에 `t('errors.genericError')` 추가 시 raw key 노출 위험. verify-i18n Step 20 FAIL gate. 대상: `messages/{ko,en}/{disposal,notifications,teams,users}.json`
