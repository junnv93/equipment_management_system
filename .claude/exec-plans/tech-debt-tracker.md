...
### ~~2026-05-09 phase-c-verify-implementation-pre-existing 후속~~ ✅ CLOSED 2026-05-09

> `verify-impl-preexisting-ssot-closure` Mode 2 harness PASS (iter 1, MUST 26/26). 6 파일 수정. SelectItem SSOT × 2 파일, BasicInfoTab 리터럴 비교 7건, UPCOMING_DAYS → CALIBRATION_THRESHOLDS, FRONTEND_ROUTES 3건, getTimelineNodeClasses + getPageContainerClasses('list'), calibration-history page.tsx searchParams + initialFilters 전달. Issue 7 (filter-utils-extraction) no-change 확정 (단일 caller + 컨벤션 일치).


- [ ] **[2026-05-09 verify-impl-preexisting S-2] 🟢 LOW calibration-history-result-filter-type-narrowing** — `CalibrationHistoryClient.tsx` `ResultFilter = '' | 'pass' | 'fail' | 'conditional'` 인라인 union → `'' | CalibrationResult` 타입으로 좁히기. 트리거: `CalibrationResult` 타입 변경 또는 result 값 추가 시.
