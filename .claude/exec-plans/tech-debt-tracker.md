...
- [ ] **[2026-04-20 e2e-api-endpoints-migration] POST /checkouts/:id/return HTTP 메서드 불일치** — `checkouts.e2e-spec.ts`. SSOT(`CHECKOUTS.RETURN`)는 PATCH이나 실제 테스트는 POST 유지. NestJS 컨트롤러 메서드 확인 후 정합 필요.
...
- [ ] **[2026-04-20 calibration-plan-confirm] S5 — confirmAllItems 서비스 단위 테스트 미작성** — `calibration-plans.service.spec.ts`. 정상(2건), non-approved 400, actualCalibrationId 없는 항목 미포함, CAS 409 케이스 추가 권장.
- [ ] **[2026-04-20 calibration-plan-confirm] S6 — 렌더러 테스트에 확인 셀 assertion 없음** — `calibration-plans-export.service.spec.ts`. 두 확인자 이름이 각 셀에 기록되는지 + confirmedBy 없는 항목이 `'-'`인지 검증하는 assertion 추가.
