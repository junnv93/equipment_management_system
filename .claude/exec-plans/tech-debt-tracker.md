...
- [ ] **[2026-04-27 approvals-ui-r2] 🟡 MEDIUM ar-13-self-inspection-category** — `self_inspection` ApprovalCategory enum 미포함. backend `self-inspections` 모듈 승인 엔드포인트 존재 여부 확인 후 `APPROVAL_CATEGORY_VALUES` + `ROLE_APPROVAL_CATEGORIES` 추가 결정. 트리거: self-inspection 승인 워크플로우 설계 시.
...
- [ ] **[2026-04-27 sprint4-3-to-5] 🔴 HIGH rejection-presets-seed** — `rejection_presets` 테이블·컨트롤러·서비스 완료(M9 PASS). **미완**: seed 데이터 미삽입 — UL-QP-18 절차서 기준 반려 사유 프리셋 5건 사용자 확인 후 `seed-test-new.ts`에 추가 필요. 마이그레이션(`0047_add_rejection_presets.sql`) 존재 — `db:reset`/`db:migrate` 실행 시 자동 적용. 트리거: 반려 사유 프리셋 UI 개발 착수 시.
...
### 2026-04-27 verify-implementation: approvals-ar7-ar6-e2e 검증 이슈

- [ ] **[2026-04-27 approvals-ar7-ar6-e2e] 🟢 LOW approval-detail-modal-deprecated-section-tokens** — `ApprovalDetailModal.tsx:28,157,171`에서 `APPROVAL_DETAIL_SECTION_TOKENS` 사용 중. `approval.ts:562`에 `@deprecated use APPROVAL_DETAIL_MODAL_TOKENS.sectionBody` 표시됨. 기능 정상이나 deprecated API 사용. 수정: `APPROVAL_DETAIL_SECTION_TOKENS` → `APPROVAL_DETAIL_MODAL_TOKENS.sectionBody` 교체. 트리거: ApprovalDetailModal 다음 수정 시.
