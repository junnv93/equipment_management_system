...
- [ ] **[2026-05-02 versioned-base-upgrade 후속] 🟡 MEDIUM notFoundCode-type-full-migration** — `versioned-base.service.ts:154` `notFoundCode: string` → `notFoundCode: ErrorCode` 완전 타입 격상. 전제: 38개 호출부 string literal 10종 (`CABLE_NOT_FOUND`/`NC_NOT_FOUND`/`SELF_INSPECTION_NOT_FOUND`/`INTERMEDIATE_INSPECTION_NOT_FOUND`/`SOFTWARE_VALIDATION_NOT_FOUND`/`TEST_SOFTWARE_NOT_FOUND`/`CALIBRATION_PLAN_NOT_FOUND`/`IMPORT_NOT_FOUND`/`TEST_PLAN_NOT_FOUND`) → ErrorCode enum 추가 + 38개 호출처 수정. tsc 0 에러. 트리거: tier-2-error-codes-other-domains-migration 진행 시.
