...
- [ ] **[2026-04-30 deps-supply-chain] 🔴 IMMEDIATE A3 ci-supply-chain-gate** — `.github/workflows/supply-chain-gate.yml` 신설. PR 머지 전 `node scripts/check-dependabot-drift.mjs` + `pnpm audit:dependabot` 실행. preinstall 우회(`--ignore-scripts`) 방어. 본 세션 supply-chain hardening 작업의 진짜 완성. 비용 30분. 트리거: 다음 세션 head.
- [ ] **[2026-04-30 deps-supply-chain] 🔴 IMMEDIATE A1 mock-providers-identifier-helper** — `apps/backend/src/common/testing/mock-providers.ts`에 `createMockIdentifierService()` 헬퍼. `@Global()` 모듈은 `Test.createTestingModule`에서 자동 import 안 됨 → 향후 신규 spec 작성 시 silent DI failure 방지. verify-ssot Step 44에 mock 등록 grep 한 줄 추가. 비용 15분. 트리거: 다음 세션 head.
- [ ] **[2026-04-30 deps-supply-chain] 🔴 IMMEDIATE A5 identifier-policy-docs** — `docs/references/identifier-policy.md` 신설 (≤80 lines). IdentifierService 도입 트레이드오프(ulid/nanoid 미선택 사유, hex prefix scheme), 4개 도메인 헬퍼 명명 규칙, 호출처 패턴(@Injectable DI vs 모듈 함수 import). verify-ssot Step 44가 *왜* 존재하는지의 외부화. 비용 30분. 트리거: 다음 세션 head.
- [ ] **[2026-04-30 deps-supply-chain] 🔴 IMMEDIATE A6 eslint-no-restricted-imports-crypto** — `apps/backend/.eslintrc.js`에 `no-restricted-imports` 규칙 — `node:crypto` (identifier.service.ts 외) 차단 + `crypto.randomUUID` member call 차단. verify-ssot Step 44 정적 grep으로 못 잡는 우회 패턴(`* as crypto`, alias rename) 이중 가드. 비용 10분. 트리거: 다음 세션 head.
