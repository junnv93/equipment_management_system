...
- [ ] `disposal-review-tech-manager.spec.ts:40` 토스트/상태 배지 어느 것을 검증하려 했는지 root-cause 조사 — `getByText(/검토|승인|완료/i).first()` regex가 너무 광범위해 의도 불명. 백엔드 액션이 무엇을 토스트로 띄우는지 + 상태 변경이 어떻게 표시되는지 확인 후 helper 마이그레이션 또는 명시적 배지 셀렉터로 분기 — `apps/frontend/tests/e2e/features/equipment/detail/group4-disposal-workflow/disposal-review-tech-manager.spec.ts:40` — 2026-04-08
