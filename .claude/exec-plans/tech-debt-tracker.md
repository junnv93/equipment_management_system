...
### 2026-05-12 checkouts-sprint4-followups-sh1-sh7 후속 (SH-8)

> SH-7 통합 작업 중 발견된 architectural debt. sprint scope 초과 → 별도 sprint 분리.

- [ ] **[2026-05-12 checkouts-sprint4-followups-sh1-sh7 SH-8] 🟡 MED use-undo-toast-revoke-policy** — `apps/frontend/lib/api/checkout-revoke-approval.ts:10` 의 `revokeApproval(checkoutId)` 가 body 없이 POST → backend `revoke-approval.dto` (version + reason 필수) Zod 검증 항상 400 → `use-undo-toast.tsx:49` 의 5초 undo 액션이 사실상 broken (catch → undoError toast). **정책 결정 필요**: (a) backend `/api/checkouts/:id/undo-approval` 별도 endpoint 신설 (no reason, 5초 윈도우 strict) 또는 (b) frontend 에서 system reason (i18n key `checkouts.undo.systemReason`) 자동 전달 — UL-QP-18 정책상 "철회는 사용자 사유 필수" 위반 여부 확인 필요. 동반: `use-undo-toast.tsx` 시그니처에 version source 추가 (queryClient.getQueryData callback). WF-AP03 e2e 는 backend direct call 만 검증 — frontend wire 회귀 차단 spec 추가. 트리거: sprint 4 사용자 피드백 또는 use-undo-toast undo 실패 incident 보고.
