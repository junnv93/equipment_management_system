...
- [ ] **[2026-04-28 dashboard-redesign] 🟢 LOW kpi-status-grid-min-h-token** — `CalibrationDdayList.tsx:63`의 `minH = 280` 인라인 픽셀값. `DASHBOARD_SKELETON_MIN_H` 또는 `DIMENSION_TOKENS`로 토큰화 권고. 트리거: 디자인 토큰 정비 Sprint.
- [ ] **[2026-04-28 dashboard-redesign] 🟢 LOW system-health-react-query-monitoring-polling** — `useQuery` config가 `QUERY_CONFIG.DASHBOARD = REFETCH_STRATEGIES.NORMAL`로 탭 포커스에만 갱신. 시스템관리자 시스템 상태는 `QUERY_CONFIG.MONITORING(5분 폴링)`이 더 적합. 트리거: monitoring UX 개선 Sprint.
- [ ] **[2026-04-28 dashboard-redesign] 🟢 LOW welcome-suffix-en-empty** — `apps/frontend/messages/en/dashboard.json`의 `welcome.suffix` 빈 문자열 (사후에 한국어 "님" 같은 호칭이 영어에 불필요해 의도된 결과지만, lint/i18n-check가 null 또는 "" 인지 명시 권장). 트리거: i18n 키 정합성 점검 Sprint.

### 2026-04-28 manage-skills P2 후속 (다음 세션 권장)

- [ ] **[2026-04-28 manage-skills] 🟢 LOW verify-ssot-step-36-effective-role** — `useEffectiveRole` SSOT 경유 강제 룰 추가. 클라이언트 컴포넌트에서 `session.user.role` 직접 참조 시 `?simulateRole=` 시뮬레이션 우회 위험. 트리거: 다음 세션.
- [ ] **[2026-04-28 manage-skills] 🟢 LOW verify-frontend-state-step-24-dual-mode-asymmetry** — Dual-Mode 비대칭 props 탐지. CheckoutCard 같은 dual-mode 컴포넌트에서 controlled props 절반만 주입(예: `overdueCheckouts` 있고 `upcomingCheckoutReturns` 없음) 시 silent bug 유발. 트리거: 다음 세션.
- [ ] **[2026-04-28 manage-skills] 🟢 LOW verify-frontend-state-step-21-online-status-ssot** — `useOnlineStatus` SSOT 훅 강제. `navigator.onLine` 인라인 사용 방지. 트리거: 다음 세션.
- [ ] **[2026-04-28 manage-skills] 🟢 LOW verify-design-tokens-step-43-error-fallback** — ErrorBoundary fallback이 `EmptyState variant="error"` 경유 강제. 인라인 `role="alert"` div 작성 금지. 트리거: 다음 세션 (Step 43은 이미 추가됐으나 ErrorBoundary 검증 grep만 보강).
