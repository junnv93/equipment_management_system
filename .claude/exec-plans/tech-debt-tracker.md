...
- [ ] **[2026-04-27 vi] 🟢 LOW workflow-panel-urgency-dot-animation-ssot** — `workflow-panel.ts` L35-36 `urgencyDot.warning/critical`에서 `'motion-safe:animate-pulse'` 인라인. `ANIMATION_PRESETS.pulse` 경유로 교체하면 DRY 강화. 트리거: workflow-panel 토큰 리팩토링 시.
- [ ] **[2026-04-27 vi] 🟢 LOW condition-check-query-key-tier** — `ConditionCheckClient.tsx` L58 `queryKeys.checkouts.all` 직접 invalidate — view/resource 계층 경유로 교체 필요 (대상 외 파일). 트리거: checkouts queryKeys 감사 시.
...
- [ ] **[2026-04-27 equipment-pwa-audit-i18n] 🟢 LOW formatFilters-compound-i18n** — `PrintableAuditReport.tsx` `formatFilters()` 내부 `대상=`, `액션=`, `사용자=`, `시작일=`, `종료일=`, `전체` 6개 compound 문자열 하드코딩 잔존. 단순 key 교체가 아닌 format message 패턴(`{label}: {value}`)으로 리팩토링 필요. 트리거: audit 보고서 i18n 리뷰 시 또는 영문 프린트 보고서 요구사항 발생 시.
...
- [ ] **[2026-04-27 dashboard-design-review] 🟢 LOW ap16-ssr-strategy-docs** — `DashboardClient.tsx` AP-16 `dynamic({ ssr: true })` 주석이 "First Load JS -15~30KB"로 기재되나, `ssr: true`는 서버 번들 포함이므로 실제 First Load JS 감소 효과는 클라이언트 청크 분리에 한정됨. 주석 수정 또는 `ssr: false` 전환 + 수화 전략 검토 필요. 트리거: Phase 4.5 Bundle baseline 갱신 시 실측 후 판단.
- [ ] **[2026-04-27 dashboard-design-review] 🟢 LOW dashboard-client-row-extraction** — `DashboardClient.tsx` 445라인. Phase 6 AP-15 DashboardRow0~4 추출 시 ~60라인으로 감소 예정. 트리거: Phase 4~5 완료 후 Phase 6 착수.
