...
- [ ] **[2026-04-19 test-software-detail-refactor] S1 — optimisticUpdate `if (!old) return old!` non-null assertion** — `TestSoftwareDetailContent.tsx` lines 161, 177. `old`가 undefined일 때 타입 체커를 우회. 실용적 위험 없음(cancelQueries 후 캐시 항상 존재)이나 엄밀한 패턴으로 교체 권장: `if (!old) throw new Error('cache miss in optimistic update')`.
