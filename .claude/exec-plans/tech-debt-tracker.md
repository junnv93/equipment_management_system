...
### 2026-05-03 errorcode-residual-closure verify-implementation 발견 항목

- [ ] **[2026-05-03] 🟢 LOW verify-security-RevocationReasonRequired-comment** — `security-auditable-codes.ts`: `RevocationReasonRequired`가 400(유효성 오류)임에도 보안 감사 등록된 이유(fail-close defense-in-depth 의도)를 인라인 주석으로 명시 필요. 트리거: security-auditable-codes 문서화 sprint.
- [ ] **[2026-05-03] 🟢 LOW frontend-enum-ssot-equipment-calibration** — `apps/frontend/lib/errors/equipment-errors.ts:18` `EquipmentErrorCode` / `calibration-errors.ts:13` `CalibrationErrorCode` 로컬 enum이 `packages/schemas` ErrorCode 미격상 잔여. packages/schemas로 격상 시 B-option specific routing 패턴 유지 필요. 트리거: frontend-error-enum-ssot-migration sprint.
- [ ] **[2026-05-03] 🟢 LOW revoke-approval-dto-spec-trim-symmetry** — `revoke-approval.dto.ts`는 `.trim().min(REVOCATION_REASON_MIN_LENGTH)` 패턴 사용하나 전용 spec 파일(`revoke-approval.dto.spec.ts`) 미존재. verify-zod Step 17 trim→reject + trim→accept 양방향 케이스 추가 필요. 트리거: dto spec coverage 강화 sprint.

### 2026-05-03 entropy-pass + verify-implementation 발견 항목

- [ ] **[2026-05-03] 🟡 MEDIUM validation-rules-domain-constants** — `create-cable.dto.ts`/`update-cable.dto.ts` max(20/50/100), `create-notification.dto.ts` max(100/500/200), `bulk-reject/approve.dto.ts` max(50), `create-equipment-import.dto.ts` dateRange 영어 메시지 — `VALIDATION_RULES`에 미등록된 도메인 상수들. 트리거: VALIDATION_RULES 도메인 확장 sprint (cables 전용 상수 + NOTIFICATION_TITLE_MAX_LENGTH + LINK_URL_MAX_LENGTH + BULK_MAX_COUNT 신설 + VM.equipmentImport.dateRangeInvalid 추가).
- [ ] **[2026-05-03] 🟢 LOW bulk-ops-array-message-ssot** — `bulk-reject.dto.ts`/`bulk-approve.dto.ts` ids 배열의 `.min(1, '최소 1건...')` / `.max(50, '최대 50건...')` — VM SSOT 미사용 인라인 한국어 문자열. VM에 `array.minCases`/`array.maxCases` ("건" 단위) 추가 필요. 트리거: VM 확장 + bulk DTO 메시지 SSOT 정리 sprint.
- [ ] **[2026-05-03] 🟢 LOW refine-messages-ssot** — `create-checkout.dto.ts` 중복 UUID refine 메시지 `'동일한 장비를 중복으로 선택할 수 없습니다'`, `create-equipment-import.dto.ts` dateRange refine 영어 메시지, `create-measurement.dto.ts`/`handover-token.dto.ts` 인라인 문자열 — VM.equipmentImport/VM.checkout에 해당 메시지 키 추가 필요. 트리거: VM 메시지 레지스트리 확장 sprint.
