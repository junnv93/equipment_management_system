...
- [~] `checkouts.service.spec.ts` SQL-shape 회귀 unit 테스트 부재 — 부분 해결: 2026-04-08 — `checkout-scope.util.spec.ts` (135줄, 4-case predicate scenarios) 가 병렬 세션에서 합류 → SSOT helper 자체 회귀 가드 확보. 원본 `checkouts.service.spec.ts` 는 여전히 SQL 조건 검증 미지원(현 mock 인프라가 select/from/innerJoin/where 체인을 only 식별자만 추적) — controller 레벨 e2e (TC-08) + util 단위 테스트로 갭이 좁혀졌으나 service 메서드별 SQL shape 회귀는 여전히 갭. mock 인프라 확장(SQL chunk 캡처 lambda)을 별도 작업으로 분리 — 2026-04-08
