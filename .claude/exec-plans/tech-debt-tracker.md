...
- [ ] **[2026-04-20 calibration-status-filter-url] E2E 테스트 deprecated status 파라미터 잔존** — `tests/e2e/features/equipment/list/group-a-calibration-display.spec.ts:78`, `group-a-filter-status.spec.ts:126–127`. `status=calibration_overdue` → `calibrationDueFilter=overdue`로 교체 필요. 현재 백엔드 400 또는 빈 결과를 받을 가능성.
