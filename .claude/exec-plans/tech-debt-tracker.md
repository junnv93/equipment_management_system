...
- [ ] **[2026-05-02 system-arch] 🟡 MEDIUM tier-2-rejectmodal-ssot-integration** — `apps/frontend/components/approvals/RejectModal.tsx`(REJECTION_MIN_LENGTH SSOT + CharsCounter + RejectReasonSchema)를 7 도메인 일괄 통합. 대상 호출처:
   - `EquipmentImportDetail.tsx:567` (자체 dialog → RejectModal)
   - `CalibrationApprovalActions.tsx:108` (자체 dialog → RejectModal)
   - `IntermediateInspectionList.tsx:263` (자체 dialog → RejectModal)
   - `NCDetailClient.tsx:614` (자체 dialog → RejectModal)
   - `SelfInspectionFormDialog.tsx` reject 분기 (자체 dialog → RejectModal)
   - `software-validations/[id]/validation/_components/ValidationRejectDialog.tsx` (자체 dialog → RejectModal)
   - `calibration-factors/<TBD>` (반려 dialog → RejectModal)
   각 통합과 동시에 backend Zod도 `.trim().min(REJECTION_REASON_MIN_LENGTH).max(LONG_TEXT_MAX_LENGTH)` 페어링 격상 (frontend ≥10 룰이 RejectModal로 자동 획득되므로 회귀 위험 LOW).
   **Mode 2 sprint** (frontend 통합 + backend 7 dto 격상 + 전수 e2e 회귀 = 12+ files). 트리거: 다음 시스템적 sprint.
