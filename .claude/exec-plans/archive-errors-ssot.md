...
### equipment-reject-zod-fail-close-hardening — 완료 (Mode 1 harness, 2 iter PASS — 9/9 MUST)

- [x] **[2026-05-02 verify-implementation] 🟡 MEDIUM equipment-reject-request-dto-zod-hardening** — ✅ 완료 (equipment-reject-zod-fail-close-hardening, 2026-05-02). `reject-request.dto.ts` `.trim().min(REJECTION_REASON_MIN_LENGTH).max(LONG_TEXT_MAX_LENGTH)` SSOT 격상. VALIDATION_RULES import. 하드코딩 숫자 0건.
- [x] **[2026-05-02 verify-implementation] 🟡 MEDIUM equipment-approval-service-fail-close-asymmetry** — ✅ 완료 (equipment-reject-zod-fail-close-hardening, 2026-05-02). `equipment-approval.service.ts` fail-close `trim().length === 0` → `< VALIDATION_RULES.REJECTION_REASON_MIN_LENGTH`. disposal.service.ts와 완전 대칭. VALIDATION_RULES import 추가.
- [x] **[2026-05-02 equipment-controller-errorcode-ssot] 🟡 MEDIUM versioned-base-service-notFoundCode-type-upgrade** — ✅ 부분 완료 (equipment-reject-zod-fail-close-hardening, 2026-05-02). `EntityNotFound = 'ENTITY_NOT_FOUND'` enum 신설 + mapping 404. `notFoundCode` 기본값: string literal → `ErrorCode.EntityNotFound` SSOT 격상. 파라미터 타입 완전 격상은 별도 sprint (`notFoundCode-type-full-migration`). backend 1132/1132 tests PASS, tsc 0 errors.
