...
commits:
  ...
  - 14684717 — feat(dashboard): expose dbSizeBytes hint when storage unmeasured (senior follow-up)
---
...
- `backendInfoTrigger`: tooltip trigger button (focus-visible ring)
- `backendInfoIcon`: 작은 ⓘ 아이콘 (`h-3 w-3 text-muted-foreground/70`)
- `backendUnmeasured`: pg-database 측정 불가 inline 라벨 (warn 톤)
...
- `dashboard.systemHealth.backend.storage.{host-disk,configured-capacity,pg-database}`
- `dashboard.systemHealth.backend.queue.{pending-work-aggregate,bullmq}`
- `dashboard.systemHealth.backend.error.{system-error-events,audit-rejection-proxy}`
- `dashboard.systemHealth.backendInfoLabel` ("데이터 소스" / "Data source")
...
- loading 케이스 + metrics undefined fallback 케이스
...
## Recovery Note
2026-05-06 진행 중, 다른 Claude 세션의 lint-staged stash로 untracked 파일(contract.md, test.tsx)이 일시 손실. modified 파일은 stash@{0} `other-session-WIP-2026-05-06`에서 surgical checkout으로 복원, untracked 파일은 conversation context에서 재생성.
