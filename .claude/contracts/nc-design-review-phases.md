...
### Phase 3 완료 (2026-04-24, commit `3a41918c`)

**완료된 작업:**
- `GuidanceCallout.tsx` 전면 재작성:
  - `isHero = entry.ctaKind !== 'none'` → size='hero'/'default' 동적 선택
  - `--callout-hero-shadow` CSS 변수 주입 (color-mix)
  - role chip: `getRoleChipClasses(entry.roleChip)` + step badge 옆에 렌더
  - CTA: `NC_GUIDANCE_CTA_TOKENS.primarySolid(variant)` / `secondaryOutlined`
  - `role="alert"` (hero) / `role="status"` (non-hero) + `aria-live="polite"` 양쪽 모두
- `non-conformance.ts` compact 토큰 추가:
  - `NC_WORKFLOW_TOKENS.compactDot` (5 variant)
  - `NC_WORKFLOW_TOKENS.compactConnector` (base/done/pending)
  - `NC_WORKFLOW_TOKENS.compactCurrentLabel` / `compactCurrentDate`
  - `getNCWorkflowCompactDotClasses(idx, currentIdx, isLongOverdue)` 유틸
  - `NC_SPACING_TOKENS.detail.calloutTimelineGap` 추가, `calloutAfterTimeline` @deprecated
- `NCDetailClient.tsx`:
  - `WorkflowTimeline` compact prop 지원
  - `isClosed` 분기: closed → Timeline full / open → Callout hero + Timeline compact
  - `calloutAfterTimeline` wrapper div 제거 (space-y-3 statusGroup 간격 담당)
  - `CompactStepDate` 헬퍼 추가
- `index.ts` `getNCWorkflowCompactDotClasses` re-export
- i18n ko/en `detail.timeline.label` 키 추가

### Phase 4 진입 체크리스트 (다음 세션 시작점)

1. `git log --oneline -3` — `3a41918c` 확인
2. Phase 4 작업:
   - `NonConformanceBanner.tsx`: variant='compact' prop + CompactBanner
   - `NCDocumentsSection.tsx`: view state + list markup + AttachmentThumbnail size='xs'
   - `NCEditDialog.tsx`: REQUIRED_FIELD_TOKENS + useCasGuardedMutation 전환
   - `NCRepairDialog.tsx`: 2-step (input → confirm) + useCasGuardedMutation 전환
   - `NonConformancesContent.tsx`: row memoized chip
   - E2E 스펙 갱신 (2-step 플로우, compact banner selector)
   - bundle-baseline.json 갱신
3. `useCasGuardedMutation` 시그니처 재확인:
   `hooks/use-cas-guarded-mutation.ts`
